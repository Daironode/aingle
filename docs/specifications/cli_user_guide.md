# AIngle CLI Instruction

[English|[中文](cli_user_guide_CN.md)]

AIngle CLI is an AIngle node command line client for starting and managing AIngle nodes, managing user wallets, sending transactions, and deploying and invoking contracts.

* [AIngle CLI Instruction](#aingle-cli-instruction)
	* [1. Start and Manage AIngle Nodes](#1-start-and-manage-aingle-nodes)
		* [1.1 Startup Parameters](#11-startup-parameters)
			* [1.1.1 AIngle System Parameters](#111-aingle-system-parameters)
...
## 1. Start and Manage AIngle Nodes

AIngle CLI has a lot of startup parameters for configuring some of the AIngle node's behavior. Use ./AIngle -help to see all startup parameters supported by the AIngle CLI node. If AIngle CLI is started without any parameters, it will access the AIngle main network as a synchronous node by default.

### 1.1 Startup Parameters

The following are the command line parameters supported by AIngle CLI:

#### 1.1.1 AIngle System Parameters

--config
The config parameter specifies the file path of the genesis block for the current Ontolgy node. If not specified, AIngle will use the config of Polaris TestNet. Note that the genesis block configuration must be the same for all nodes in the same network, otherwise it will not be able to synchronize blocks or start nodes due to block data incompatibility.

--loglevel
The loglevel parameter is used to set the log level the AIngle outputs. AIngle supports 7 different log levels, i.e. 0:Trace 1:Debug 2:Info 3:Warn 4:Error 5:Fatal 6:MaxLevel. The logs are logged from low to high, and the log output volume is from high to low. The default value is 2, which means that only logs at the info level or higher level.

--disable-event-log
The disable-event-log parameter is used to disable the event log output when the smart contract is executed to improve the node transaction execution performance. The AIngle node enables the event log output function by default.
...
--wallet, -w
The wallet parameter is used to specify the wallet file path when the AIngle node starts. The default value is "./wallet.dat".

--account, -a
The account parameter is used to specify the account address when the AIngle node starts. If the account is null, it uses the wallet default account.

--password, -p
The password parameter is used to specify the account password when AIngle node starts. Because the account password entered in the command line is saved in the log, it is easy to leak the password. Therefore, it is not recommended to use this parameter in a production environment.
...
--disable-rpc
The disable-rpc parameter is used to shut down the EPC server. The AIngle node starts the RPC server by default at startup.
...
--testmode
The testmode parameter is used to start a single node test network for ease of development and debug. In testmode, AIngle will start RPC, RESTful and WebSocket server, and blockchain data will be clear generated by the last start in testmode.
...
The disable-tx-pool-pre-exec parameter is used to disable preExecution of a transaction from network in the transaction pool. By default, preExecution is enabled when aingle bootstrap.
...
The disable-broadcast-net-tx is used to disable broadcast a transaction from network in the transaction pool. By default, this function is enabled when aingle bootstrap.

### 1.2 Node Deployment

#### 1.2.1 MainNet Bookkeeping Node Deployment

According to different roles of nodes, they can be divided into bookkeeping nodes and synchronization nodes. Bookkeeping nodes participate in the network consensus, and synchronization nodes only synchronize the blocks generated by the bookkeeping nodes. Since AIngle node won't start consensus by default, consensus must be turned on by the --enable-consensus parameter. The AIngle node will start the RPC server by default and output the event log of the smart contract. Therefore, if there is no special requirement, you can use the --disable-rpc and --disable-event-log command line parameters to turn off the RPC and eventlog modules.

Recommended bookkeeping node startup parameters:

```
./AIngle --enbale-consensus --disable-rpc --disable-event-log
```
...
AIngle supports single-node network deployment for the development of test environments. To start a single-node test network, you only need to add the --testmode command line parameter.
...
Note that, AIngle will turn consensus RPC, RESTful, and WebSocket server on in test mode.

## 2. Wallet Management

Wallet management commands can be used to add, view, modify, delete, and import account.
You can use ./AIngle account --help command to view help information of wallet management command.

### 2.1. Add Account

AIngle supports multiple encryption algorithms, including ECDSA, SM2, and ED25519.
...
Each wallet has a default account, which is generally the first account added. The default account cannot be deleted, you can modify the default account by ./AIngle account set command.
...
```
./AIngle account add --default
```

You can view the help information by ./AIngle account add --help.
...
In AIngle CLI, you can find accounts by Index, Address, or a non-empty Label.

Use --v to view the details of the account. You can view the help information via ./AIngle account list --help.

### 2.3 Modify Account

Using the modify account command to modify the account's label, reset the default account, modify the account's password. If the account is the key of the ECDSA encryption algorithm, you can also modify the key's signature scheme. You can view the help information via ./AIngle account add --help.
...
```
./AIngle account set --d <address|index|label>
```
**Edit account label**

```
./AIngle account set --label=XXX <address|index|label>
```
**Change account password**

```
./AIngle account set --changepasswd <address|index|label>
```

**Modify ECDSA key signature scheme**

```
./AIngle account set --s=SHA256withECDSA <address|index|label>
```
### 2.4 Delete Account
...
```
/AIngle account del <address|index|label>
```
### 2.5 Import Account
...
```
./AIngle asset transfer --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=ont
```
...
```
./AIngle asset approve --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=ont
```
...
```
./AIngle asset allowance --from=<address|index|label> --to=<address|index|label>
```
...
```
./AIngle asset transferfrom --from=<address|index|label> --to=<address|index|label> --sender=<address|index|label> --amount=XXX
```
...
```
./AIngle asset unboundong <address|index|label>
```
...
**Extract Unbound ONG**
```
./AIngle asset withdrawong <address|index|label>
```
## 4. Query Information

Query information command can query information such as blocks, transactions, and transaction executions. You can use the ./AIngle info block --help command to view help information.

### 4.1 Query Block Information

```
./AIngle info block <height|blockHash>
```
...
```
./AIngle contract deploy --name=xxx --code=xxx --author=xxx --desc=xxx --email=xxx --needstore --gaslimit=100000000
```
...
```
./AIngle contract invoke --address=XXX --params=XXX --return=XXX --p
```
Return example:
...
```
./AIngle contract invoke --address=XXX --params=XXX --gaslimit=XXX
```
...
AIngle supports direct execution of smart contact code after deploying a contract.
...
```
./AIngle contract invokeCode --code=XXX --gaslimit=XXX
```

## 6. Block Import and Export

AIngle CLI supports exporting the local node's block data to a compressed file. The generated compressed file can be imported into the AIngle node. For security reasons, the imported block data file must be obtained from a trusted source.
...
```
./aingle import --importfile=./OntBlocks.dat
```

## 7. Build Transaction

Build transaction command can build transaction raw data, such as transfer transaction, approve tansaction, and so on. Note that before send to AIngle, the transaction after built should be signed by private key.
...
```
./aingle buildtx transfer --from=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 --to=AaCe8nVkMRABnp5YgEjYZ9E5KYCxks2uce --amount=10
```
...
```
./aingle buildtx approve  --from=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 --to=AaCe8nVkMRABnp5YgEjYZ9E5KYCxks2uce --amount=10
```
Return example:
...
```
./aingle buildtx transferfrom --sender=AMFrW7hrSRw1Azz6hQohni8BdStZDvectW --from=Aaxjf7utmjSstmTD1LjtYfhZ3CoWaxC7Tt --to=AMFrW7hrSRw1Azz6hQohni8BdStZDvectW --amount=10
```
...
```
./aingle buildtx withdrawong ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48
```
...
The transaction build by buildtx command, should be signed before send to AIngle. Note that if transction fee payer is different with transfer from, both account should sign to the transaction.
...
--send
--send parameter specifies whether send transaction to AIngle after signed.

--prepare
prepare parameter specifies whether prepare execute transaction, without send to AIngle.

--rpcport
The rpcport parameter specifies the port number to which the RPC server is bound. The default is 20336.

```
./aingle sigtx --account=ARVVxBPGySL56CvSSWfjRVVyZYpNZ7zp48 
```
...
```
./aingle account list -v
```
The max number of public key support by AIngle is 16 at present.

-m
m parameter specifies the least number of signature. Default value is 1.

```
./aingle multisigaddr --pubkey=03c0c30f11c7fc1396e8595bf2e339d553d728ea6f21ae831e8ab704ca14fe8a56,02b2b9fb60a0add9ef6715ffbac8bc7e81cb47cd06c157c19e6a858859c0158231 -m=1
```
Return example:
...
The transaction after being signed can be sent to AIngle via sendtx command.
...
```
./aingle sendtx 
```
...
Tip:
  Using './aingle info status f8ea91da985af249e808913b6398150079cdfb02273146e4eb69c43947a42db2' to query transaction status.
```
...
```
./aingle showtx 
```
Return:

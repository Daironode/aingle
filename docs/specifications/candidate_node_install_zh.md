...
运行命令: 
```
curl  <https://dev.ont.io/aingle_install>  | sh`
```
   或者可以自行在这里下载：<https:// github.com/Daironode/aingle/releases> 

3、把生成的钱包文件 wallet.dat拷贝到aingle二进制目录下，执行命令 ./aingle account list -v  确认Public key和之前提交的保持一致。注意：不能用同一个钱包开启2条链。

4、后台运行 `./aingle --rest --enable-consensus`
...
```
#! /bin/bash
./aingle --rest --enable-consensus<<eof
your password
eof >log &
```
然后用以下命令启动

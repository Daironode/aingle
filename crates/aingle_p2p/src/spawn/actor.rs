...
#[derive(Clone)]
struct WrapEvtSender(futures::channel::mpsc::Sender<AIngleP2pEvent>);
...
impl WrapEvtSender {
    ...
    ) -> impl Future<Output = AIngleP2pResult<()>> + 'static + Send {
        timing_trace!(
            {
                self.0
                    .put_agent_info_signed(saf_hash, to_agent, agent_info_signed)
            },
    ...
    ) -> impl Future<Output = AIngleP2pResult<Option<AgentInfoSigned>>> + 'static + Send {
        timing_trace!(
            {
                self.0
                    .get_agent_info_signed(saf_hash, to_agent, kitsune_space, kitsune_agent)
            },
    ...
    fn query_agent_info_signed(
        ...
        kitsune_space: Arc<kitsune_p2p::KitsuneSpace>,
        kitsune_agent: Arc<kitsune_p2p::KitsuneAgent>,
    ) -> impl Future<Output = AIngleP2pResult<Vec<AgentInfoSigned>>> + 'static + Send {
        timing_trace!(
            {
                self.0
                    .query_agent_info_signed(saf_hash, to_agent, kitsune_space, kitsune_agent)
            },
    ...
    fn put_metric_datum(
        ...
        timing_trace!(
            {
                self.0
                    .put_metric_datum(saf_hash, to_agent, agent, kind, timestamp)
            },
    ...
    fn query_metrics(
        ...
        to_agent: AgentPubKey,
        query: MetricQuery,
    ) -> impl Future<Output = AIngleP2pResult<MetricQueryAnswer>> + 'static + Send {
        timing_trace!(
            { self.0.query_metrics(saf_hash, to_agent, query) },
    ...
    fn call_remote(
        ...
        payload: ExternIO,
    ) -> impl Future<Output = AIngleP2pResult<SerializedBytes>> + 'static + Send {
        timing_trace!(
            {
                self.0.call_remote(
                    saf_hash, to_agent, from_agent, zome_name, fn_name, cap, payload,
                )
            },
    ...
    fn publish(
        ...
        timing_trace!({
            self.0.publish(saf_hash, to_agent, from_agent, request_validation_receipt, sgd_hash, ops)
    ...
    ) -> impl Future<Output = AIngleP2pResult<ValidationPackageResponse>> + 'static + Send {
        timing_trace!(
            {
                self.0
                    .get_validation_package(saf_hash, to_agent, header_hash)
            },
    ...
    ) -> impl Future<Output = AIngleP2pResult<WireOps>> + 'static + Send {
        timing_trace!(
            { self.0.get(saf_hash, to_agent, sgd_hash, options) },
            "(hp2p:handle) get",
        )
    }
    ...
    ) -> impl Future<Output = AIngleP2pResult<MetadataSet>> + 'static + Send {
        timing_trace!(
            { self.0.get_meta(saf_hash, to_agent, sgd_hash, options) },
    ...
    ) -> impl Future<Output = AIngleP2pResult<WireLinkOps>> + 'static + Send {
        timing_trace!(
            { self.0.get_links(saf_hash, to_agent, link_key, options) },
    ...
    ) -> impl Future<Output = AIngleP2pResult<AgentActivityResponse<HeaderHash>>> + 'static + Send
    {
        timing_trace!(
            {
                self.0
                    .get_agent_activity(saf_hash, to_agent, agent, query, options)
            },
    ...
    fn validation_receipt_received(
        ...
        timing_trace!(
            {
                self.0
                    .validation_receipt_received(saf_hash, to_agent, receipt)
            },
    ...
    ) -> impl Future<Output = AIngleP2pResult<Vec<ai_hash::SgdOpHash>>> + 'static + Send {
        timing_trace!(
            {
                self.0
                    .fetch_op_hashes_for_constraints(saf_hash, to_agent, sgd_arc, since, until)
            },
    ...
    >
        ...
        timing_trace!(
            { self.0.fetch_op_hash_data(saf_hash, to_agent, op_hashes) },
            %op_count,
    ...
    fn sign_network_data(
        ...
        to_agent: AgentPubKey,
        data: Vec<u8>,
    ) -> impl Future<Output = AIngleP2pResult<Signature>> + 'static + Send {
        let byte_count = data.len();
        timing_trace!(
            { self.0.sign_network_data(saf_hash, to_agent, data) },
            %byte_count,
...
impl AIngleP2pActor {
    ...
    #[tracing::instrument(skip(self, saf_hash, to_agent, sgd_hash, options), level = "trace")]
    fn handle_incoming_get(
        ...
        let evt_sender = self.evt_sender.clone();
        Ok(async move {
            let res = evt_sender.get(saf_hash, to_agent, sgd_hash, options).await;
    ...
    fn handle_incoming_get_meta(
        ...
        Ok(async move {
            let res = evt_sender
                .get_meta(saf_hash, to_agent, sgd_hash, options)
                .await;
            res.and_then(|r| Ok(SerializedBytes::try_from(r)?))
                .map_err(kitsune_p2p::KitsuneP2pError::from)
                .map(|res| UnsafeBytes::from(res).into())
    ...
    fn handle_incoming_get_links(
        ...
        Ok(async move {
            let res = evt_sender
                .get_links(saf_hash, to_agent, link_key, options)
                .await;
            res.and_then(|r| Ok(SerializedBytes::try_from(r)?))
                .map_err(kitsune_p2p::KitsuneP2pError::from)
                .map(|res| UnsafeBytes::from(res).into())
    ...
    fn handle_incoming_validation_receipt(
        ...
        Ok(async move {
            evt_sender
                .validation_receipt_received(saf_hash, agent_pub_key, receipt)
                .await?;
    ...
    fn handle_incoming_get_validation_package(
        ...
        Ok(async move {
            let res = evt_sender
                .get_validation_package(saf_hash, agent_pub_key, header_hash)
                .await;
...
impl kitsune_p2p::event::KitsuneP2pEventHandler for AIngleP2pActor {
    ...
    fn handle_put_agent_info_signed(
        ...
        let space = SafHash::from_kitsune(&space);
    ...
    fn handle_get_agent_info_signed(
        ...
        let h_space = SafHash::from_kitsune(&space);
    ...
    fn handle_put_metric_datum(
        ...
        let saf_hash = SafHash::from_raw_32([0; 32].to_vec());
    ...
    fn handle_query_metrics(
        ...
        Ok(
            async move { Ok(evt_sender.query_metrics(saf_hash, to_agent, query).await?) }
                .boxed()
                .into(),
        )
    }
    ...
    fn handle_call(
        ...
        match request {
            ...
            crate::wire::WireMessage::Get { sgd_hash, options } => {
                self.handle_incoming_get(space, to_agent, sgd_hash, options)
            }
            crate::wire::WireMessage::GetMeta { sgd_hash, options } => {
                self.handle_incoming_get_meta(space, to_agent, sgd_hash, options)
            }
    ...
    fn handle_gossip(
        ...
        let op_hash = SgdOpHash::from_kitsune(&op_hash);
        let op_data =
            crate::wire::WireSgdOpData::decode(op_data).map_err(AIngleP2pError::from)?;
        self.handle_incoming_publish(
            space,
            to_agent,
            op_data.from_agent,
    ...
    ) -> kitsune_p2p::event::KitsuneP2pEventHandlerResult<Vec<Arc<kitsune_p2p::KitsuneOpHash>>>
    {
        ...
        Ok(async move {
            Ok(evt_sender
                .fetch_op_hashes_for_constraints(space, agent, sgd_arc, since, until)
                .await?
                .into_iter()
    ...
    > {
        ...
        let op_hashes = op_hashes
            .into_iter()
            .map(|h| SgdOpHash::from_kitsune(&h))
    ...
    fn handle_sign_network_data(
        ...
        let space = SafHash::from_kitsune(&input.space);
...
impl AIngleP2pHandler for AIngleP2pActor {
    ...
    fn handle_join(
        ...
        let space = saf_hash.into_kitsune();
    ...
    fn handle_publish(
        ...
        let space = saf_hash.into_kitsune();
        let from_agent = from_agent.into_kitsune();
        let basis = sgd_hash.to_kitsune();

        let payload = crate::wire::WireMessage::publish(request_validation_receipt, sgd_hash, ops)
            .encode()?;
    ...
    fn handle_get_validation_package(
        ...
        let space = input.saf_hash.into_kitsune();
    ...
    #[tracing::instrument(skip(self, saf_hash, from_agent, sgd_hash, options), level = "trace")]
    fn handle_get(
        &mut self,
        saf_hash: SafHash,
        from_agent: AgentPubKey,
        ...
        let payload = crate::wire::WireMessage::get(sgd_hash, r_options).encode()?;
    ...
    fn handle_get_meta(
        ...
        let payload = crate::wire::WireMessage::get_meta(sgd_hash, r_options).encode()?;
    ...
    fn handle_get_links(
        ...
        let space = saf_hash.into_kitsune();
        let from_agent = from_agent.into_kitsune();
        let basis = AnySgdHash::from(link_key.base.clone()).to_kitsune();
    ...
    fn handle_get_agent_activity(
        ...
        let space = saf_hash.into_kitsune();
        let from_agent = from_agent.into_kitsune();
        // Convert the agent key to an any sgd hash so it can be used
        // as the basis for sending this request
        let agent_hash: AnySgdHash = agent.clone().into();

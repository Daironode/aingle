...
use anyhow::bail;
use anyhow::ensure;
use aingle_conductor_api::AdminInterfaceConfig;
use aingle_conductor_api::AdminRequest;
use aingle_conductor_api::AdminResponse;
use aingle_conductor_api::InterfaceDriver;
use aingle_p2p::kitsune_p2p;
use aingle_p2p::kitsune_p2p::agent_store::AgentInfoSigned;
use aingle_types::prelude::AgentPubKey;
use aingle_types::prelude::CellId;
use aingle_types::prelude::DnaHash;
use aingle_types::prelude::InstallAppDnaPayload;
use aingle_types::prelude::InstallAppPayload;
use aingle_types::prelude::InstalledCell;
        ...
        AdminRequestCli::ListAgents(args) => {
            ...
            for info in agent_infos {
                ...
                let agents = cell_info
                    ...
                    .map(|a| (a.clone(), aingle_p2p::agent_aingle_to_kit(a)))
                    .collect::<Vec<_>>();

                let dnas = cell_info
                    .iter()
                    .map(|c| c.dna_hash().clone())
                    .map(|d| (d.clone(), aingle_p2p::space_aingle_to_kit(d)))
                    .collect::<Vec<_>>();

...
//! There's probably a few bugs. If you find one please open an [issue](https://github.com/Daironode/aingle/issues)
//! or make a PR.
//!
//! ## CLI
...
//! - [AIngle](https://github.com/Daironode/aingle) binary on the path
//! #### Building
//! From github:
//! ```shell
//! cargo install aingle_hc --git https://github.com/Daironode/aingle
//! ```
...
impl CmdRunner {
    ...
    pub async fn from_setup(setup_path: PathBuf) -> anyhow::Result<(Self, tokio::process::Child)> {
        Self::from_setup_with_bin_path(&Path::new(Self::AINGLE_PATH), setup_path).await
    }
    ...
    pub async fn from_setup_with_bin_path(
        ...
        let conductor = run::run_async(aingle_bin_path, setup_path, None).await?;

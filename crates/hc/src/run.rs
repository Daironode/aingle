...
pub async fn run(
    ...
    let (port, aingle) = run_async(aingle_path, setup_path.clone(), force_admin_port).await?;
    ...
    let e = format!("Failed to run aingle at {}", setup_path.display());

    aingle.await.expect(&e);
    Ok(())
}
...
async fn check_started(aingle: &mut Child) {
    let started = tokio::time::timeout(std::time::Duration::from_millis(20), aingle).await;
...
fn check_setup(line: &str, needs_setup: &mut bool) -> (bool, Option<u16>) {
    if let Some(line) = line.strip_prefix("###") {
        if let Some(line) = line.strip_suffix("###") {
            match line {
                "AINGLE_SETUP" => tracing::info!("Found config"),
                "AINGLE_SETUP_END" => *needs_setup = false,
                _ => {

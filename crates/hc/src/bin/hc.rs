...
/// AIngle CLI - Helper for generating, running, and interacting with AIngle Conductor "setups".
///
...
struct Ops {
    ...
    op: Op,
    /// Force the admin port that hc uses to talk to aingle to a specific value.
    ...
    #[structopt(short, long, env = "HC_AINGLE_PATH", default_value = "aingle")]
    aingle_path: PathBuf,
}
...
enum Op {
    /// Generate one or more new AIngle Conductor setup(s) for later use.
    ///
...
async fn main() -> anyhow::Result<()> {
    ...
    match ops.op {
        Op::Generate {
            ...
            let paths = generate(&ops.aingle_path, dnas, num_conductors, gen).await?;
            ...
            if let Some(ports) = run {
                let aingle_path = ops.aingle_path.clone();
        ...
        Op::Call(call) => hc::calls::call(&ops.aingle_path, call).await?,
...
async fn run_n(
    ...
    let run_aingle = |aingle_path: PathBuf, path: PathBuf, ports, force_admin_port| async move {
        hc::run::run(&aingle_path, path, ports, force_admin_port).await?;
...
async fn generate(
    ...
    let paths = hc::setups::default_n(aingle_path, num_conductors, create, dnas).await?;

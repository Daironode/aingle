...
use aingle_websocket::{websocket_connect, WebsocketConfig, WebsocketReceiver, WebsocketSender};
...
async fn run_aingle() {
    ...
    cmd.arg("run")
        ...
        .kill_on_drop(true);
    let _hc_admin = cmd.spawn().expect("Failed to spawn aingle");
...
async fn run_multiple_on_same_port() {
    ...
    cmd.arg(format!("-f={}", port))
        ...
        .arg("list-dnas")
        .kill_on_drop(true);
    let _hc_admin2 = cmd.spawn().expect("Failed to spawn aingle");
    tokio::time::delay_for(std::time::Duration::from_secs(4)).await;
}

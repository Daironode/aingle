...
pub struct P2pAgentsDump {
    ...
    /// The saf as a [`SafHash`] and [`kitsune_p2p::KitsuneSpace`].
    pub this_saf: Option<(SafHash, kitsune_p2p::KitsuneSpace)>,
...
impl std::fmt::Display for P2pAgentsDump {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(this_saf) = &self.this_saf {
            writeln!(f, "This SAF {:?} is {:?}", this_saf.0, this_saf.1)?;
        }

...
impl From<TestWasm> for SafWasm {
    fn from(test_wasm: TestWasm) -> SafWasm {
        SafWasm::from(match test_wasm {
            TestWasm::AgentInfo => {
            ...
            TestWasm::AdkExtern => {
                get_code("wasm32-unknown-unknown/release/test_wasm_adk_extern.wasm")
            }
            TestWasm::InitFail => {
...
fn get_code(path: &'static str) -> Vec<u8> {
    let path = match option_env!("AI_TEST_WASM_DIR") {
    ...
    let warning = format!(
        ...
        Run `cargo build --features 'build_wasms' --manifest-path=crates/aingle/Cargo.toml`
...
impl From<TestWasm> for ZomeDef {
    fn from(test_wasm: TestWasm) -> Self {
        tokio_helper::block_forever_on(async move {
            let saf_wasm: SafWasm = test_wasm.into();
            let (_, wasm_hash) = aingle_types::saf::wasm::SafWasmHashed::from_content(saf_wasm)
                .await
                .into_inner();

...
// TODO: run ai-sandbox then see if we can call the app websocket
// TODO: Put aingle on the path
// TODO:

use assert_cmd::prelude::*;
use aingle_conductor_api::AppRequest;
use aingle_conductor_api::AppResponse;
use aingle_websocket::{self as ws, WebsocketConfig, WebsocketReceiver, WebsocketSender};
...
/// Runs aingle and creates a temp directory
#[tokio::test(flavor = "multi_thread")]
#[ignore = "Figure out how to get aingle bin in CI"]
async fn run_aingle() {
    observability::test_run().ok();
    let port: u16 = pick_unused_port().expect("No ports free");
    let cmd = std::process::Command::cargo_bin("ai").unwrap();
    let mut cmd = Command::from(cmd);
    cmd.arg("run")
        .arg(format!("-p={}", port))
        .arg("../../../elemental-chat/elemental-chat.saf")
        .kill_on_drop(true);
    let _ai_admin = cmd.spawn().expect("Failed to spawn aingle");
...
async fn run_multiple_on_same_port() {
    ...
    cmd.arg(format!("-f={}", port))
        ...
        .arg("list-safs")
        .kill_on_drop(true);
    let _ai_admin2 = cmd.spawn().expect("Failed to spawn aingle");
    tokio::time::sleep(std::time::Duration::from_secs(4)).await;
}

...
//! For simple calls like [`AdminRequest::ListSafs`] this is probably easier
...
use aingle_conductor_api::InterfaceDriver;
use aingle_conductor_api::{AdminInterfaceConfig, InstalledAppInfo};
use aingle_p2p::kitsune_p2p::agent_store::AgentInfoSigned;
use aingle_types::prelude::SafHash;
use aingle_types::prelude::InstallAppSafPayload;
use aingle_types::prelude::InstallAppPayload;
use aingle_types::prelude::RegisterSafPayload;
use aingle_types::prelude::YamlProperties;
use aingle_types::prelude::{AgentPubKey, AppBundleSource};
use aingle_types::prelude::{CellId, InstallAppBundlePayload};
use aingle_types::prelude::{SafSource, Uid};
...
#[derive(Debug, StructOpt, Clone)]
/// Calls AdminRequest::RegisterSaf
/// and registers a Saf. You can only use a path or a hash not both.
pub struct RegisterSaf {
    #[structopt(short, long)]
    /// UID to override when installing this Saf
    pub uid: Option<String>,
    #[structopt(short, long)]
    /// Properties to override when installing this Saf
    pub properties: Option<PathBuf>,
    #[structopt(short, long, conflicts_with = "hash", required_unless = "hash")]
    /// Path to a SafBundle file.
    pub path: Option<PathBuf>,
    #[structopt(short, long, parse(try_from_str = parse_saf_hash), required_unless = "path")]
    /// Hash of an existing saf you want to register.
    pub hash: Option<SafHash>,
}
...
pub struct InstallApp {
    ...
    #[structopt(required = true, min_values = 1, parse(try_from_str = parse_saf_hash))]
    /// The saf hashes to use in this app.
    pub safs: Vec<SafHash>,
}
...
pub struct InstallAppBundle {
    ...
    /// Optional UID override for every SAF in this app
    pub uid: Option<Uid>,
}
...
pub struct DumpState {
    #[structopt(parse(try_from_str = parse_saf_hash))]
    /// The saf hash half of the cell id to dump.
    pub saf: SafHash,
...
pub struct ListAgents {
    #[structopt(short, long, parse(try_from_str = parse_agent_key), requires = "saf")]
    /// Optionally request agent info for a particular cell id.
    pub agent_key: Option<AgentPubKey>,
    #[structopt(short, long, parse(try_from_str = parse_saf_hash), requires = "agent_key")]
    /// Optionally request agent info for a particular cell id.
    pub saf: Option<SafHash>,
}
...
#[doc(hidden)]
pub async fn call(aingle_path: &Path, req: Call) -> anyhow::Result<()> {
    let Call {
        existing,
        running,
        call,
    ...
    let cmds = if running.is_empty() {
        ...
        for (port, path) in ports.into_iter().zip(paths.into_iter()) {
            match CmdRunner::try_new(port).await {
                ...
                Err(e) => {
                    if let aingle_websocket::WebsocketError::Io(e) = &e {
                        if let std::io::ErrorKind::ConnectionRefused = e.kind() {
                            let (port, aingle) = run_async(aingle_path, path, None).await?;
                            cmds.push((CmdRunner::new(port).await, Some(aingle)));
                            continue;
                        }
                    }
                    bail!(
...
async fn call_inner(cmd: &mut CmdRunner, call: AdminRequestCli) -> anyhow::Result<()> {
    match call {
        ...
        AdminRequestCli::RegisterSaf(args) => {
            let safs = register_saf(cmd, args).await?;
            msg!("Registered Saf: {:?}", safs);
        }
        ...
        AdminRequestCli::ListSafs => {
            let safs = list_safs(cmd).await?;
            msg!("Safs: {:?}", safs);
        }
        ...
        AdminRequestCli::ListAgents(args) => {
            ...
            for info in agent_infos {
                ...
                let agents = cell_info
                    ...
                    .map(|a| (a.clone(), aingle_p2p::agent_ai_to_kit(a)))
                    .collect::<Vec<_>>();

                let safs = cell_info
                    .iter()
                    .map(|c| c.saf_hash().clone())
                    .map(|d| (d.clone(), aingle_p2p::space_ai_to_kit(d)))
                    .collect::<Vec<_>>();

                let this_agent = agents.iter().find(|a| *info.agent == a.1);
                let this_saf = safs.iter().find(|d| *info.space == d.1).unwrap();
                if let Some(this_agent) = this_agent {
                    writeln!(out, "This Agent {:?} is {:?}", this_agent.0, this_agent.1)?;
                }
                writeln!(out, "This SAF {:?} is {:?}", this_saf.0, this_saf.1)?;
...
/// Calls [`AdminRequest::RegisterSaf`] and registers saf.
pub async fn register_saf(cmd: &mut CmdRunner, args: RegisterSaf) -> anyhow::Result<SafHash> {
    let RegisterSaf {
        uid,
        properties,
        path,
    ...
    let source = match (path, hash) {
        (None, Some(hash)) => SafSource::Hash(hash),
        (Some(path), None) => SafSource::Path(path),
        _ => unreachable!("Can't have hash and path for saf source"),
    };
    let saf = RegisterSafPayload {
        uid,
        properties,
        source,
    };

    let r = AdminRequest::RegisterSaf(Box::new(saf));
    let registered_saf = cmd.command(r).await?;
    let hash =
        expect_match!(registered_saf => AdminResponse::SafRegistered, "Failed to register saf");
    Ok(hash)
}

/// Calls [`AdminRequest::InstallApp`] and installs a new app.
/// Creates an app per saf with the app id of `{app-id}-{saf-index}`
/// e.g. `my-cool-app-3`.
pub async fn install_app(
    ...
    let safs = safs
        .into_iter()
        .enumerate()
        .map(|(i, hash)| InstallAppSafPayload::hash_only(hash, format!("{}-{}", app_id, i)))
        .collect();
...
/// Calls [`AdminRequest::ListCellIds`].
pub async fn list_safs(cmd: &mut CmdRunner) -> anyhow::Result<Vec<SafHash>> {
    let resp = cmd.command(AdminRequest::ListSafs).await?;
    Ok(expect_match!(resp => AdminResponse::SafsListed, "Failed to list safs"))
}
...
fn parse_saf_hash(arg: &str) -> anyhow::Result<SafHash> {
    SafHash::try_from(arg).map_err(|e| anyhow::anyhow!("{:?}", e))
}
...
impl From<CellId> for DumpState {
    fn from(cell_id: CellId) -> Self {
        let (saf, agent_key) = cell_id.into_saf_and_agent();
        Self { saf, agent_key }
    }
}
...
impl From<DumpState> for CellId {
    fn from(ds: DumpState) -> Self {
        CellId::new(ds.saf, ds.agent_key)
    }
}

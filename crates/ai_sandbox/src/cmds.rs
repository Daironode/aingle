...
use aingle_p2p::kitsune_p2p::TransportConfig;
use structopt::StructOpt;
use url2::Url2;

#[derive(Debug, StructOpt, Clone)]
// This creates a new aingle sandbox
// which is a
// - conductor config
// - databases
// - keystore
pub struct Create {
    ...
    ///
    /// For example `ai gen -r path/to/my/chains -n 3 -d=first,second,third`
...
pub struct Existing {
    ...
    /// For example `ai run -e=/tmp/kAOXQlilEtJKlTM_W403b,/tmp/kddsajkaasiIII_sJ`.
    ...
    /// Existing sandboxes are visible via `ai list`.
    /// Use the index to choose which sandboxes to use.
    /// For example `ai run 1 3 5` or `ai run 1`
...
impl Existing {
    pub fn load(mut self) -> anyhow::Result<Vec<PathBuf>> {
        ...
        if self.all {
            ...
            msg!(
                "
There are multiple sandboxes and ai doesn't know which to run.
You can run:
...
Run `ai list` to see the sandboxes or `ai r --help` for more information."
            );
...
You can use `ai generate` or `ai g` to do this.
Run `ai g --help` for more options."
            );
        }
...
impl From<Network> for KitsuneP2pConfig {
    fn from(n: Network) -> Self {
        ...
        match transport {
            NetworkType::Mem => (),
            NetworkType::Mdns => {
                kit.network_type = aingle_p2p::kitsune_p2p::NetworkType::QuicMdns;
            ...
            NetworkType::Quic(Quic {
                ...
                kit.transport_pool = vec![TransportConfig::Proxy {
                    ...
                    proxy_config: aingle_p2p::kitsune_p2p::ProxyConfig::RemoteProxyClient {
                        proxy_url,
                    },
                }]
            }

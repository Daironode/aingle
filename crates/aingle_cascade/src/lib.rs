...
use aingle_p2p::actor::GetOptions as NetworkGetOptions;
...
use aingle_state::query::element_details::GetElementDetailsQuery;
use aingle_state::query::entry_details::GetEntryDetailsQuery;
use aingle_state::query::link::GetLinksQuery;
use aingle_state::query::link_details::GetLinkDetailsQuery;
use aingle_state::query::live_element::GetLiveElementQuery;
use aingle_state::query::live_entry::GetLiveEntryQuery;
use aingle_state::query::DbScratch;
...
#[derive(Clone)]
pub struct Cascade<Network = AIngleP2pCell> {
...
where
    Network: AIngleP2pCellT + Clone + 'static + Send,
{
    ...
    pub fn with_network<N: AIngleP2pCellT + Clone>(
        self,
        network: N,
        cache_env: EnvWrite,
    ) -> Cascade<N> {
...
impl Cascade<AIngleP2pCell> {
    ...
    pub fn from_workspace_network<N: AIngleP2pCellT + Clone>(
    ...
    Network: AIngleP2pCellT + Clone + 'static + Send,
{
    ...
    pub async fn retrieve_entry(
        ...
        options.request_type = aingle_p2p::event::GetRequest::Pending;
    ...
    pub async fn get_concurrent<I: IntoIterator<Item = AnySgdHash>>(
        &mut self,
        hashes: I,
        options: GetOptions,
        ...
        let iter = hashes.into_iter().map({
            |hash| {
                ...
                async move { cascade.sgd_get(hash, options).await }
            }
        });
    ...
    /// This gives you the latest possible picture of the current sgd state.
    /// Data from your zome call is also added to the cache.
    pub async fn sgd_get(
        &mut self,
        hash: AnySgdHash,
        options: GetOptions,
    ) -> CascadeResult<Option<Element>> {
        match *hash.hash_type() {
            AnySgd::Entry => self.sgd_get_entry(hash.into(), options).await,
            AnySgd::Header => self.sgd_get_header(hash.into(), options).await,
        }
    }
    ...
    pub async fn get_details(
        ...
        match *hash.hash_type() {
            AnySgd::Entry => Ok(self
                .get_entry_details(hash.into(), options)
                .await?
                .map(Details::Entry)),
            AnySgd::Header => Ok(self
    ...
    fn am_i_authoring(&mut self, hash: &AnySgdHash) -> CascadeResult<bool> {
        let scratch = ok_or_return!(self.scratch.as_ref(), false);
        Ok(scratch.apply_and_then(|scratch| scratch.contains_hash(hash))?)
    }

    async fn am_i_an_authority(&mut self, hash: AnySgdHash) -> CascadeResult<bool> {

...
pub fn test_dbs_and_mocks(
    ...
    ElementBuf,
    aingle_state::metadata::MockMetadataBuf,
    ElementBuf,
    aingle_state::metadata::MockMetadataBuf,
) {
    let cas = ElementBuf::vault(env.clone().into(), true).unwrap();
    let element_cache = ElementBuf::cache(env.clone().into()).unwrap();
    let metadata = aingle_state::metadata::MockMetadataBuf::new();
    let metadata_cache = aingle_state::metadata::MockMetadataBuf::new();

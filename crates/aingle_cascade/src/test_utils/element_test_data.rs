...
impl ElementTestData {
    pub fn create() -> Self {
        ...
        let signature = fixt!(Signature);
        let deleted_by_op =
            SgdOpHashed::from_content_sync(SgdOp::RegisterDeletedBy(signature.clone(), delete));

        let wire_delete = Judged::valid(SignedHeader(delete_header, signature).try_into().unwrap());

        let signature = fixt!(Signature);
        let update_element_op = SgdOpHashed::from_content_sync(SgdOp::RegisterUpdatedElement(
            signature.clone(),
            update,
        ...
        let signature = fixt!(Signature);
        let any_store_element_op = SgdOpHashed::from_content_sync(SgdOp::StoreElement(
            signature.clone(),
            any_header.clone(),
            any_entry.clone(),
        ));

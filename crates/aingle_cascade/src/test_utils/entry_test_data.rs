...
use aingle_types::header::WireNewEntryHeader;
...
pub struct EntryTestData {
    ...
    pub update_content_op: SgdOpHashed,
...
impl EntryTestData {
    pub fn create() -> Self {
        ...
        let delete_entry_header_op = SgdOpHashed::from_content_sync(
            SgdOp::RegisterDeletedEntryHeader(signature.clone(), delete),
        );

        let wire_delete = Judged::valid(SignedHeader(delete_header, signature).try_into().unwrap());

        let signature = fixt!(Signature);
        let update_content_op = SgdOpHashed::from_content_sync(SgdOp::RegisterUpdatedContent(
            signature.clone(),
            update,
        ...
        let signature = fixt!(Signature);
        let create_link_op = SgdOpHashed::from_content_sync(SgdOp::RegisterAddLink(
            signature.clone(),
            create_link.clone(),
        ));
        ...
        let signature = fixt!(Signature);
        let delete_link_op = SgdOpHashed::from_content_sync(SgdOp::RegisterRemoveLink(
            signature.clone(),
            delete_link.clone(),
        ));

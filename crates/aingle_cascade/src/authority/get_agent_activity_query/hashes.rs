...
use aingle_state::{prelude::*, query::QueryData};
...
impl Query for GetAgentActivityQuery {
    ...
    fn query(&self) -> String {
        "
            SELECT Header.hash, SgdOp.validation_status, Header.blob AS header_blob,
            SgdOp.when_integrated
            FROM Header
            JOIN SgdOp ON SgdOp.header_hash = Header.hash
            WHERE Header.author = :author
            AND SgdOp.type = :op_type
            ORDER BY Header.seq ASC
        "
        .to_string()
    }

    fn params(&self) -> Vec<aingle_state::query::Params> {
        (named_params! {
            ":author": self.agent,
            ":op_type": SgdOpType::RegisterAgentActivity,
        })
        .to_vec()
    }

...
use aingle_state::mutations::insert_op_scratch;
use aingle_state::prelude::test_cell_env;
use aingle_state::scratch::Scratch;
use aingle_zome_types::Details;
use aingle_zome_types::ElementDetails;
use aingle_zome_types::EntryDetails;
use aingle_zome_types::EntrySgdStatus;
use aingle_zome_types::GetOptions;
use aingle_zome_types::ValidationStatus;

async fn assert_can_get<N: AIngleP2pCellT + Clone + Send + 'static>(
    ...
    // - Get via entry hash
    let r = cascade
        .sgd_get(td_entry.hash.clone().into(), options.clone())
        .await
        .unwrap()
    ...
    // - Get via header hash
    let r = cascade
        .sgd_get(td_element.any_header_hash.clone().into(), options.clone())
        .await
        .unwrap()
    ...
    let expected = Details::Entry(EntryDetails {
        ...
        deletes: vec![],
        updates: vec![],
        entry_sgd_status: EntrySgdStatus::Live,
    });
...
async fn assert_is_none<N: AIngleP2pCellT + Clone + Send + 'static>(
    td_entry: &EntryTestData,
    td_element: &ElementTestData,
    cascade: &mut Cascade<N>,
    options: GetOptions,
...
async fn assert_rejected<N: AIngleP2pCellT + Clone + Send + 'static>(
    td_entry: &EntryTestData,
    td_element: &ElementTestData,
    cascade: &mut Cascade<N>,
    options: GetOptions,
    ...
    let expected = Details::Entry(EntryDetails {
        ...
        deletes: vec![],
        updates: vec![],
        entry_sgd_status: EntrySgdStatus::Dead,
    });
...
async fn assert_can_retrieve<N: AIngleP2pCellT + Clone + Send + 'static>(
    td_entry: &EntryTestData,
    cascade: &mut Cascade<N>,
    options: GetOptions,
) {
...
async fn entry_authoring() {
    ...
    let mut mock = MockAIngleP2pCellT::new();

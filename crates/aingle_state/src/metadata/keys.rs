...
use aingle_zome_types::{timestamp::Timestamp, validate::ValidationStatus};
pub(super) use misc::*;
...
impl LinkMetaVal {
    ...
    pub fn into_link(self) -> aingle_zome_types::link::Link {
        aingle_zome_types::link::Link {
            target: self.target,
...
impl From<BytesKey> for ChainItemKey {
    fn from(b: BytesKey) -> Self {
        ...
        debug_assert_eq!(bytes.len(), AINGLE_HASH_FULL_LEN * 2 + SEQ_SIZE + STATUS_SIZE);
        let mut start = 0;
        let mut end = AINGLE_HASH_FULL_LEN;

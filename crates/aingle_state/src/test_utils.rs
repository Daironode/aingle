...
use aingle_zome_types::test_utils::fake_cell_id;
...
pub fn test_cache_env() -> TestEnv {
    let saf = fake_cell_id(1).saf_hash().clone();
    test_env(DbKind::Cache(saf))
}
...
pub fn test_envs_with_keystore(keystore: KeystoreSender) -> TestEnvs {
    let tempdir = TempDir::new("aingle-test-environments").unwrap();
...
//     env.conn()
//         .unwrap()
//         .transaction_with_behavior(aingle_sqlite::rusqlite::TransactionBehavior::Exclusive)
//         .unwrap()
...
pub fn dump_db(txn: &Transaction) {
    ...
    tracing::debug!("SgdOps:");
    let stmt = txn.prepare("SELECT * FROM SgdOp").unwrap();
    dump(stmt);
}

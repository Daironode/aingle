...
impl TestData {
    ...
    fn with_same_keys(mut td: Self) -> Self {
        td.link_add.timestamp = aingle_types::timestamp::now().into();
        let link_add_hash =
    ...
    fn add_link(&self) {
        let op = SgdOpHashed::from_content_sync(SgdOp::RegisterAddLink(
            fixt!(Signature),
            self.link_add.clone(),
        ));
        self.env
    ...
    fn delete_link(&self) {
        let op = SgdOpHashed::from_content_sync(SgdOp::RegisterRemoveLink(
            fixt!(Signature),

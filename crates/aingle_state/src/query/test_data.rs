...
use aingle_types::{sgd_op::SgdOp, header::NewEntryHeader};
...
pub struct LinkTestData {
    pub create_link_op: SgdOpHashed,
    pub later_create_link_op: SgdOpHashed,
    pub delete_link_op: SgdOpHashed,
    pub link: Link,
    pub later_link: Link,
    pub base_op: SgdOpHashed,
    pub target_op: SgdOpHashed,
...
pub struct EntryTestData {
    ...
    pub update_store_entry_op: SgdOpHashed,
    pub delete_entry_header_op: SgdOpHashed,
    pub entry: Entry,
    pub hash: EntryHash,
...
pub struct ElementTestData {
    ...
    pub update_store_element_op: SgdOpHashed,
    pub delete_by_op: SgdOpHashed,
    pub entry: Entry,
    pub header: SignedHeaderHashed,
    pub update_header: SignedHeaderHashed,
...
impl LinkTestData {
    pub fn new() -> Self {
        ...
        let now = aingle_types::timestamp::now();
        ...
        let create_link_op = SgdOp::RegisterAddLink(create_link_sig.clone(), create_link.clone());
        ...
        let later_create_link_op =
            SgdOp::RegisterAddLink(later_create_link_sig.clone(), later_create_link.clone());
        ...
        let delete_link_op = SgdOp::RegisterRemoveLink(fixt!(Signature), delete_link.clone());

        let base_op = SgdOp::StoreEntry(
            fixt!(Signature),
            NewEntryHeader::Create(create_base.clone()),
            Box::new(base.clone()),
        );

        let target_op = SgdOp::StoreEntry(
            fixt!(Signature),
        ...
        Self {
            create_link_op: SgdOpHashed::from_content_sync(create_link_op),
            later_create_link_op: SgdOpHashed::from_content_sync(later_create_link_op),
            delete_link_op: SgdOpHashed::from_content_sync(delete_link_op),
            link,
            base_op: SgdOpHashed::from_content_sync(base_op),
            target_op: SgdOpHashed::from_content_sync(target_op),
            base_query,
            tag_query,
            later_link,
            details_tag_query,
...
impl EntryTestData {
    pub fn new() -> Self {
        ...
        let store_entry_op = SgdOpHashed::from_content_sync(SgdOp::StoreEntry(
            signature.clone(),
        ...
        let signature = fixt!(Signature);
        let delete_entry_header_op = SgdOpHashed::from_content_sync(
            SgdOp::RegisterDeletedEntryHeader(signature.clone(), delete.clone()),
        );

        let signature = fixt!(Signature);
        let update_store_entry_op = SgdOpHashed::from_content_sync(SgdOp::StoreEntry(
            signature.clone(),
...
impl ElementTestData {
    pub fn new() -> Self {
        ...
        let signature = fixt!(Signature);
        let store_element_op = SgdOpHashed::from_content_sync(SgdOp::StoreElement(
            signature.clone(),
        ...
        let signature = fixt!(Signature);
        let delete_by_op = SgdOpHashed::from_content_sync(SgdOp::RegisterDeletedBy(
            signature.clone(),
            delete.clone(),
        ));

        let signature = fixt!(Signature);
        let update_store_element_op = SgdOpHashed::from_content_sync(SgdOp::StoreElement(
            signature.clone(),

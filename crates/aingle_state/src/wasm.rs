...
use aingle_lmdb::buffer::CasBufFreshAsync;
...
mod tests {
    ...
    use aingle_types::dna::wasm::DnaWasm;
    ...
    async fn wasm_store_round_trip() -> DatabaseResult<()> {
        ...
        let env = aingle_lmdb::test_utils::test_wasm_env();
        let mut wasm_buf = WasmBuf::new(
            env.env().into(),
            env.get_db(&*aingle_lmdb::db::WASM).unwrap(),
        )
        .unwrap();

        // a wasm
        let wasm =
            DnaWasmHashed::from_content(DnaWasm::from(aingle_wasm_test_utils::TestWasm::Foo))
                .await;

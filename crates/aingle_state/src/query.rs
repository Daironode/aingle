...
use aingle_sqlite::rusqlite::Statement;
...
pub mod prelude {
    ...
    pub use super::Txn;
    pub use super::Txns;
    pub use aingle_sqlite::rusqlite::named_params;
    pub use aingle_sqlite::rusqlite::Row;
    pub use std::sync::Arc;
}

/// Alias for the params required by rusqlite query execution
pub type Params<'a> = (&'a str, &'a dyn aingle_sqlite::rusqlite::ToSql);
...
pub trait Store {
    ...
    fn get_element(&self, hash: &AnySgdHash) -> StateQueryResult<Option<Element>>;

    /// Check if a hash is contained in the store
    fn contains_hash(&self, hash: &AnySgdHash) -> StateQueryResult<bool> {
        match *hash.hash_type() {
            AnySgd::Entry => self.contains_entry(&hash.clone().into()),
            AnySgd::Header => self.contains_header(&hash.clone().into()),
        }
    }
...
impl<'stmt> Store for Txn<'stmt, '_> {
    ...
    fn contains_entry(&self, hash: &EntryHash) -> StateQueryResult<bool> {
        ...
        if let Err(aingle_sqlite::rusqlite::Error::QueryReturnedNoRows) = &exists {
            Ok(false)
        } else {
            Ok(exists?)
        }
    ...
    fn get_header(&self, hash: &HeaderHash) -> StateQueryResult<Option<SignedHeaderHashed>> {
        ...
        if let Err(aingle_sqlite::rusqlite::Error::QueryReturnedNoRows) = &shh {
            Ok(None)
        } else {
            Ok(Some(shh??))
        }
    ...
    fn get_element(&self, hash: &AnySgdHash) -> StateQueryResult<Option<Element>> {
        match *hash.hash_type() {
            AnySgd::Entry => self.get_any_element(&hash.clone().into()),
            AnySgd::Header => self.get_exact_element(&hash.clone().into()),
        }
    }
}
...
impl<'stmt> Txn<'stmt, '_> {
    fn get_exact_element(&self, hash: &HeaderHash) -> StateQueryResult<Option<Element>> {
        ...
        if let Err(aingle_sqlite::rusqlite::Error::QueryReturnedNoRows) = &element {
            Ok(None)
        } else {
            Ok(Some(element??))
        }
...
pub fn to_blob<T: Serialize + std::fmt::Debug>(t: T) -> StateQueryResult<Vec<u8>> {
    Ok(aingle_middleware_bytes::encode(&t)?)
}
...
pub fn from_blob<T: DeserializeOwned + std::fmt::Debug>(blob: Vec<u8>) -> StateQueryResult<T> {
    Ok(aingle_middleware_bytes::decode(&blob)?)
}

    ...
    pub fn valid_cap_grant(
        ...
        let committed_valid_grant = fresh_reader!(self.env(), |r| {
            let (references, headers): (
                HashSet<HeaderHash>,
                Vec<AIngleHashed<aingle_zome_types::element::SignedHeader>>,
            ) = self
                .0
                .headers()
                .iter_fail(&r)?
...
pub mod tests {
    ...
    use aingle_types::test_utils::fake_dna_hash;
    use aingle_zome_types::capability::CapAccess;
    use aingle_zome_types::capability::ZomeCallCapGrant;

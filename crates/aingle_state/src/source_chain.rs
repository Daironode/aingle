...
use aingle_types::sgd_op::produce_op_lights_from_elements;
use aingle_types::sgd_op::produce_op_lights_from_iter;
use aingle_types::sgd_op::SgdOpLight;
use aingle_types::sgd_op::SgdOpType;
use aingle_types::sgd_op::OpOrder;
use aingle_types::sgd_op::UniqueForm;
use aingle_types::element::SignedHeaderHashedExt;
...
use aingle_zome_types::CapAccess;
use aingle_zome_types::CapGrant;
use aingle_zome_types::CapSecret;
use aingle_zome_types::Element;
use aingle_zome_types::Entry;
use aingle_zome_types::EntryVisibility;
use aingle_zome_types::GrantedFunction;
use aingle_zome_types::Header;
use aingle_zome_types::HeaderBuilder;
use aingle_zome_types::HeaderBuilderCommon;
use aingle_zome_types::HeaderHashed;
use aingle_zome_types::HeaderInner;
...
impl SourceChain {
    ...
    pub fn valid_cap_grant(
        ...
        let valid_cap_grant = self.vault.conn()?.with_reader(|txn| {
            ...
            JOIN SgdOp AS D_REF ON D_REF.header_hash = H_REF.hash
            WHERE
            D_REF.is_authored = 1
            AND
            (
            ...
            let sql = format!(
                ...
                JOIN SgdOp ON Header.hash = SgdOp.header_hash
                WHERE
                SgdOp.is_authored = 1
                AND
        ...
        let (headers, ops, entries) = self.scratch.apply_and_then(|scratch| {
            ...
            for shh in signed_headers {
                ...
                for op in ops_inner {
                    ...
                    // Header is required by value to produce the SgdOpHash.
                    let (header, op_hash) =
                    ...
                    h = Some(header);
                    // Collect the SgdOpLight, SgdOpHash and OpOrder.
        ...
        self.vault
            .async_commit(move |txn| {
                ...
                for (op, op_hash, op_order, timestamp, visibility) in ops {
                    ...
                    if !(op_type == SgdOpType::StoreEntry
...
pub async fn genesis(
    ...
    let saf_header = Header::Saf(header::Saf {
        author: agent_pubkey.clone(),
        timestamp: timestamp::now(),
        hash: saf_hash,
    });
    let saf_header = HeaderHashed::from_content_sync(saf_header);
    let saf_header = SignedHeaderHashed::new(&keystore, saf_header).await?;
    let saf_header_address = saf_header.as_hash().clone();
    let element = Element::new(saf_header, None);
    let saf_ops = produce_op_lights_from_elements(vec![&element])?;
    let (saf_header, _) = element.into_inner();
    ...
    let agent_validation_header = Header::AgentValidationPkg(header::AgentValidationPkg {
        ...
        timestamp: timestamp::now(),
        header_seq: 1,
        prev_header: saf_header_address,
        membrane_proof,
    });
    ...
    vault
        .async_commit(move |txn| {
            source_chain::put_raw(txn, saf_header, saf_ops, None)?;
...
async fn _put_db<H: HeaderInner, B: HeaderBuilder<H>>(
    vault: aingle_types::env::EnvWrite,
    author: Arc<AgentPubKey>,
    header_builder: B,
    maybe_entry: Option<Entry>,
) -> SourceChainResult<HeaderHash> {
    ...
    Ok(vault
        .async_reader(move |txn| {
            ...
            let published_ops_count = txn.query_row(
                "
                SELECT COUNT(SgdOp.hash) FROM SgdOp
...
pub mod tests {
    ...
    async fn test_get_cap_grant() -> SourceChainResult<()> {
        ...
        source_chain::genesis(env.clone(), fake_saf_hash(1), alice.clone(), None)
            .await
            .unwrap();
    ...
    //         store
    //             .genesis(fake_saf_hash(1), fake_agent_pubkey_1(), None)
    //             .await?;
    ...
    async fn source_chain_buffer_dump_entries_json() -> SourceChainResult<()> {
        ...
        let vault = test_env.env();
        let author = test_env.cell_id().unwrap().agent_pubkey().clone();
        genesis(vault.clone().into(), fixt!(SafHash), author.clone(), None)
            .await
            .unwrap();
        ...
        assert_eq!(parsed["elements"][0]["header"]["type"], "Saf");

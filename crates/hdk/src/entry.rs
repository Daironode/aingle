...
///
<<<<<<< HEAD
/// Usually you want to hash a value that you want to reference on the DGD with [ `get` ] etc. because
...
/// also be used with [ `get` ] etc. to retreive a _specific_ element from the DGD rather than the
=======
...
///
<<<<<<< HEAD
/// [ `hash_entry` ] is "dumb" in that it doesn't check that the entry is defined, committed, on the DGD or
/// any other validation, it simply generates the hash for the serialized representation of
/// something in the same way that the DGD would.
=======
...
<<<<<<< HEAD
    AnyDgdHash: From<H>,
{
    host_call::<GetInput, Option<Element>>(__get, GetInput::new(AnyDgdHash::from(hash), options))
=======
    AnyDhtHash: From<H>,
{
...
<<<<<<< HEAD
pub fn get_details<H: Into<AnyDgdHash>>(
=======
...
<<<<<<< HEAD
/// The [ `EntryDefIndex` ] is used to reference the entry type in headers on the DGD and as the index of the type exported to tooling.
=======

...
//!
//! AiHashes are serialized as a plain 39-byte sequence.
...
//! L: 4 byte hash of the core hash, for SGD location
//! ```
//!
...
use crate::encode;
use crate::error::AiHashResult;
use crate::has_hash::HasHash;
use crate::HashType;
use crate::PrimitiveHashType;

/// Length of the prefix bytes (3)
pub const AI_HASH_PREFIX_LEN: usize = 3;

/// Length of the core bytes (32)
pub const AI_HASH_CORE_LEN: usize = 32;

/// Length of the location bytes (4)
pub const AI_HASH_LOC_LEN: usize = 4;

/// Length of the core bytes + the loc bytes (36 = 32 + 4),
/// i.e. everything except the type prefix
pub const AI_HASH_UNTYPED_LEN: usize = AI_HASH_CORE_LEN + AI_HASH_LOC_LEN; // 36

/// Length of the full AiHash bytes (39 = 3 + 32 + 4)
pub const AI_HASH_FULL_LEN: usize = AI_HASH_PREFIX_LEN + AI_HASH_CORE_LEN + AI_HASH_LOC_LEN;
...
macro_rules! assert_length {
    ($len:expr, $hash:expr) => {
        debug_assert_eq!(
            $hash.len(),
            $len,
            "invalid byte count for AiHash {:?}",
            $hash
        );
    };
}

/// A AiHash contains a vector of 36 bytes representing a 32-byte blake2b hash
/// plus 4 bytes representing a SGD location. It also contains a zero-sized
/// type which specifies what it is a hash of.
///
/// There is custom de/serialization implemented in [ser.rs]
#[derive(Clone, Hash, PartialEq, Eq, PartialOrd, Ord)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
pub struct AiHash<T: HashType> {
    hash: Vec<u8>,
    hash_type: T,
}

impl<T: HashType> AiHash<T> {
    /// Raw constructor: Create a AiHash from 39 bytes, using the prefix
    /// bytes to determine the hash_type
    pub fn from_raw_39(hash: Vec<u8>) -> AiHashResult<Self> {
        assert_length!(AI_HASH_FULL_LEN, &hash);
    ...
    pub fn from_raw_36_and_type(mut bytes: Vec<u8>, hash_type: T) -> Self {
        assert_length!(AI_HASH_UNTYPED_LEN, &bytes);
    ...
    /// Change the type of this AiHash, keeping the same bytes
    pub fn retype<TT: HashType>(mut self, hash_type: TT) -> AiHash<TT> {
        let prefix = hash_type.get_prefix();
        self.hash[0..AI_HASH_PREFIX_LEN].copy_from_slice(&prefix[0..AI_HASH_PREFIX_LEN]);
        AiHash {
            hash: self.hash,
            hash_type,
        }
    ...
    pub fn get_raw_36(&self) -> &[u8] {
        let bytes = &self.hash[AI_HASH_PREFIX_LEN..];
        assert_length!(AI_HASH_UNTYPED_LEN, bytes);
        bytes
    }
    ...
    pub fn get_raw_32(&self) -> &[u8] {
        let bytes = &self.hash[AI_HASH_PREFIX_LEN..AI_HASH_PREFIX_LEN + AI_HASH_CORE_LEN];
        assert_length!(AI_HASH_CORE_LEN, bytes);
        bytes
    }

    /// Fetch the ai sgd location for this hash
    pub fn get_loc(&self) -> u32 {
        bytes_to_loc(&self.hash[AI_HASH_FULL_LEN - AI_HASH_LOC_LEN..])
    }

    /// consume into the inner byte vector
    pub fn into_inner(self) -> Vec<u8> {
        assert_length!(AI_HASH_FULL_LEN, &self.hash);
        self.hash
    }
}

impl<T: HashType> AiHash<T> {
    /// Construct a AiHash from a 32-byte hash.
    ...
    pub fn from_raw_32_and_type(mut hash: Vec<u8>, hash_type: T) -> Self {
        if hash.len() == AI_HASH_CORE_LEN {
            hash.append(&mut encode::ai_sgd_location_bytes(&hash));
        }

        assert_length!(AI_HASH_UNTYPED_LEN, &hash);

        AiHash::from_raw_36_and_type(hash, hash_type)
    }
}

impl<P: PrimitiveHashType> AiHash<P> {
    ...
    /// Construct a AiHash from a prehashed raw 32-byte slice.
...
impl<T: HashType> AsRef<[u8]> for AiHash<T> {
    fn as_ref(&self) -> &[u8] {
        assert_length!(AI_HASH_FULL_LEN, &self.hash);
        &self.hash
    }
...
impl<T: HashType> rusqlite::ToSql for AiHash<T> {
...
#[cfg(feature = "rusqlite")]
impl<T: HashType> rusqlite::types::FromSql for AiHash<T> {
...
impl<T: HashType> IntoIterator for AiHash<T> {
    type Item = u8;
...
impl<T: HashType> HasHash<T> for AiHash<T> {
...
impl<T: HashType> std::fmt::Debug for AiHash<T> {
...
mod tests {
    ...
    fn assert_type<T: HashType>(t: &str, h: AiHash<T>) {
    ...
    fn test_enum_types() {
        assert_type(
            "SafHash",
            SafHash::from_raw_36(vec![0xdb; AI_HASH_UNTYPED_LEN]),
        );
        assert_type(
            "NetIdHash",
            NetIdHash::from_raw_36(vec![0xdb; AI_HASH_UNTYPED_LEN]),
        );
        assert_type(
            "AgentPubKey",
            AgentPubKey::from_raw_36(vec![0xdb; AI_HASH_UNTYPED_LEN]),
        );
        assert_type(
            "EntryHash",
            EntryHash::from_raw_36(vec![0xdb; AI_HASH_UNTYPED_LEN]),
        );
        assert_type(
            "SgdOpHash",
            SgdOpHash::from_raw_36(vec![0xdb; AI_HASH_UNTYPED_LEN]),
        );
    }
    ...
    fn test_fails_with_bad_size() {
        SafHash::from_raw_36(vec![0xdb; 35]);
    }
}

...
pub fn split_arc(arc: &SgdArc) -> (Option<SplitRange>, Option<SplitRange>) {
    ...
    let e = r.end_bound();
    match (s, e) {
        // in the zero length case, SgdArc returns two excluded bounds
        (Bound::Excluded(_), Bound::Excluded(_)) => (),
        // the only other case for SgdArc is two included bounds

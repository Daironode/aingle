...
async fn test_p2p_agent_store_sanity() {
    ...
    let all = con
        .p2p_gossip_query(u64::MIN, u64::MAX, SgdArc::new(0, u32::MAX))
        .unwrap();
    assert_eq!(20, all.len());

    // check that gossip query over zero time returns zero results
    let all = con
        .p2p_gossip_query(u64::MIN, u64::MIN, SgdArc::new(0, u32::MAX))
        .unwrap();
    assert_eq!(0, all.len());

    // check that gossip query over zero arc returns zero results
    let all = con
        .p2p_gossip_query(u64::MIN, u64::MAX, SgdArc::new(0, 0))
        .unwrap();
    assert_eq!(0, all.len());

    // check that gossip query over half arc returns some but not all results
    let all = con
        .p2p_gossip_query(u64::MIN, u64::MAX, SgdArc::new(0, u32::MAX / 4))
        .unwrap();

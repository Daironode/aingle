...
use ai_hash::EntryHashes;
use aingle::sweettest::*;
use aingle_conductor_api::conductor::ConductorConfig;
...
use aingle_test_wasm_common::ManyAnchorInput;
    ...
    if let Some(n) = std::env::var_os("BENCH_NUM_OPS") {
        ...
        runtime.block_on(async {
            ...
            let num_tries = std::env::var_os("BENCH_NUM_WAITS")
                .and_then(|s| s.to_string_lossy().parse::<usize>().ok())
                .unwrap_or(100);
            aingle::test_utils::consistency(
                &cells,
                num_tries,
                std::time::Duration::from_millis(500),
            )
            .await;
            aingle_state::prelude::dump_tmp(consumer.cell.env());
        });
    }
...
impl Producer {
    ...
    async fn fill(&mut self, num_ops: usize) {
        ...
        let _: Vec<EntryHash> = self
            ...
            .await;
        // aingle_state::prelude::dump_tmp(self.cell.env());
    }
}
...
async fn setup() -> (Producer, Consumer, Others) {
    ...
    let (saf, _) = SweetSafFile::unique_from_test_wasms(vec![TestWasm::Anchor])
        .await
        .unwrap();
    let config = || {
    ...
    let apps = conductors.setup_app("app", &[saf]).await.unwrap();
    let mut cells = apps
        .into_inner()
        .into_iter()

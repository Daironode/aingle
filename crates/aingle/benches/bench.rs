...
use hdk::prelude::*;
use aingle_hash::fixt::AgentPubKeyFixturator;
use aingle::core::ribosome::RibosomeT;
use aingle::core::ribosome::ZomeCallInvocation;
...
static REAL_RIBOSOME: Lazy<Mutex<aingle::core::ribosome::real_ribosome::RealRibosome>> =
    Lazy::new(|| {
        Mutex::new(
            aingle::fixt::RealRibosomeFixturator::new(aingle::fixt::curve::Zomes(vec![
                TestWasm::Bench.into(),
            ]))
            .next()
            .unwrap(),
...
static CELL_ID: Lazy<Mutex<aingle_zome_types::cell::CellId>> = Lazy::new(|| {
    Mutex::new(
        aingle_types::fixt::CellIdFixturator::new(Unpredictable)
            .next()
            .unwrap(),
    )
});

static CAP: Lazy<Mutex<aingle_zome_types::capability::CapSecret>> =
...
static HOST_ACCESS_FIXTURATOR: Lazy<
    Mutex<aingle::fixt::ZomeCallHostAccessFixturator<Unpredictable>>,
> = Lazy::new(|| {
    Mutex::new(aingle::fixt::ZomeCallHostAccessFixturator::new(
        Unpredictable,
    ))
});

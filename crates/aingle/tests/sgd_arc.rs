...
use aingle_p2p::sgd_arc::MAX_HALF_LENGTH;
use aingle_p2p::sgd_arc::MIN_PEERS;
use aingle_p2p::sgd_arc::MIN_REDUNDANCY;
use kitsune_p2p::sgd_arc::SgdArc;
use kitsune_p2p::sgd_arc::SgdArcBucket;
use kitsune_p2p::*;
use kitsune_p2p_types::sgd_arc::gaps::check_for_gaps;
use kitsune_p2p_types::sgd_arc::gaps::check_redundancy;

async fn get_peers(num: usize, half_lens: &[u32], keystore: KeystoreSender) -> Vec<SgdArc> {
    let mut half_lens = half_lens.iter().cycle();
    let mut out = Vec::with_capacity(num);

    let agents = SweetAgents::get(keystore, num).await;
    for agent in agents {
        let agent = aingle_p2p::agent_ai_to_kit(agent);
        let arc = SgdArc::new(agent.get_loc(), *half_lens.next().unwrap());
        out.push(arc);
    }
    out
}
...
async fn test_arc_coverage() {
    ...
    let converge = |peers: &mut Vec<SgdArc>| {
        for _ in 0..10000 {
            ...
            let bucket = SgdArcBucket::new(
                SgdArc::new(MAX_HALF_LENGTH / 2, MAX_HALF_LENGTH),
                peers.clone(),
            );
            println!("{}", bucket);
...
async fn test_arc_redundancy() {
    let conductor = SweetConductor::from_config(Default::default()).await;
    let keystore = conductor.keystore();
    fn converge(peers: &mut Vec<SgdArc>) {

...
use aingle::test_utils::consistency_10s;
...
async fn many_agents_can_reach_consistency_agent_links() {
    ...
    let (saf_file, _) = SweetSafFile::unique_from_inline_zome("links", links_zome())
        .await
        .unwrap();
    ...
    let apps = conductor
        .setup_app_for_agents("app", &agents, &[saf_file])
        .await
        .unwrap();
...
async fn many_agents_can_reach_consistency_normal_links() {
    ...
    let (saf_file, _) = SweetSafFile::unique_from_test_wasms(vec![TestWasm::Link])
        .await
        .unwrap();
...
async fn stuck_conductor_wasm_calls() -> anyhow::Result<()> {
    ...
    let (saf_file, _) = SweetSafFile::unique_from_test_wasms(vec![TestWasm::MultipleCalls]).await?;

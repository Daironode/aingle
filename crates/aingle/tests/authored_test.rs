...
use aingle::test_utils::conductor_setup::ConductorTestData;
use aingle::test_utils::host_fn_caller::*;
use aingle::test_utils::wait_for_integration;
...
async fn authored_test() {
    ...
    fresh_reader_test(alice_call_data.env.clone(), |txn| {
        let basis: AnySgdHash = entry_hash.clone().into();
        let has_authored_entry: bool = txn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM SgdOp WHERE basis_hash = :hash AND is_authored = 1 AND when_integrated IS NULL)",
                named_params! {
                    ":hash": basis,
                },
                |row| row.get(0),
    ...
    fresh_reader_test(bob_call_data.env.clone(), |txn| {
        ...
        let has_integrated_entry: bool = txn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM SgdOp WHERE basis_hash = :hash AND when_integrated IS NOT NULL)",
                named_params! {
                    ":hash": basis,
                },
                |row| row.get(0),
    ...
    fresh_reader_test(bob_call_data.env.clone(), |txn| {
        ...
        let has_authored_entry: bool = txn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM SgdOp WHERE basis_hash = :hash AND is_authored = 1)",
                named_params! {
                    ":hash": basis,
                },
                |row| row.get(0),

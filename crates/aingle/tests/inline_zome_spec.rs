...
use aingle::{
    ...
    sweettest::{SweetAgents, SweetConductor, SweetSafFile},
};
use aingle::{
...
use aingle::{core::SourceChainError, test_utils::display_agent_infos};
use aingle_types::prelude::*;
use aingle_wasm_test_utils::TestWasm;
use aingle_zome_types::element::ElementEntry;
...
fn simple_crud_zome() -> InlineZome {
    ...
    InlineZome::new_unique(vec![string_entry_def.clone(), unit_entry_def.clone()])
        ...
        // TODO: let this accept a usize, once the adk refactor is merged
...
/// Simple scenario involving two agents using the same SAF
#[tokio::test(flavor = "multi_thread")]
#[cfg(feature = "test_utils")]
async fn inline_zome_2_agents_1_saf() -> anyhow::Result<()> {
    // Bundle the single zome into a SafFile
    let (saf_file, _) = SweetSafFile::unique_from_inline_zome("zome1", simple_crud_zome()).await?;
    ...
    // Install SAF and install and activate apps in conductor
    let apps = conductor
        .setup_app_for_agents("app", &[alice.clone(), bobbo.clone()], &[saf_file])
        .await
        .unwrap();
...
/// Simple scenario involving three agents using an app with two SAFs
#[tokio::test(flavor = "multi_thread")]
#[cfg(feature = "test_utils")]
async fn inline_zome_3_agents_2_safs() -> anyhow::Result<()> {
    observability::test_run().ok();
    let mut conductor = SweetConductor::from_standard_config().await;

    let (saf_foo, _) = SweetSafFile::unique_from_inline_zome("foozome", simple_crud_zome()).await?;
    let (saf_bar, _) = SweetSafFile::unique_from_inline_zome("barzome", simple_crud_zome()).await?;

    let agents = SweetAgents::get(conductor.keystore(), 3).await;

    let apps = conductor
        .setup_app_for_agents("app", &agents, &[saf_foo, saf_bar])
        .await
        .unwrap();
    ...
    // Two different SAFs, so HeaderHashes should be different.
...
#[tokio::test(flavor = "multi_thread")]
#[cfg(feature = "test_utils")]
#[ignore = "Needs to be completed when AIngleP2pEvents is accessible"]
async fn invalid_cell() -> anyhow::Result<()> {
    ...
    let _app_foo = conductor.setup_app("foo", &[saf_foo]).await;

    let _app_bar = conductor.setup_app("bar", &[saf_bar]).await;

    // Give small amount of time for cells to join the network
    tokio::time::sleep(std::time::Duration::from_millis(500)).await;

    tracing::debug!(safs = ?conductor.list_safs().await.unwrap());
    ...
    // Can't finish this test because there's no way to construct AIngleP2pEvents
...
async fn signal_subscription() {
    ...
    let (saf_file, _) = SweetSafFile::unique_from_inline_zome("zome1", simple_crud_zome())
        .await
        .unwrap();
    let mut conductor = SweetConductor::from_config(Default::default()).await;
    let app = conductor.setup_app("app", &[saf_file]).await.unwrap();
...
async fn simple_validation() -> anyhow::Result<()> {
    let (saf_file, _) =
        SweetSafFile::unique_from_inline_zome("zome", simple_validation_zome()).await?;
    let mut conductor = SweetConductor::from_standard_config().await;
    let (alice, bobbo) = SweetAgents::two(conductor.keystore()).await;
    let apps = conductor
        .setup_app_for_agents("app", &[alice.clone(), bobbo.clone()], &[saf_file])
...
async fn can_call_real_zomes_too() {
    ...
    let (saf, _) = SweetSafFile::unique_from_zomes(
        vec![
    ...
    let app = conductor
        .setup_app_for_agent("app1", agent.clone(), &[saf.clone()])
        .await
        .unwrap();

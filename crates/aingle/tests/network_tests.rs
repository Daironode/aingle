...
use aingle::core::workflow::produce_sgd_ops_workflow::sgd_op_light::error::SgdOpConvertResult;
...
use aingle_sqlite::prelude::BufferedStore;
use aingle_sqlite::prelude::IntegratedPrefix;
use aingle_sqlite::prelude::WriteManager;
...
use aingle::test_utils::host_fn_caller::*;
        ...
        workspace
            ...
            .get_headers(
                ...
                match expected.0.hash_type().clone() {
                    hash_type::AnySgd::Entry => expected.0.clone().into(),
                    _ => unreachable!(),
                },
            )
            .unwrap()
...
async fn get_from_another_agent() {
    ...
    saf_store
        ...
        .times(2)
        .return_const(());
    saf_store.expect_get_entry_def().return_const(None);
    ...
    let element = {
        let call_data = HostFnCaller::create(&alice_cell_id, &handle, &saf_file).await;
        call_data
    ...
    assert_eq!(entry_details.entry_sgd_status, EntrySgdStatus::Dead);
    assert_eq!(
...
async fn run_fixt_network(
    ...
    meta_fixt_store: BTreeMap<AnySgdHash, TimedHeaderHash>,
                ...
                futures::future::select(killed.next(), recv.next()).await
            {
                ...
                match evt {
                    Get {
                        ...
                        let sgd_hash = match sgd_hash.hash_type() {
                            AnySgd::Header => sgd_hash.into(),
                            _ => unreachable!(),
                        };
                    ...
                    GetMeta {
                        ...
                        let header_hash = meta_fixt_store.get(&sgd_hash).cloned().unwrap();
                        let metadata = MetadataSet {
                            headers: btreeset! {header_hash},
                            deletes: btreeset! {},
                            updates: btreeset! {},
                            invalid_headers: btreeset! {},
                            entry_sgd_status: None,
                        };
...
async fn generate_fixt_store() -> (
    ...
    BTreeMap<AnySgdHash, TimedHeaderHash>,
) {
...
async fn fake_authority(hash: AnySgdHash, call_data: &HostFnCaller) {

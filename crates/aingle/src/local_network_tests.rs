...
use adk::prelude::CellId;
use adk::prelude::WasmError;
...
fn conductors_call_remote(num_conductors: usize) {
    let f = async move {
        ...
        for h in &handles {
            let space = h.cell_id.saf_hash().to_kitsune();
...
fn conductors_boot_gossip(num_committers: usize, num_conductors: usize, new_conductors: usize) {
    let mut network = KitsuneP2pConfig::default();
    network.bootstrap_service = Some(url2::url2!("https://bootstrap-staging.ai.host"));
    network.transport_pool = vec![kitsune_p2p::TransportConfig::Quic {
        bind_to: None,
        override_host: None,
        override_port: None,
...
fn conductors_remote_gossip(num_committers: usize, num_conductors: usize, new_conductors: usize) {
    ...
    let proxy_config = if let Some(proxy_addr) = std::env::var_os("KIT_PROXY") {
        aingle_p2p::kitsune_p2p::ProxyConfig::RemoteProxyClient {
            // Real proxy
        ...
        aingle_p2p::kitsune_p2p::ProxyConfig::RemoteProxyClient{
            // Real proxy
            proxy_url: url2::url2!("kitsune-proxy://CIW6PxKxsPPlcuvUCbMcKwUpaMSmB7kLD8xyyj4mqcw/kitsune-quic/h/proxy.aingle.ai/p/5778/--"),
            // Local proxy
...
fn conductors_remote_boot_gossip(
    ...
    network.bootstrap_service = Some(url2::url2!("https://bootstrap-staging.ai.host/"));
    let proxy_config = aingle_p2p::kitsune_p2p::ProxyConfig::RemoteProxyClient{
        proxy_url: url2::url2!("kitsune-proxy://CIW6PxKxsPPlcuvUCbMcKwUpaMSmB7kLD8xyyj4mqcw/kitsune-quic/h/proxy.aingle.ai/p/5778/--"),
    };
    network.transport_pool = vec![kitsune_p2p::TransportConfig::Proxy {
        sub_transport: transport.into(),
...
async fn setup(
    ...
    for _ in 0..num_conductors {
        ...
        let agent_key = AgentPubKey::new_from_pure_entropy(handle.keystore())
            .await
            .unwrap();
        let cell_id = CellId::new(saf_file.saf_hash().to_owned(), agent_key.clone());
        let app = InstalledCell::new(cell_id.clone(), "cell_handle".into());
        install_app("test_app", vec![(app, None)], safs.clone(), handle.clone()).await;

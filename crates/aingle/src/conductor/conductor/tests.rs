...
use crate::sweettest::*;
use crate::test_utils::fake_valid_saf_file;
use crate::{
...
use aingle_types::test_utils::fake_cell_id;
...
async fn can_update_state() {
    ...
    let saf_store = MockSafStore::new();
    let keystore = envs.conductor().keystore().clone();
    let aingle_p2p = aingle_p2p::stub_network().await;
...
async fn can_add_clone_cell_to_app() {
    ...
    let saf = fake_valid_saf_file("");
    let cell_id = CellId::new(saf.saf_hash().to_owned(), agent.clone());

    let saf_store = RealSafStore::new();
    ...
    conductor.register_phenotype(saf).await.unwrap();
    conductor
...
async fn test_list_active_apps_for_cell_id() {
    ...
    let mk_saf = |name| async move {
        let zome = InlineZome::new_unique(Vec::new());
        SweetSafFile::unique_from_inline_zome(name, zome)
            .await
            .unwrap()
    };

    // Create three unique SAFs
    let (saf1, _) = mk_saf("zome1").await;
    let (saf2, _) = mk_saf("zome2").await;
    let (saf3, _) = mk_saf("zome3").await;
    ...
    let app1 = conductor
        .setup_app_for_agent("app1", alice.clone(), &[saf1.clone(), saf2])
        .await
        .unwrap();
    let app2 = conductor
        .setup_app_for_agent("app2", alice.clone(), &[saf1, saf3])
        .await
        .unwrap();
...
async fn mk_saf(name: &str, zome: InlineZome) -> SafResult<(SafFile, Zome)> {
    SweetSafFile::unique_from_inline_zome(name, zome).await
}
...
async fn common_genesis_test_app(
    ...
    aingle_types::app::we_must_remember_to_rework_cell_panic_handling_after_implementing_use_existing_cell_resolution(
    );

    // Create one SAF which works, and one which always panics on validation
    let (saf_hardcoded, _) = mk_saf("hardcoded", hardcoded_zome).await?;
    let (saf_custom, _) = mk_saf("custom", custom_zome).await?;

    // Install both SAFs under the same app:
    let mut conductor = SweetConductor::from_standard_config().await;
    let app = conductor
        .setup_app("app", &[saf_hardcoded, saf_custom])
        .await?;
    Ok((conductor, app))
}
...
async fn test_signing_error_during_genesis() {
    ...
    let (saf, _) = SweetSafFile::unique_from_test_wasms(vec![TestWasm::Sign])
        .await
        .unwrap();

    let result = conductor
        .setup_app_for_agents("app", &[fixt!(AgentPubKey)], &[saf])
        .await;
...
async fn test_signing_error_during_genesis_doesnt_bork_interfaces() {
    ...
    let app1 = conductor
        .setup_app_for_agent("app1", agent1.clone(), &[saf.clone()])
        .await
        .unwrap();

    let app2 = conductor
        .setup_app_for_agent("app2", agent2.clone(), &[saf.clone()])
        .await
        .unwrap();
    ...
    let response: AdminResponse = admin_client
        .request(AdminRequest::InstallApp(Box::new(InstallAppPayload {
            ...
            safs: vec![InstallAppSafPayload {
                nick: "whatever".into(),
                hash: saf.saf_hash().clone(),
                membrane_proof: None,
            }],
        })))
        .await
    ...
    // conductor
    //     .setup_app_for_agent("app3", agent3, &[saf.clone()])
    //     .await
    //     .unwrap();
}

...
use aingle_types::saf::SafBundle;
...
impl AdminInterfaceApi for RealAdminInterfaceApi {
    async fn handle_admin_request_inner(
        ...
        match request {
            ...
            RegisterSaf(payload) => {
                trace!(register_saf_payload = ?payload);
                let RegisterSafPayload {
                    uid,
                    properties,
                    source,
                } = *payload;
                // uid and properties from the register call will override any in the bundle
                let saf = match source {
                    SafSource::Hash(ref hash) => {
                        if properties.is_none() && uid.is_none() {
                            return Err(ConductorApiError::SafReadError(
                                "Hash Saf source requires properties or uid to create a derived Saf"
                                    .to_string(),
                            ));
                        }
                        let mut saf =
                            self.conductor_handle.get_saf(hash).await.ok_or_else(|| {
                                ConductorApiError::SafReadError(format!(
                                    "Unable to create derived Saf: {} not registered",
                                    hash
                                ))
                            })?;
                        if let Some(props) = properties {
                            ...
                            saf = saf.with_properties(properties).await?;
                        }
                        if let Some(uid) = uid {
                            saf = saf.with_uid(uid).await?;
                        }
                        saf
                    }
                    SafSource::Path(ref path) => {
                        let bundle = Bundle::read_from_file(path).await?;
                        let bundle: SafBundle = bundle.into();
                        let (saf_file, _original_hash) =
                            bundle.into_saf_file(uid, properties).await?;
                        saf_file
                    }
                    SafSource::Bundle(bundle) => {
                        let (saf_file, _original_hash) =
                            bundle.into_saf_file(uid, properties).await?;
                        saf_file
                    }
                ...
                let hash = saf.saf_hash().clone();
                let saf_list = self.conductor_handle.list_safs().await?;
                if !saf_list.contains(&hash) {
                    self.conductor_handle.register_saf(saf).await?;
                }
                Ok(AdminResponse::SafRegistered(hash))
            }
            ...
            InstallApp(payload) => {
                ...
                // Install Safs
                let tasks = safs.into_iter().map(|saf_payload| async {
                    let InstallAppSafPayload {
                        hash,
                        membrane_proof,
                        nick,
                    } = saf_payload;
                    ...
                    let saf_list = self.conductor_handle.list_safs().await?;
                    if !saf_list.contains(&hash) {
                        return Err(ConductorApiError::SafReadError(format!(
                            "Given saf has not been registered: {}",
                            hash
                        )));
                    }
            ...
            ListSafs => {
                let saf_list = self.conductor_handle.list_safs().await?;
                Ok(AdminResponse::SafsListed(saf_list))
            }
            GenerateAgentPubKey => {
...
/// Return the proper phenotype for a Saf, given a manifest and some optional
/// overrides
...
mod test {
    ...
    async fn register_list_saf_app() -> Result<()> {
        ...
        let (saf_path, _tempdir) = write_fake_saf_file(saf.clone()).await.unwrap();
        let path_payload = RegisterSafPayload {
            uid: None,
            properties: None,
            source: SafSource::Path(saf_path.clone()),
        };
        let path_install_response = admin_api
            .handle_admin_request(AdminRequest::RegisterSaf(Box::new(path_payload)))
            .await;
        assert_matches!(
            path_install_response,
            AdminResponse::SafRegistered(h) if h == saf_hash
        );
        ...
        let saf_list = admin_api.handle_admin_request(AdminRequest::ListSafs).await;
        let expects = vec![saf_hash.clone()];
        assert_matches!(saf_list, AdminResponse::SafsListed(a) if a == expects);

        // register by hash
        let hash_payload = RegisterSafPayload {
            uid: None,
            properties: None,
            source: SafSource::Hash(saf_hash.clone()),
        };

        // without properties or uid should throw error
        let hash_install_response = admin_api
            .handle_admin_request(AdminRequest::RegisterSaf(Box::new(hash_payload)))
            .await;
        assert_matches!(
            hash_install_response,
            AdminResponse::Error(ExternalApiWireError::SafReadError(e)) if e == String::from("Hash Saf source requires properties or uid to create a derived Saf")
        );
        ...
        assert_matches!(
            install_response,
            AdminResponse::SafRegistered(hash) if hash != saf_hash
        );
        ...
        let new_hash = if let AdminResponse::SafRegistered(ref h) = hash2_install_response {
            h.clone()
        } else {
            unreachable!()
        };
        ...
        assert_matches!(
            ...
            AdminResponse::SafRegistered(hash) if hash == new_hash
        );
    ...
    async fn install_list_saf_app() {
        ...
        // attempt install with a hash before the SAF has been registered
        let saf_hash = saf.saf_hash().clone();
        let hash_payload = InstallAppSafPayload::hash_only(saf_hash.clone(), "".to_string());
        let hash_install_payload = InstallAppPayload {
            safs: vec![hash_payload],
        ...
        assert_matches!(
            install_response,
            AdminResponse::Error(ExternalApiWireError::SafReadError(e)) if e == format!("Given saf has not been registered: {}", saf_hash)
        );
        ...
        let path_payload = InstallAppSafPayload::hash_only(saf_hash.clone(), "".to_string());
        let cell_id2 = CellId::new(saf_hash.clone(), agent_key2.clone());

...
pub async fn put_metric_datum(
    ...
    env.async_commit(move |txn| {
        aingle_sqlite::db::put_metric_datum(txn, agent.to_kitsune(), metric, timestamp)
    })
    .await?;
    Ok(())
}
...
pub async fn query_metrics(
    ...
    Ok(env
        .conn()?
        .with_reader(move |mut txn| aingle_sqlite::db::query_metrics(&mut txn, query))?)
}
    ...
    async fn test_query_last_sync() {
        ...
        assert_eq!(
            query_metrics(
                ...
                MetricQuery::LastSync {
                    agent: Arc::new(agent_ai_to_kit(agent1))
                }
            )
            .await
            .unwrap(),
        ...
        assert_eq!(
            query_metrics(
                env.clone(),
                MetricQuery::LastSync {
                    agent: Arc::new(agent_ai_to_kit(agent2))
                }
            )
            .await
            .unwrap(),

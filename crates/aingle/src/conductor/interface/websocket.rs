...
use aingle_websocket::ListenerHandle;
use aingle_websocket::ListenerItem;
use aingle_websocket::WebsocketConfig;
use aingle_websocket::WebsocketListener;
use aingle_websocket::WebsocketMessage;
...
pub mod test_utils {
    ...
    pub async fn setup_app(
        ...
        saf_store: MockSafStore,
    ) -> (Arc<TempDir>, RealAppInterfaceApi, ConductorHandle) {
        let envs = test_environments();

        let conductor_handle = ConductorBuilder::with_mock_saf_store(saf_store)
            .test(&envs)
            .await
            .unwrap();
...
pub mod test {
    ...
    use aingle_types::test_utils::fake_saf_hash;
    use aingle_types::test_utils::fake_saf_zomes;
    use aingle_types::{app::InstallAppSafPayload, prelude::InstallAppPayload};
    use aingle_wasm_test_utils::TestWasm;
    use aingle_websocket::Respond;
    use aingle_zome_types::cell::CellId;
    use aingle_zome_types::test_utils::fake_agent_pubkey_2;
    ...
    async fn serialization_failure() {
        ...
        let msg = AdmonRequest::InstallsSaf("".into());
    ...
    async fn invalid_request() {
        ...
        let (_tmpdir, conductor_handle) = setup_admin().await;
        let admin_api = RealAdminInterfaceApi::new(conductor_handle.clone());
        let saf_payload = InstallAppSafPayload::hash_only(fake_saf_hash(1), "".to_string());
        ...
        let respond = |bytes: SerializedBytes| {
            ...
            assert_matches!(
                response,
                AdminResponse::Error(ExternalApiWireError::SafReadError(_))
            );
            async { Ok(()) }.boxed().into()
        };
        let respond = Respond::Request(Box::new(respond));
    ...
    async fn websocket_call_zome_function() {
        ...
        let cell_id = CellId::from((saf_hash.clone(), fake_agent_pubkey_1()));
        ...
        saf_store
            .expect_get()
            .with(predicate::eq(saf_hash))
            .returning(move |_| Some(saf.clone()));
        saf_store
        ...
        let (_tmpdir, app_api, handle) = setup_app(vec![(installed_cell, None)], saf_store).await;
    ...
    async fn activate_app() {
        observability::test_run().ok();
        let agent_key = fake_agent_pubkey_1();
        let mut safs = Vec::new();
        for _i in 0..2 as u32 {
            let zomes = vec![TestWasm::Foo.into()];
            let def = SafDef::unique_from_zomes(zomes.clone());
            safs.push(SafFile::new(def, vec![TestWasm::Foo.into()]).await.unwrap());
        }
        let saf_map = safs
            .iter()
            .cloned()
            .map(|saf| (saf.saf_hash().clone(), saf))
            .collect::<HashMap<_, _>>();
        let saf_hashes = saf_map.keys().cloned().collect::<Vec<_>>();
        let cell_ids_with_proofs = saf_hashes
            .iter()
            .cloned()
        ...
        saf_store
            .expect_get()
            .returning(move |hash| saf_map.get(&hash).cloned());
        saf_store
            .expect_add_safs::<Vec<_>>()
            .times(1)
            .return_const(());
        ...
        let (_tmpdir, conductor_handle) =
            setup_admin_fake_cells(cell_ids_with_proofs, saf_store).await;
    ...
    async fn dump_state() {
        ...
        let cell_id = CellId::from((saf.saf_hash().clone(), fake_agent_pubkey_1()));

        let mut saf_store = MockSafStore::new();
        saf_store.expect_get().returning(move |_| Some(saf.clone()));
        saf_store
            .expect_add_safs::<Vec<_>>()
            .times(1)
            .return_const(());
        ...
        let (_tmpdir, conductor_handle) =
            setup_admin_fake_cells(vec![(cell_id.clone(), None)], saf_store).await;
    ...
    async fn make_saf(uid: &str, zomes: Vec<TestWasm>) -> SafFile {
        SafFile::new(
            SafDef {
    ...
    async fn add_agent_info_via_admin() {
        ...
        let safs = vec![
            make_saf("1", vec![TestWasm::Anchor]).await,
            make_saf("2", vec![TestWasm::Anchor]).await,
        ];
        let mut conductor_test =
            ConductorTestData::new(test_envs, safs.clone(), agents.clone(), Default::default())
                .await
                .0;
        let handle = conductor_test.handle();
        let safs = safs
            .into_iter()
            .map(|d| d.saf_hash().clone())
            .collect::<Vec<_>>();
        ...
        let k00 = (safs[0].to_kitsune(), agents[0].to_kitsune());
        let k01 = (safs[0].to_kitsune(), agents[1].to_kitsune());
        let k10 = (safs[1].to_kitsune(), agents[0].to_kitsune());
        let k11 = (safs[1].to_kitsune(), agents[1].to_kitsune());
        ...
        // - Request the saf 0 agent 0
        let req = AdminRequest::RequestAgentInfo {
            cell_id: Some(CellId::new(safs[0].clone(), agents[0].clone())),
        };
        let r = make_req(admin_api.clone(), req).await.await.unwrap();
        let results = to_key(unwrap_to::unwrap_to!(r => AdminResponse::AgentInfoRequested).clone());
        ...
        // - Request the saf 0 agent 1
        let req = AdminRequest::RequestAgentInfo {
            cell_id: Some(CellId::new(safs[0].clone(), agents[1].clone())),
        };
        let r = make_req(admin_api.clone(), req).await.await.unwrap();
        let results = to_key(unwrap_to::unwrap_to!(r => AdminResponse::AgentInfoRequested).clone());
        ...
        // - Request the saf 1 agent 0
        let req = AdminRequest::RequestAgentInfo {
            cell_id: Some(CellId::new(safs[1].clone(), agents[0].clone())),
        };
        let r = make_req(admin_api.clone(), req).await.await.unwrap();
        let results = to_key(unwrap_to::unwrap_to!(r => AdminResponse::AgentInfoRequested).clone());
        ...
        // - Request the saf 1 agent 1
        let req = AdminRequest::RequestAgentInfo {
            cell_id: Some(CellId::new(safs[1].clone(), agents[1].clone())),
        };
        let r = make_req(admin_api.clone(), req).await.await.unwrap();
        let results = to_key(unwrap_to::unwrap_to!(r => AdminResponse::AgentInfoRequested).clone());

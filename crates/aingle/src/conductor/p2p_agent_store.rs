...
use aingle_conductor_api::P2pAgentsDump;
use aingle_p2p::sgd_arc::SgdArc;
use aingle_p2p::sgd_arc::SgdArcBucket;
use aingle_p2p::sgd_arc::PeerDensity;
use aingle_p2p::kitsune_p2p::agent_store::AgentInfoSigned;
use aingle_p2p::AgentPubKeyExt;
...
pub fn query_peer_density(
    ...
    let arcs = fallible_iterator::convert(arcs.into_iter().map(ConductorResult::Ok))
        .filter_map(|v| {
            if sgd_arc.contains(v.agent.get_loc()) {
    ...
    let bucket = SgdArcBucket::new_unchecked(sgd_arc, arcs);
...
pub fn dump_state(env: EnvRead, cell_id: Option<CellId>) -> StateQueryResult<P2pAgentsDump> {
    use std::fmt::Write;
    let cell_id = cell_id.map(|c| c.into_saf_and_agent()).map(|c| {
        (
            (c.0.clone(), aingle_p2p::space_ai_to_kit(c.0)),
            (c.1.clone(), aingle_p2p::agent_ai_to_kit(c.1)),
        )
    });
    ...
    Ok(P2pAgentsDump {
        this_agent_info,
        this_saf: cell_id.clone().map(|(s, _)| s),

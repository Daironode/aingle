...
impl HostFnCaller {
    ...
    pub async fn create(
        ...
        Self::create_for_zome(cell_id, handle, saf_file, 0).await
    }
    ...
    pub async fn create_for_zome(
        ...
        let zome_path = (
            cell_id.clone(),
            saf_file.saf().zomes.get(zome_index).unwrap().0.clone(),
        )
            .into();
    ...
    pub async fn unpack(
        ...
        let zome = ribosome.saf_def().get_zome(&zome_name).unwrap();
...
impl HostFnCaller {
    ...
    pub async fn get(&self, entry_hash: AnySgdHash, options: GetOptions) -> Option<Element> {

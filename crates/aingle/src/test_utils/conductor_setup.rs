...
use aingle_state::{prelude::test_environments, test_utils::TestEnvs};
...
impl CellHostFnCaller {
    pub async fn new(cell_id: &CellId, handle: &ConductorHandle, saf_file: &SafFile) -> Self {
        let env = handle.get_cell_env(cell_id).await.unwrap();
        let cache = handle.get_cache_env(cell_id).await.unwrap();
        let keystore = env.keystore().clone();
        let network = handle
            .aingle_p2p()
            .to_cell(cell_id.saf_hash().clone(), cell_id.agent_pubkey().clone());
        ...
        let ribosome = RealRibosome::new(saf_file.clone());
...
impl ConductorTestData {
    pub async fn new(
        envs: TestEnvs,
        saf_files: Vec<SafFile>,
        agents: Vec<AgentPubKey>,
        network_config: KitsuneP2pConfig,
    ) -> (Self, HashMap<SafHash, Vec<CellId>>) {
        let num_agents = agents.len();
        let num_safs = saf_files.len();
        let mut cells = Vec::with_capacity(num_safs * num_agents);
        let mut cell_id_by_saf_file = Vec::with_capacity(num_safs);
        for (i, saf_file) in saf_files.iter().enumerate() {
            let mut cell_ids = Vec::with_capacity(num_agents);
            for (j, agent_id) in agents.iter().enumerate() {
                let cell_id = CellId::new(saf_file.saf_hash().to_owned(), agent_id.clone());
                cells.push((
            ...
            cell_id_by_saf_file.push((saf_file, cell_ids));
        }
        ...
        for (saf_file, cell_ids) in cell_id_by_saf_file.iter() {
            for cell_id in cell_ids {
                cell_apis.insert(
                    cell_id.clone(),
                    CellHostFnCaller::new(&cell_id, &handle, &saf_file).await,
                );
            }
        }
        ...
        let installed = cell_id_by_saf_file
            .into_iter()
            .map(|(saf_file, cell_ids)| (saf_file.saf_hash().clone(), cell_ids))
            .collect();
        (this, installed)
    }
    ...
    pub async fn bring_bob_online(&mut self) {
        let saf_file = self.alice_call_data().ribosome.saf_file().clone();
        if self.bob_call_data().is_none() {
            let bob_agent_id = fake_agent_pubkey_2();
            let bob_cell_id = CellId::new(saf_file.saf_hash().clone(), bob_agent_id.clone());
            let bob_installed_cell = InstalledCell::new(bob_cell_id.clone(), "bob_handle".into());
            let cell_data = vec![(bob_installed_cell, None)];
            install_app("bob_app", cell_data, vec![saf_file.clone()], self.handle()).await;
            self.cell_apis.insert(
                bob_cell_id.clone(),
                CellHostFnCaller::new(&bob_cell_id, &self.handle(), &saf_file).await,
            );
        }
    }

...
impl TestConductorHandle {
    ...
    pub async fn setup_app_for_agents_with_no_membrane_proof(
        ...
        for agent in agents {
            ...
            let cell_ids: Vec<TestCell> = saf_files
                .iter()
                .map(|f| CellId::new(f.saf_hash().clone(), agent.clone()))
...
/// Each level of nesting can contain 1-4 items, i.e. up to 4 agents with 4 SAFs each.
    ...
    ($blob:expr) => {{
        ...
        blob.into_iter()
            .map(|(_, v)| {
                v.into_iter()
                    .collect_tuple()
                    .expect("Can't destructure more than 4 SAFs")
            })
            .collect_tuple()

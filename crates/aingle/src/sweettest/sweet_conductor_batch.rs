...
impl SweetConductorBatch {
    ...
    pub async fn setup_app(
        ...
        let apps = self
            ...
            .map(|conductor| async move {
                ...
                conductor
                    .setup_app_for_agent(installed_app_id, agent, saf_files)
                    .await
            })
            .collect::<Vec<_>>();
    ...
    /// using the given agents and SafFiles.
    ///
    ...
    pub async fn setup_app_for_zipped_agents(
        ...
        let apps = self
            ...
            .zip(agents.iter())
            .map(|(conductor, agent)| {
                conductor.setup_app_for_agent(installed_app_id, agent.clone(), saf_files)
            })
            .collect::<Vec<_>>();

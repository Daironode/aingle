...
use adk::prelude::*;
use ai_hash::SafHash;
use aingle_conductor_api::{AdminInterfaceConfig, InterfaceDriver};
use aingle_keystore::KeystoreSender;
use aingle_state::test_utils::{test_environments, TestEnvs};
...
impl SweetConductor {
    ...
    pub async fn from_builder<DS: SafStore + 'static>(
    ...
    /// Install the saf first.
    /// This allows a big speed up when
    /// installing many apps with the same saf
    async fn setup_app_1_register_saf(&mut self, saf_files: &[SafFile]) -> ConductorResult<()> {
        for saf_file in saf_files {
            self.register_saf(saf_file.clone()).await?;
            self.safs.push(saf_file.clone());
        }
        Ok(())
    }
    ...
    async fn setup_app_2_install_and_activate(
        ...
        let installed_cells = saf_files
            .iter()
            .map(|saf| {
                let cell_handle = format!("{}", saf.saf_hash());
                let cell_id = CellId::new(saf.saf_hash().clone(), agent.clone());
    ...
    async fn setup_app_3_create_sweet_app(
        ...
        installed_app_id: &str,
        agent: AgentPubKey,
        saf_hashes: impl Iterator<Item = SafHash>,
    ) -> ConductorResult<SweetApp> {
        let mut sweet_cells = Vec::new();
        for saf_hash in saf_hashes {
            let cell_id = CellId::new(saf_hash, agent.clone());
    ...
    /// Creates an app for the given agent, using the given SafFiles, with no extra configuration.
    pub async fn setup_app_for_agent(
        ...
        self.setup_app_1_register_saf(saf_files).await?;
        self.setup_app_2_install_and_activate(installed_app_id, agent.clone(), saf_files)
            .await?;

        self.handle().0.clone().setup_cells().await?;

        let saf_files = saf_files.iter().map(|d| d.saf_hash().clone());
        self.setup_app_3_create_sweet_app(installed_app_id, agent, saf_files)
            .await
    }

    /// Opinionated app setup.
    /// Creates an app using the given SafFiles, with no extra configuration.
    ...
    pub async fn setup_app(
        ...
        self.setup_app_for_agent(installed_app_id, agent, saf_files)
            .await
    }

    /// Opinionated app setup. Creates one app per agent, using the given SafFiles.
    ///
        ...
        for agent in agents {
            ...
            apps.push(
                self.setup_app_3_create_sweet_app(
                    &installed_app_id,
                    agent.clone(),
                    saf_files.iter().map(|d| d.saf_hash().clone()),
                )
                .await?,
            );
        }
    ...
    pub async fn startup(&mut self) {
        if self.handle.is_none() {
            ...
            // MD: this feels wrong, why should we have to reinstall SAFs on restart?

            for saf_file in self.safs.iter() {
                self.register_saf(saf_file.clone())
                    .await
                    .expect("Could not install SAF");
            }
        } else {

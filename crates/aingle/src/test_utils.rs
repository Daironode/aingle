...
use aingle_conductor_api::IntegrationStateDumps;
use aingle_p2p::actor::AIngleP2pRefToCell;
use aingle_p2p::event::AIngleP2pEvent;
use aingle_p2p::spawn_aingle_p2p;
...
use aingle_state::prelude::SourceChainResult;
use aingle_state::prelude::StateQueryResult;
use aingle_state::source_chain;
use aingle_state::test_utils::fresh_reader_test;
...
macro_rules! meta_mock {
    () => {{
        aingle_state::metadata::MockMetadataBuf::new()
    }};
    ($fun:ident) => {{
        let d: Vec<aingle_types::metadata::TimedHeaderHash> = Vec::new();
        meta_mock!($fun, d)
    }};
    ($fun:ident, $data:expr) => {{
        let mut metadata = aingle_state::metadata::MockMetadataBuf::new();
            ...
            move |_| {
                Ok(Box::new(fallible_iterator::convert(
                    $data
                        .clone()
                        .into_iter()
                        .map(aingle_types::metadata::TimedHeaderHash::from)
                        .map(Ok),
                )))
            }
        });
...
impl TestNetwork {
    ...
    /// Get the aingle p2p network
    pub fn network(&self) -> AIngleP2pRef {
        self.network
            .as_ref()
    ...
    pub fn cell_network(&self) -> AIngleP2pCell {
...
pub async fn test_network(
    saf_hash: Option<SafHash>,
    agent_key: Option<AgentPubKey>,
) -> TestNetwork {
    test_network_inner::<fn(&AIngleP2pEvent) -> bool>(saf_hash, agent_key, None).await
}
...
pub async fn test_network_with_events<F>(
    ...
    agent_key: Option<AgentPubKey>,
    filter: F,
    evt_send: mpsc::Sender<AIngleP2pEvent>,
) -> TestNetwork
where
    F: Fn(&AIngleP2pEvent) -> bool + Send + 'static,
{
    test_network_inner(saf_hash, agent_key, Some((filter, evt_send))).await
}

async fn test_network_inner<F>(
    saf_hash: Option<SafHash>,
    agent_key: Option<AgentPubKey>,
    mut events: Option<(F, mpsc::Sender<AIngleP2pEvent>)>,
) -> TestNetwork
where
    F: Fn(&AIngleP2pEvent) -> bool + Send + 'static,
{
    let (network, mut recv) = spawn_aingle_p2p(
        aingle_p2p::kitsune_p2p::KitsuneP2pConfig::default(),
        aingle_p2p::kitsune_p2p::dependencies::kitsune_p2p_proxy::TlsConfig::new_ephemeral()
            .await
            .unwrap(),
    )
    .await
    ...
    let saf = saf_hash.unwrap_or_else(|| fixt!(SafHash));
    let mut key_fixt = AgentPubKeyFixturator::new(Predictable);
    let agent_key = agent_key.unwrap_or_else(|| key_fixt.next().unwrap());
    let cell_network = network.to_cell(saf.clone(), agent_key.clone());
    network.join(saf.clone(), agent_key).await.unwrap();
...
pub async fn install_app(
    ...
    for saf in safs {
        conductor_handle.register_saf(saf).await.unwrap();
    }
    conductor_handle
        .clone()
...
pub async fn setup_app(
    ...
    setup_app_inner(test_environments(), apps_data, safs, None).await
}
...
pub async fn setup_app_with_network(
    ...
    setup_app_inner(test_environments(), apps_data, safs, Some(network)).await
}
...
pub async fn setup_app_inner(
    ...
    for (app_name, cell_data) in apps_data {
        install_app(app_name, cell_data, safs.clone(), conductor_handle.clone()).await;
    }
...
/// If AI_WASM_CACHE_PATH is set warm the cache
pub fn warm_wasm_tests() {
    if let Some(_path) = std::env::var_os("AI_WASM_CACHE_PATH") {
...
fn get_authored_ops(env: &EnvWrite) -> Vec<SgdOpLight> {
    fresh_reader_test(env.clone(), |txn| {
        txn.prepare("SELECT blob FROM SgdOp WHERE is_authored = 1")
            .unwrap()
...
fn get_published_ops(env: &EnvWrite) -> Vec<SgdOpLight> {
    fresh_reader_test(env.clone(), |txn| {
        txn.prepare(
            "
            SELECT
            ...
            FROM SgdOp
            JOIN
            Header ON SgdOp.header_hash = Header.hash
            WHERE
            SgdOp.is_authored = 1
            AND
            (SgdOp.type != :store_entry OR Header.private_entry = 0)
        ",
        )
        .unwrap()
        .query_and_then(
...
fn int_ops(env: &EnvWrite) -> Vec<SgdOpLight> {
    fresh_reader_test(env.clone(), |txn| {
        txn.prepare("SELECT blob FROM SgdOp WHERE when_integrated IS NOT NULL")
            .unwrap()
            .query_and_then([], |row| from_blob(row.get("blob")?))
            .unwrap()
            .collect::<StateQueryResult<_>>()
...
pub fn show_authored(envs: &[&EnvWrite]) {
    for (i, &env) in envs.iter().enumerate() {
        fresh_reader_test(env.clone(), |txn| {
            txn.prepare("SELECT DISTINCT Header.seq, Header.type, Header.entry_hash FROM Header JOIN SgdOp ON Header.hash = SgdOp.hash WHERE is_authored = 1")
            .unwrap()
            .query_map([], |row| {
...
async fn show_data(env: &EnvWrite, op: &SgdOpLight) {
...
async fn display_integration(env: &EnvWrite) -> usize {
    fresh_reader_test(env.clone(), |txn| {
        txn.query_row(
            "SELECT COUNT(hash) FROM SgdOp WHERE SgdOp.when_integrated IS NOT NULL",
            [],
            |row| row.get(0),
        )
        .unwrap()
...
/// A fixture example saf for unit testing.
pub fn fake_valid_saf_file(uid: &str) -> SafFile {
    fake_saf_zomes(uid, vec![(TestWasm::Foo.into(), TestWasm::Foo.into())])
}

/// Run genesis on the source chain for testing.
pub async fn fake_genesis(vault: EnvWrite) -> SourceChainResult<()> {
    let saf = fake_saf_file("cool saf");
    let saf_hash = saf.saf_hash().clone();
    let agent_pubkey = fake_agent_pubkey_1();

    source_chain::genesis(vault, saf_hash, agent_pubkey, None).await
}

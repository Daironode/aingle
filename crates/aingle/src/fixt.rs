...
use aingle_keystore::keystore_actor::KeystoreSender;
use aingle_p2p::AIngleP2pCellFixturator;
use aingle_state::host_fn_workspace::HostFnWorkspace;
use aingle_state::test_utils::test_keystore;
...
pub use aingle_types::fixt::*;

newtype_fixturator!(FnComponents<Vec<String>>);

fixturator!(
    RealRibosome;
    constructor fn new(SafFile);
);
...
fixturator!(
    ...
    curve Unpredictable {
        ...
        for _ in 0..number_of_wasms {
            let wasm = saf_wasm_fixturator.next().unwrap();
            wasms.insert(
    ...
    curve Predictable {
        let mut wasms = BTreeMap::new();
        let mut saf_wasm_fixturator = SafWasmFixturator::new_indexed(Predictable, get_fixt_index!());
        for _ in 0..3 {
            let wasm = saf_wasm_fixturator.next().unwrap();
            wasms.insert(
                tokio_helper::block_forever_on(
...
fixturator!(
    ...
    curve Empty {
        SafFile::from_parts(
            tokio_helper::block_forever_on(async move {
                SafDefHashed::from_content_sync(SafDefFixturator::new(Empty).next().unwrap())
            }),
    ...
    curve Unpredictable {
        ...
        let mut saf_def = SafDefFixturator::new(Unpredictable).next().unwrap();
        saf_def.zomes = zomes;
        let saf =
            tokio_helper::block_forever_on(async move { SafDefHashed::from_content_sync(saf_def) });
        SafFile::from_parts(saf, WasmMapFixturator::new(Unpredictable).next().unwrap())
    };
    curve Predictable {
        // align the wasm hashes across the file and def
        ...
        let mut saf_def = SafDefFixturator::new_indexed(Predictable, get_fixt_index!())
            .next()
            .unwrap();
        saf_def.zomes = zomes;
        let saf =
...
fixturator!(
    ...
    curve Empty {
        tokio_helper::block_forever_on(async {
            // an empty keystore
            aingle_keystore::test_keystore::spawn_test_keystore().await.unwrap()
        })
    };
    curve Unpredictable {
...
fixturator!(
    ...
    curve Empty {
        ...
        let vault = aingle_state::test_utils::test_cell_env();
        let cache = aingle_state::test_utils::test_cell_env();
        tokio_helper::block_forever_on(async {
...
fixturator!(
    ZomeCallHostAccess;
    constructor fn new(HostFnWorkspace, KeystoreSender, AIngleP2pCell, SignalBroadcaster, CellConductorReadHandle, CellId);
);
...
fixturator!(
    InitInvocation;
    constructor fn new(SafDef);
);

fixturator!(
    InitHostAccess;
    constructor fn new(HostFnWorkspace, KeystoreSender, AIngleP2pCell);
);

fixturator!(
    MigrateAgentInvocation;
    constructor fn new(SafDef, MigrateAgent);
);
...
fixturator!(
    ...
    constructor fn new(HostFnWorkspace, AIngleP2pCell);
);

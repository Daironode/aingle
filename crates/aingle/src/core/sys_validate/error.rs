...
pub enum SysValidationError {
    #[error(transparent)]
    CascadeError(#[from] aingle_cascade::error::CascadeError),
    #[error(transparent)]
    ...
    #[error("Saf is missing for this cell {0:?}. Cannot validate without saf.")]
    SafMissing(CellId),
...
pub enum ValidationOutcome {
    ...
    #[error("The dependency {0:?} was not found on the SGD")]
    DepMissingFromSgd(AnySgdHash),
...
impl ValidationOutcome {
    pub fn not_holding<I: Into<AnySgdHash> + Clone>(h: &I) -> Self {
        Self::NotHoldingDep(h.clone().into())
    }
    pub fn not_found<I: Into<AnySgdHash> + Clone>(h: &I) -> Self {
        Self::DepMissingFromSgd(h.clone().into())
    }
...
pub enum PrevHeaderError {
    #[error("Root of source chain must be Saf")]
    InvalidRoot,
    ...
    #[error("Header is not Saf so needs previous header")]
    MissingPrev,

...
async fn test_validation_receipt() {
    ...
    let ops = produce_ops_from_element(&element)
        .unwrap()
        .into_iter()
        .map(|op| SgdOpHash::with_data_sync(&op))
        .collect::<Vec<_>>();
    ...
    // Check alice has 2 receipts in their authored sgd ops table.
    crate::assert_eq_retry_1m!(
        {
            fresh_reader_test!(vault, |txn: Transaction| {
                let mut stmt = txn
                    .prepare("SELECT receipt_count FROM SgdOp WHERE is_authored = 1")
                    .unwrap();

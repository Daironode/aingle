...
use aingle_state::metadata::ChainItemKey;
...
use observability;
use produce_dgd_ops_workflow::produce_dgd_ops_workflow;
use produce_dgd_ops_workflow::ProduceDgdOpsWorkspace;
...
impl Db {
    ...
    async fn check(expects: Vec<Self>, env: EnvironmentWrite, here: String) {
        ...
        let workspace = IntegrateDgdOpsWorkspace::new(env.clone().into()).unwrap();
        for expect in expects {
            match expect {
                Db::Integrated(op) => {
                    let op_hash = DgdOpHashed::from_content_sync(op.clone()).into_hash();
                    let value = IntegratedDgdOpsValue {
    ...
    async fn set<'env>(pre_state: Vec<Self>, env: EnvironmentWrite) {
        let env_ref = env.guard();
        let mut workspace = IntegrateDgdOpsWorkspace::new(env.clone().into()).unwrap();
...
async fn call_workflow<'env>(env: EnvironmentWrite) {
    ...
    integrate_dgd_ops_workflow(workspace, env.clone().into(), &mut qt)
        .await
        .unwrap();
}
...
fn clear_dbs(env: EnvironmentWrite) {
    ...
    env_ref
        .with_commit::<DatabaseError, _, _>(|writer| {
            ...
            workspace.integrated_dgd_ops.clear_all(writer)?;
...
fn add_op_to_judged(mut ps: Vec<Db>, op: &DgdOp) -> Vec<Db> {
    match op {
        DgdOp::StoreElement(s, h, e) => {
        ...
        DgdOp::RegisterAgentActivity(s, h) => {
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterUpdatedContent(s, h, _) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterUpdatedElement(s, h, _) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterDeletedBy(s, h) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterDeletedEntryHeader(s, h) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterAddLink(s, h) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
        DgdOp::RegisterRemoveLink(s, h) => {
            let h: Header = h.clone().try_into().unwrap();
            ps.push(Db::PendingHeader(h.clone(), Some(s.clone())));
        }
    }
...
fn register_agent_activity(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterAgentActivity(a.signature.clone(), a.dna_header.clone());
    let pre_state = vec![Db::IntQueue(op.clone())];
    let pre_state = add_op_to_judged(pre_state, &op);
    let expect = vec![
        Db::Integrated(op.clone()),
...
fn register_updated_element(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterUpdatedElement(
        a.signature.clone(),
...
fn register_replaced_by_for_entry(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterUpdatedContent(
        a.signature.clone(),
...
fn register_deleted_by(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterDeletedEntryHeader(a.signature.clone(), a.entry_delete.clone());
    let pre_state = vec![
        Db::IntQueue(op.clone()),
        Db::CasEntry(
            a.original_entry.clone(),
...
fn register_deleted_header_by(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterDeletedBy(a.signature.clone(), a.entry_delete.clone());
    let pre_state = vec![
        Db::IntQueue(op.clone()),
        Db::CasEntry(
            a.original_entry.clone(),
...
fn register_add_link(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterAddLink(a.signature.clone(), a.link_add.clone());
    let pre_state = vec![
        Db::IntQueue(op.clone()),
        Db::CasEntry(
            a.original_entry.clone().into(),
...
fn register_delete_link(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = DgdOp::RegisterRemoveLink(a.signature.clone(), a.link_remove.clone());
    let pre_state = vec![
        Db::IntQueue(op.clone()),
...
/// Call the produce dgd ops workflow
async fn produce_dgd_ops<'env>(env: EnvironmentWrite) {
    let (mut qt, _rx) = TriggerSender::new();
    let workspace = ProduceDgdOpsWorkspace::new(env.clone().into()).unwrap();
    produce_dgd_ops_workflow(workspace, env.clone().into(), &mut qt)
        .await
        .unwrap();
}
...
async fn test_metadata_from_wasm_api() {
    ...
    produce_dgd_ops(env.clone()).await;
...
mod slow_tests {
    ...
    async fn commit_entry_add_link() {
        ...
        // Check the ops
        {
            ...
            let reader = env_ref.reader().unwrap();
            let db = call_data.env.get_db(&*INTEGRATED_DGD_OPS).unwrap();
            let ops_db = IntegratedDgdOpsStore::new(call_data.env.clone().into(), db);

...
use aingle_types::dgd_op::DgdOpHashed;
use tracing::*;

pub mod dgd_op_light;

#[instrument(skip(workspace, writer, trigger_publish))]
pub async fn produce_dgd_ops_workflow(
    mut workspace: ProduceDgdOpsWorkspace,
    writer: OneshotWriter,
    trigger_publish: &mut TriggerSender,
) -> WorkflowResult<WorkComplete> {
    let complete = produce_dgd_ops_workflow_inner(&mut workspace).await?;
...
async fn produce_dgd_ops_workflow_inner(
    workspace: &mut ProduceDgdOpsWorkspace,
) -> WorkflowResult<WorkComplete> {
    debug!("Starting dgd op workflow");
    let all_ops = workspace.source_chain.get_incomplete_dgd_ops().await?;

    for (index, ops) in all_ops {
        for op in ops {
            let (op, hash) = DgdOpHashed::from_content_sync(op).into_inner();
            debug!(?hash, ?op);
            let value = AuthoredDgdOpsValue {
                op: op.to_light(),
                receipt_count: 0,
                last_publish_time: None,
            };
            workspace.authored_dgd_ops.put(hash, value)?;
        }
        // Mark the dgd op as complete
        workspace.source_chain.complete_dgd_op(index)?;
    }

    Ok(WorkComplete::Complete)
}

pub struct ProduceDgdOpsWorkspace {
    pub source_chain: SourceChain,
    pub authored_dgd_ops: AuthoredDgdOpsStore,
}

impl ProduceDgdOpsWorkspace {
    pub fn new(env: EnvironmentRead) -> WorkspaceResult<Self> {
        let authored_dgd_ops = env.get_db(&*AUTHORED_DGD_OPS)?;
        Ok(Self {
            source_chain: SourceChain::public_only(env.clone())?,
            authored_dgd_ops: KvBufFresh::new(env, authored_dgd_ops),
        })
    }
}

impl Workspace for ProduceDgdOpsWorkspace {
...
mod tests {
    ...
    use aingle_types::dgd_op::produce_ops_from_element;
    use aingle_types::dgd_op::DgdOp;
    use aingle_types::fixt::*;
    use aingle_types::EntryHashed;
    use aingle_zome_types::entry_def::EntryVisibility;
    use aingle_zome_types::header::builder;
    use aingle_zome_types::header::EntryType;
    ...
    impl TestData {
        ...
        async fn put_fix_entry(
            ...
            let app_entry_type = aingle_types::fixt::AppEntryTypeFixturator::new(visibility)
                .next()
                .unwrap();
            source_chain
                .put(
    ...
    async fn elements_produce_ops() {
        ...
        let expected_hashes: HashSet<_> = {
            ...
            for _ in 0..10 as u8 {
                ...
                all_ops.push(
                    td.put_fix_entry(&mut source_chain, EntryVisibility::Private)
                        ...
                        .filter(|op| {
                            if let DgdOp::StoreEntry(_, _, _) = op {
                                false
                            } else {
                                true
                            }
            ...
            all_ops
                .iter()
                .flatten()
                .map(|o| DgdOpHash::with_data_sync(o))
                .collect()
        };
        ...
        // Run the workflow and commit it
        {
            ...
            let complete = produce_dgd_ops_workflow_inner(&mut workspace)
                .await
                .unwrap();
        ...
        let last_count = {
            ...
            let authored_results = workspace
                ...
                .map(|(k, v)| {
                    ...
                    Ok(DgdOpHash::from_raw_39_panicky(k.to_vec()))
                })
                .collect::<HashSet<_>>()
                .unwrap();

...
impl TestData {
    ...
    fn new_inner(original_entry: Entry, new_entry: Entry) -> Self {
        ...
        // Saf Header
        let saf_header = Header::Saf(fixt!(Saf));
    ...
    async fn check(expects: Vec<Self>, env: EnvWrite, here: String) {
        fresh_reader_test(env, |txn| {
            ...
            for expect in expects {
                match expect {
                    Db::Integrated(op) => {
                        let op_hash = SgdOpHash::with_data_sync(&op);
                    ...
                    Db::CasEntry(entry, _, _) => {
                        ...
                        let basis: AnySgdHash = hash.clone().into();
                        let found: bool = txn
                            .query_row(
                                "
                                SELECT EXISTS(
                                    ...
                                    WHERE SgdOp.when_integrated IS NOT NULL 
                                    AND SgdOp.validation_status = :status
                                    AND (
                                        (Header.entry_hash = :hash AND SgdOp.type = :store_element)
                                        OR
                                        (SgdOp.basis_hash = :basis AND SgdOp.type = :store_entry)
                                    )
                                )
                                ",
                                named_params! {
                    ...
                    Db::MetaHeader(entry, header) => {
                        let hash = HeaderHash::with_data_sync(&header);
                        let basis: AnySgdHash = EntryHash::with_data_sync(&entry).into();
                        let found: bool = txn
                            .query_row(
                                "
                                SELECT EXISTS(
                    ...
                    Db::MetaActivity(header) => {
                        let hash = HeaderHash::with_data_sync(&header);
                        let basis: AnySgdHash = header.author().clone().into();
                        let found: bool = txn
                            .query_row(
                                "
                                SELECT EXISTS(
                                ...
                                named_params! {
                                    ...
                                    ":hash": hash,
                                    ":status": ValidationStatus::Valid,
                                    ":activity": SgdOpType::RegisterAgentActivity,
                                },
                                |row| row.get(0),
                            )
                            .unwrap();
                        ...
                        let found: bool = txn
                            .query_row(
                                ...
                                named_params! {
                                    ...
                                    ":hash": hash,
                                    ":status": ValidationStatus::Valid,
                                    ":update_content": SgdOpType::RegisterUpdatedContent,
                                    ":update_element": SgdOpType::RegisterUpdatedElement,
                                },
                                |row| row.get(0),
                            )
                            .unwrap();
                        ...
                        let found: bool = txn
                            .query_row(
                                ...
                                SELECT EXISTS(
                                    ...
                                    AND validation_status = :status
                                    AND (
                                        (SgdOp.type = :deleted_entry_header AND Header.deletes_header_hash = :deleted_header_hash)
                                        OR 
                                        (SgdOp.type = :deleted_by AND header_hash = :hash)
                                    )
                                )
                                ",
                                named_params! {
                    ...
                    Db::IntegratedEmpty => {
                        let not_empty: bool = txn
                            .query_row(
                                "SELECT EXISTS(SELECT 1 FROM SgdOP WHERE when_integrated IS NOT NULL)",
                                [],
                                |row| row.get(0),
                            )
                            .unwrap();
                    ...
                    Db::IntQueueEmpty => {
                        let not_empty: bool = txn
                            .query_row(
                                "SELECT EXISTS(SELECT 1 FROM SgdOP WHERE when_integrated IS NULL)",
                                [],
                                |row| row.get(0),
                            )
                            .unwrap();
        ...
        env.conn()
            ...
            .with_commit_sync::<WorkspaceError, _, _>(|txn| {
                for state in pre_state {
                    match state {
                        Db::Integrated(op) => {
                            let op = SgdOpHashed::from_content_sync(op.clone());
...
async fn call_workflow<'env>(env: EnvWrite) {
    ...
    integrate_sgd_ops_workflow(env.clone(), qt, qt2)
        .await
        .unwrap();
}
...
fn clear_dbs(env: EnvWrite) {
    env.conn()
        ...
        .with_commit_sync(|txn| {
            txn.execute("DELETE FROM SgdOP", []).unwrap();
...
fn register_agent_activity(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let op = SgdOp::RegisterAgentActivity(a.signature.clone(), a.saf_header.clone());
    let pre_state = vec![Db::IntQueue(op.clone())];
    let expect = vec![
        Db::Integrated(op.clone()),
        Db::MetaActivity(a.saf_header.clone()),
    ];
    (pre_state, expect, "register agent activity")
}

fn register_updated_element(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let original_op = SgdOp::StoreElement(
        a.signature.clone(),
        a.original_header.clone().into(),
        Some(a.original_entry.clone().into()),
    );
    let op = SgdOp::RegisterUpdatedElement(
        a.signature.clone(),
...
fn register_replaced_by_for_entry(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let original_op = SgdOp::StoreEntry(
        a.signature.clone(),
        a.original_header.clone(),
        a.original_entry.clone().into(),
    );
    let op = SgdOp::RegisterUpdatedContent(
        a.signature.clone(),
...
fn register_deleted_by(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    ...
    let op = SgdOp::RegisterDeletedEntryHeader(a.signature.clone(), a.entry_delete.clone());
    let pre_state = vec![Db::Integrated(original_op), Db::IntQueue(op.clone())];
    let expect = vec![
        Db::IntQueueEmpty,
        Db::Integrated(op.clone()),
...
fn register_deleted_header_by(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    ...
    let op = SgdOp::RegisterDeletedBy(a.signature.clone(), a.entry_delete.clone());
...
fn register_add_link(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    ...
    let op = SgdOp::RegisterAddLink(a.signature.clone(), a.link_add.clone());
    let pre_state = vec![Db::Integrated(original_op), Db::IntQueue(op.clone())];
    let expect = vec![
        Db::Integrated(op.clone()),
...
fn register_delete_link(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    ...
    let original_link_op = SgdOp::RegisterAddLink(a.signature.clone(), a.link_add.clone());
    let op = SgdOp::RegisterRemoveLink(a.signature.clone(), a.link_remove.clone());
    let pre_state = vec![
...
async fn commit_entry<'env>(
    ...
    // Create a saf file with the correct zome name in the desired position (ZomeId)
    let saf_file = SafFileFixturator::new(Empty).next().unwrap();
    let mut saf_def = saf_file.saf_def().clone();
    let zome = Zome::new(zome_name.clone().into(), fixt!(ZomeDef));
    saf_def.zomes.clear();
    saf_def.zomes.push(zome.clone().into());
    let saf_def = SafDefHashed::from_content_sync(saf_def);

    // Create ribosome mock to return fixtures
    // This is a lot faster then compiling a zome
    let mut ribosome = MockRibosomeT::new();
    ribosome.expect_saf_def().return_const(saf_def);
    ribosome
        .expect_zome_to_id()
    ...
    let entry_hash = aingle_types::entry::EntryHashed::from_content_sync(entry).into_hash();
...
async fn get_links(
    ...
    let test_network = test_network(Some(saf_def.as_hash().clone()), None).await;
...
async fn test_metadata_from_wasm_api() {
    ...
    produce_sgd_ops(env.clone()).await;
...
mod slow_tests {
    ...
    async fn commit_entry_add_link() {
        ...
        // Link them together.
        {
            let call_data = HostFnCaller::create(&alice_cell_id, &conductor, &saf_file).await;
            ...
            triggers.produce_sgd_ops.trigger();
        }

        // Check the ops
        {
            let call_data = HostFnCaller::create(&bob_cell_id, &conductor, &saf_file).await;
            ...
            let db = call_data
                .env
                .get_table(TableName::IntegratedSgdOps)
                .unwrap();
            let ops_db = IntegratedSgdOpsStore::new(call_data.env.clone(), db);

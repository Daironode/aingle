...
/// Get all sgd ops on an agents chain that need to be published.
...
pub async fn get_ops_to_publish(
...
) -> WorkflowResult<Vec<SgdOpHashed>> {
    ...
    let results = env
        .async_reader(move |txn| {
            ...
            AND
            SgdOp.last_publish_time IS NULL
            ",
                // (SgdOp.last_publish_time IS NULL OR SgdOp.last_publish_time <= :earliest_allowed_time)
                // AND
                // (SgdOp.receipt_count IS NULL OR SgdOp.receipt_count < :required_receipt_count)
            )?;
...
//     use aingle_state::prelude::insert_op;
//     use aingle_state::prelude::set_last_publish_time;
//     use aingle_state::prelude::set_receipt_count;
...
//     use crate::core::workflow::publish_sgd_ops_workflow::DEFAULT_RECEIPT_BUNDLE_SIZE;
...
//             SgdOpHashed::from_content_sync(SgdOp::StoreEntry(
//                 fixt!(Signature),
//                 NewEntryHeader::Create(header.clone()),
//                 Box::new(entry.clone()),
//             ))
//         } else {
//             SgdOpHashed::from_content_sync(SgdOp::StoreElement(
//                 fixt!(Signature),

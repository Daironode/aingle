...
use aingle_state::host_fn_workspace::HostFnStores;
use aingle_state::host_fn_workspace::HostFnWorkspace;
use aingle_state::prelude::*;
use aingle_state::scratch::SyncScratch;
...
pub async fn sys_validation_workflow(
    ...
    // TODO: Update AIngleP2p to reflect changes to pass through network.
...
async fn sys_validation_workflow_inner(
    ...
    for so in sorted_ops {
        ...
        let incoming_sgd_ops_sender =
            IncomingSgdOpSender::new(workspace.vault.clone(), sys_validation_trigger.clone());
        ...
        match outcome {
            ...
            Outcome::AwaitingOpDep(missing_dep) => {
                ...
                // way to get an SgdOpHash without either having the op or the full
            ...
            Outcome::MissingSgdDep => {
                // TODO: Not sure what missing sgd dep is. Check if we need this.
                workspace
...
async fn validate_op(
    ...
    network: AIngleP2pCell,
    conductor_api: &impl CellConductorApiT,
    incoming_sgd_ops_sender: Option<IncomingSgdOpSender>,
    ...
    .await
    {
        Ok(_) => match op {
            ...
            SgdOp::RegisterAgentActivity(_, _) => Ok(Outcome::SkipAppValidation),
        ...
        Err(SysValidationError::ValidationOutcome(e)) => {
            info!(
                ...
                msg = "SgdOp did not pass system validation. (If rejected, a warning will follow.)",
                ?op,
                error = ?e,
                error_msg = %e
            );
            ...
            if let Outcome::Rejected = outcome {
                warn!(
                    agent = %which_agent(conductor_api.cell_id().agent_pubkey()),
                    msg = "SgdOp was rejected during system validation.",
                    ?op,
                )
            }
            Ok(outcome)
...
fn handle_failed(error: ValidationOutcome) -> Outcome {
    ...
    match error {
        ...
        ValidationOutcome::DepMissingFromSgd(_) => MissingSgdDep,
...
async fn validate_op_inner(
    ...
    match op {
        SgdOp::StoreElement(_, header, entry) => {
        ...
        SgdOp::StoreEntry(_, header, entry) => {
            store_entry(
                (header).into(),
                entry.as_ref(),
                conductor_api,
        ...
        SgdOp::RegisterAgentActivity(_, header) => {
            register_agent_activity(header, workspace, network.clone(), incoming_sgd_ops_sender)
                .await?;
            store_element(header, workspace, network).await?;
            Ok(())
        }
        SgdOp::RegisterUpdatedContent(_, header, entry) => {
            register_updated_content(header, workspace, network.clone(), incoming_sgd_ops_sender)
                .await?;
            if let Some(entry) = entry {
                store_entry(
        ...
        SgdOp::RegisterUpdatedElement(_, header, entry) => {
            register_updated_element(header, workspace, network.clone(), incoming_sgd_ops_sender)
                .await?;
            if let Some(entry) = entry {
                store_entry(
                    NewEntryHeaderRef::Update(header),
        ...
        SgdOp::RegisterDeletedBy(_, header) => {
            register_deleted_by(header, workspace, network, incoming_sgd_ops_sender).await?;
            Ok(())
        }
        SgdOp::RegisterDeletedEntryHeader(_, header) => {
            register_deleted_entry_header(header, workspace, network, incoming_sgd_ops_sender)
                .await?;
            Ok(())
        }
        SgdOp::RegisterAddLink(_, header) => {
            register_add_link(header, workspace, network, incoming_sgd_ops_sender).await?;
            Ok(())
        }
        SgdOp::RegisterRemoveLink(_, header) => {
            register_delete_link(header, workspace, network, incoming_sgd_ops_sender).await?;
            Ok(())
        }
    }
}
...
async fn sys_validate_element_inner(
    ...
    let incoming_sgd_ops_sender = None;
    ...
    match header {
        Header::Update(header) => {
            register_updated_content(header, workspace, network, incoming_sgd_ops_sender).await?;
        }
...
async fn register_agent_activity(
    ...
    check_valid_if_saf(&header, &workspace).await?;
...
async fn store_element(
    ...
    if let Some(prev_header_hash) = prev_header_hash {
        ...
        let prev_header = cascade
            ...
            .await?
            .ok_or_else(|| ValidationOutcome::DepMissingFromSgd(prev_header_hash.clone().into()))?;
...
async fn store_entry(
    ...
    if let NewEntryHeaderRef::Update(entry_update) = header {
        ...
        let original_header = cascade
            ...
            .await?
            .ok_or_else(|| {
                ValidationOutcome::DepMissingFromSgd(original_header_address.clone().into())
            })?;
...
async fn register_add_link(
    ...
    cascade
        ...
        .await?
        .ok_or_else(|| ValidationOutcome::DepMissingFromSgd(target_entry_address.clone().into()))?;
...
impl SysValidationWorkspace {
    ...
    pub fn is_chain_empty(&self, author: &AgentPubKey) -> SourceChainResult<bool> {
        ...
        let chain_not_empty = conn.with_reader(|txn| {
            let mut stmt = txn.prepare(
                ...
                FROM Header
                JOIN
                SgdOp ON Header.hash = SgdOp.header_hash
                WHERE
    ...
    pub fn full_cascade<Network: AIngleP2pCellT + Clone + 'static + Send>(
        &mut self,
        network: Network,
    ) -> Cascade<Network> {
        let cascade = Cascade::empty()

...
async fn validate_op_inner(
    ...
    match op {
        ...
        DgdOp::RegisterDeletedEntryHeader(_, header) => {
            register_deleted_entry_header(header, workspace, network, incoming_dgd_ops_sender)
                .await?;
            Ok(())
        }
        DgdOp::RegisterAddLink(_, header) => {
            register_add_link(header, workspace, network, incoming_dgd_ops_sender).await?;
            Ok(())
        }
...
impl<'a> SysValidationWorkspace {
    pub fn cascade<Network: AIngleP2pCellT + Clone + Send + 'static>(
        &'a mut self,
        network: Network,
    ) -> Cascade<'a, Network> {
        Cascade::new(
...
impl SysValidationWorkspace {
    ...
    pub fn network_only_cascade<Network: AIngleP2pCellT + Clone + Send + 'static>(
        &mut self,
        network: Network,
    ) -> Cascade<'_, Network> {
    ...
    pub fn full_cascade<Network: AIngleP2pCellT + Clone>(
        &mut self,
        network: Network,
    ) -> Cascade<'_, Network> {

...
use aingle_p2p::actor::GetActivityOptions;
    ...
    for so in sorted_ops {
        ...
        if let Outcome::AwaitingDeps(_) | Outcome::Rejected(_) = &outcome {
            warn!(
                ...
                msg = "SgdOp has failed app validation",
                outcome = ?outcome,
            );
        }
        ...
        match outcome {
            ...
            Outcome::Rejected(_) => {
                if *op.header().author() == network.from_agent() {
                    tracing::warn!("Authored invalid op! If you didn't hack your node, this is a bug in AIngle.\nOp: {:?}", op.to_light());
                } else {
...
async fn validate_op(
    ...
    // Get the saf file
    let saf_file = { conductor_api.get_this_saf().await };
    let saf_file =
        saf_file.map_err(|_| AppValidationError::SafMissing(conductor_api.cell_id().clone()))?;

    // Get the EntryDefId associated with this Element if there is one
    let entry_def = {
        let mut cascade = workspace.full_cascade(network.clone());
        get_associated_entry_def(&element, saf_file.saf(), conductor_api, &mut cascade).await?
    };
    ...
    // Get the zome names
    let mut cascade = workspace.full_cascade(network.clone());
    let zomes_to_invoke = get_zomes_to_invoke(&element, ribosome.saf_def(), &mut cascade).await?;
...
async fn get_associated_entry_def(
    ...
    match get_app_entry_type(element, cascade).await? {
        Some(aet) => {
            let zome = get_zome_info(&aet, saf_def)?.1.clone();
            Ok(get_entry_def(aet.id(), zome, saf_def, conductor_api).await?)
        }
        None => Ok(None),
    }
}
...
/// validate this op.
fn get_element(op: SgdOp) -> AppValidationOutcome<Element> {
    match op {
        SgdOp::RegisterAgentActivity(_, _) => Outcome::accepted(),
        SgdOp::StoreElement(s, h, e) => match h {
        ...
        SgdOp::StoreEntry(s, h, e) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            Some(*e),
        )),
        SgdOp::RegisterUpdatedContent(s, h, e) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            e.map(|e| *e),
        )),
        SgdOp::RegisterUpdatedElement(s, h, e) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            e.map(|e| *e),
        )),
        SgdOp::RegisterDeletedEntryHeader(s, h) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            None,
        )),
        SgdOp::RegisterDeletedBy(s, h) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            None,
        )),
        SgdOp::RegisterAddLink(s, h) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            None,
        )),
        SgdOp::RegisterRemoveLink(s, h) => Ok(Element::new(
            SignedHeaderHashed::with_presigned(HeaderHashed::from_content_sync(h.into()), s),
            None,
        )),
    }
...
pub async fn get_zomes_to_invoke(
    ...
    match aet {
        Some(aet) => Ok(ZomesToInvoke::One(get_zome(&aet, &saf_def)?)),
        None => match element.header() {
            Header::CreateLink(_) | Header::DeleteLink(_) => {
                get_link_zome(element, saf_def, cascade).await
            }
...
fn get_zome(entry_type: &AppEntryType, saf_def: &SafDef) -> AppValidationResult<Zome> {
    zome_id_to_zome(entry_type.zome_id(), saf_def)
}

fn zome_id_to_zome(zome_id: ZomeId, saf_def: &SafDef) -> AppValidationResult<Zome> {
...
async fn get_link_zome(
    ...
    match element.header() {
        Header::CreateLink(cl) => {
            let zome = zome_id_to_zome(cl.zome_id, saf_def)?;
...
async fn get_validation_package_remote(
    ...
    match entry_def.required_validation_type {
        ...
        RequiredValidationType::SubChain | RequiredValidationType::Full => {
            ...
            let mut query = aingle_zome_types::query::ChainQueryFilter::new()
...
pub async fn run_validation_callback_direct(
    ...
    let outcome = {
        ...
        get_associated_entry_def(&element, ribosome.saf_def(), conductor_api, &mut cascade).await
    };

...
use aingle_zome_types::element::Element;
...
#[instrument(skip(workspace, network, keystore, args, trigger_publish_sgd_ops))]
pub async fn call_zome_workflow<Ribosome, C>(
    workspace: HostFnWorkspace,
    network: AIngleP2pCell,
    keystore: KeystoreSender,
    args: CallZomeWorkflowArgs<Ribosome, C>,
    mut trigger_publish_sgd_ops: TriggerSender,
    ...
    C: CellConductorApiT,
{
    ...
    trigger_publish_sgd_ops.trigger();
        ...
        for chain_element in to_app_validate {
            ...
            let outcome = match chain_element.header() {
                ...
                Header::CreateLink(link_add) => {
                    let (base, target) = {
                        let mut cascade = aingle_cascade::Cascade::from_workspace_network(
                            &workspace,
                            network.clone(),
                        );
...
async fn get_zome(
    ...
    let result = app_validation_workflow::get_zomes_to_invoke(element, saf_def, &mut cascade).await;
    match result {
        Ok(zomes) => Ok(zomes),
        Err(outcome_or_err) => {
...
pub mod tests {
    ...
    use aingle_zome_types::entry::Entry;
    ...
    async fn run_call_zome<'env, Ribosome: RibosomeT + Send + Sync + 'static>(
        ...
        let cell_id = CellId::new(ribosome.saf_def().as_hash().clone(), fixt!(AgentPubKey));
    ...
    //   SGD Op that would be produced by it
    ...
    async fn creates_outputs() {
        ...
        let workspace = CallZomeWorkspace::new(env.clone().into()).unwrap();
        let mut ribosome = MockRibosomeT::new();
        let saf_def = fixt!(SafFile).saf().clone();

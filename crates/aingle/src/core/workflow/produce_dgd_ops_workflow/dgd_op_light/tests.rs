...
impl ElementTest {
    ...
    fn entry_create(mut self) -> (Element, Vec<DgdOp>) {
        ...
        let ops = vec![
            ...
            DgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            DgdOp::StoreEntry(
                self.sig.clone(),
    ...
    fn entry_update(mut self) -> (Element, Vec<DgdOp>) {
        ...
        let ops = vec![
            ...
            DgdOp::RegisterUpdatedContent(
                self.sig.clone(),
                entry_update.clone(),
                Some(self.entry.clone().into()),
            ),
            DgdOp::RegisterUpdatedElement(
                self.sig.clone(),
                entry_update,
                Some(self.entry.clone().into()),
            ),
    ...
    fn entry_delete(mut self) -> (Element, Vec<DgdOp>) {
        ...
        let ops = vec![
            DgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            DgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            DgdOp::RegisterDeletedBy(self.sig.clone(), entry_delete.clone()),
            DgdOp::RegisterDeletedEntryHeader(self.sig, entry_delete),
        ];
        (element, ops)
    }

    fn link_add(mut self) -> (Element, Vec<DgdOp>) {
        ...
        let ops = vec![
            DgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            DgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            DgdOp::RegisterAddLink(self.sig.clone(), self.link_add.clone()),
        ];
        (element, ops)
    }

    fn link_remove(mut self) -> (Element, Vec<DgdOp>) {
        ...
        let ops = vec![
            DgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            DgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            DgdOp::RegisterRemoveLink(self.sig.clone(), self.link_remove.clone()),
        ];
        (element, ops)
    }

    fn others(mut self) -> Vec<(Element, Vec<DgdOp>)> {
...
async fn test_dgd_basis() {
...

    {
        ...
        let expected_entry_hash: AnyDgdHash = original_header.entry_hash.clone().into();

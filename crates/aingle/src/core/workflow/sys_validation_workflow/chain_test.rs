...
async fn run_test(alice_cell_id: CellId, handle: ConductorHandle) {
    ...
    let op = SgdOp::RegisterAgentActivity(signature, h1.clone().into());
    ops.push((SgdOpHash::with_data_sync(&op), op));
    ...
    // Create the activity op
    let op = SgdOp::RegisterAgentActivity(signature, h2.clone().into());
    ops.push((SgdOpHash::with_data_sync(&op), op));

    // Add the ops to incoming
    incoming_sgd_ops_workflow::incoming_sgd_ops_workflow(
        &alice_env,
    ...
    // Create the activity op
    let op = SgdOp::RegisterAgentActivity(signature, h1.into());
    ops.push((SgdOpHash::with_data_sync(&op), op));
    ...
    // Create the activity op
    let op = SgdOp::RegisterAgentActivity(signature, h2.into());
    ops.push((SgdOpHash::with_data_sync(&op), op));

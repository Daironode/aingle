...
async fn sys_validation_workflow_test() {
    ...
    run_test(alice_cell_id, bob_cell_id, handle.clone(), saf_file).await;
...
async fn run_test(
    ...
    bob_links_in_a_legit_way(&bob_cell_id, &handle, &saf_file).await;
    ...
    let (bad_update_header, bad_update_entry_hash, link_add_hash) =
        bob_makes_a_large_link(&bob_cell_id, &handle, &saf_file).await;
    ...
    let bad_update_entry_hash: AnySgdHash = bad_update_entry_hash.into();
    let num_valid_ops = |txn: &Transaction| {
        let valid_ops: usize = txn
                ...
                named_params!{
                    ...
                    ":add_link": SgdOpType::RegisterAddLink,
    ...
    dodgy_bob(&bob_cell_id, &handle, &saf_file).await;
...
async fn dodgy_bob(bob_cell_id: &CellId, handle: &ConductorHandle, saf_file: &SafFile) {

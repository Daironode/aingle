...
use aingle_sqlite::error::DatabaseResult;
...
fn batch_process_entry(
    ...
    for (hash, op) in ops {
        if !op_exists_inner(txn, &hash)? {
            let op = SgdOpHashed::from_content_sync(op);
            to_pending.push(op);
        } else {
...
pub async fn incoming_sgd_ops_workflow(
    vault: &EnvWrite,
    ...
    if maybe_batch.is_some() {
        ...
        tokio::task::spawn(async move {
            while let Some(entries) = maybe_batch {
                ...
                {
                    Err(err) => {
                        tracing::error!(?err, "incoming_sgd_ops_workflow error");
                    }
                    Ok(senders) => {
...
fn needs_receipt(op: &SgdOp, from_agent: &Option<AgentPubKey>) -> bool {
    from_agent
        .as_ref()
...
async fn should_keep(op: &SgdOp) -> WorkflowResult<bool> {
...
fn op_exists_inner(txn: &rusqlite::Transaction<'_>, hash: &SgdOpHash) -> DatabaseResult<bool> {
...
pub fn op_exists(vault: &EnvWrite, hash: &SgdOpHash) -> DatabaseResult<bool> {

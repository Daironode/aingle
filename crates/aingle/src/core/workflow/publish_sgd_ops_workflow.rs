...
//       (and we'd have to reference its header  which actually exists on the SGD to make that work,
//       rather than the entry which does not exist on the SGD).
//!
//!
...
pub async fn publish_sgd_ops_workflow(
    env: EnvWrite,
    mut network: AIngleP2pCell,
) -> WorkflowResult<WorkComplete> {
    let (to_publish, hashes) =
        publish_sgd_ops_workflow_inner(env.clone().into(), network.from_agent()).await?;
...
pub async fn publish_sgd_ops_workflow_inner(
    env: EnvRead,
    agent: AgentPubKey,
) -> WorkflowResult<(HashMap<AnySgdHash, Vec<(SgdOpHash, SgdOp)>>, Vec<SgdOpHash>)> {
...
mod tests {
    ...
    async fn setup<'env>(
        ...
        env.conn()
            ...
            .with_commit_sync(|txn| {
                for _ in 0..num_hash {
                    ...
                    // Create SgdOp
                    let op = SgdOp::RegisterAddLink(sig.clone(), link_add.clone());
                    // Get the hash from the op
                    let op_hashed = SgdOpHashed::from_content_sync(op.clone());
        ...
        let filter_events = |evt: &_| match evt {
            aingle_p2p::event::AIngleP2pEvent::Publish { .. } => true,
            _ => false,
        };
        let (tx, mut recv) = tokio::sync::mpsc::channel(10);
        let test_network =
            test_network_with_events(Some(saf.clone()), Some(author.clone()), filter_events, tx)
                .await;
        ...
        for agent in agents.into_iter().skip(1) {
            AIngleP2pRef::join(&network, saf.clone(), agent)
                .await
                .unwrap();
        }
    ...
    /// Call the workflow
    async fn call_workflow(env: EnvWrite, cell_network: AIngleP2pCell) {
        publish_sgd_ops_workflow(env.clone().into(), cell_network)
            .await
            .unwrap();
    }
            ...
            let check = async move {
                ...
                fresh_reader_test!(env, |txn: Transaction| {
                    let unpublished_ops: bool = txn
                        .query_row(
                            "SELECT EXISTS(SELECT 1 FROM SgdOp WHERE last_publish_time IS NULL)",
                            [],
                            |row| row.get(0),
                        )
                        .unwrap();
    ...
    /// for a SGDOp we don't re-publish it
    #[test_case(1, 1)]
    #[test_case(1, 10)]
    #[test_case(1, 100)]
    #[test_case(10, 1)]
        ...
        tokio_helper::block_forever_on(async {
            ...
            env.conn()
                ...
                .with_commit_test(|txn| {
                    txn.execute(
                        "UPDATE SgdOp SET receipt_count = ?",
    ...
    /// There is a test to shows that SGDOps that were produced on private entries are not published.
        ...
        tokio_helper::block_forever_on(
            async {
                ...
                let expected = {
                    ...
                    let expected_op = SgdOp::RegisterAgentActivity(
                        sig.clone(),
                        entry_create_header.clone().into_content(),
                    );
                    let op_hash = SgdOpHashed::from_content_sync(expected_op.clone()).into_hash();
                    map.insert(
                        op_hash,
                        (expected_op, register_agent_activity_count.clone()),
                    );

                    let expected_op = SgdOp::StoreElement(
                        sig,
                    ...
                    let expected_op =
                        SgdOp::StoreElement(sig.clone(), entry_update_header.clone().into(), None);
                    let op_hash = SgdOpHashed::from_content_sync(expected_op.clone()).into_hash();

                    map.insert(op_hash, (expected_op, store_element_count.clone()));

                    let expected_op = SgdOp::RegisterUpdatedContent(
                        sig.clone(),
                    ...
                    let expected_op = SgdOp::RegisterUpdatedElement(
                        sig.clone(),
                        entry_update_header.clone(),
                        None,
                    );
                    ...
                    let expected_op = SgdOp::RegisterAgentActivity(sig, entry_update_header.into());
                    let op_hash = SgdOpHashed::from_content_sync(expected_op.clone()).into_hash();
                    map.insert(
                        op_hash,
                        (expected_op, register_agent_activity_count.clone()),
                                    ...
                                    for (op_hash, op) in ops {
                                        match expected.get(&op_hash) {
                                            Some((expected_op, count)) => {
                                                ...
                                                assert_eq!(sgd_hash, expected_op.sgd_basis());
                                                count.fetch_add(1, Ordering::SeqCst);
                                            }
                                            None => {
                                                if let SgdOp::StoreEntry(_, h, _) = op {

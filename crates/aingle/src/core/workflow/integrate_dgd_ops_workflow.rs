...
use super::incoming_dgd_ops_workflow::IncomingDgdOpsWorkspace;
use super::*;
...
use aingle_cascade::{error::CascadeError, integrate_single_metadata};
...
use produce_dgd_ops_workflow::dgd_op_light::error::DgdOpConvertResult;
...
pub async fn integrate_dgd_ops_workflow(
    mut workspace: IntegrateDgdOpsWorkspace,
    ...
    loop {
        ...
        for so in sorted_ops.into_sorted_vec() {
            ...
            let outcome = integrate_single_dgd_op(value.clone(), op, &mut workspace).await?;
            match outcome {
                Outcome::Integrated(integrated) => {
                    ...
                    // Currently you need to check the IntegratedDgdOpsValue for
                    // the status
...
/// Integrate a single DgdOp to the stores based on the
/// validation status.
///
/// Check for dependencies in any of our other stores.
#[instrument(skip(iv, workspace))]
async fn integrate_single_dgd_op(
    iv: IntegrationLimboValue,
    op: DgdOp,
    workspace: &mut IntegrateDgdOpsWorkspace,
    ...
    if op_dependencies_held(&op, workspace).await? {
        match iv.validation_status {
            ...
            ValidationStatus::Abandoned => {
                ...
                let integrated = IntegratedDgdOpsValue {
...
fn update_activity_status(
    ...
    if let DgdOp::RegisterAgentActivity(_, h) = &op {
...
fn update_validation_status(
    ...
    match op {
        DgdOp::StoreElement(_, h, _) => meta_integrated.register_rejected_element_header(h)?,
        DgdOp::StoreEntry(_, h, _) => meta_integrated.register_rejected_header(h.clone())?,
        _ => {}
    }
    Ok(())
}
...
async fn op_dependencies_held(
    ...
    {
        match op {
            DgdOp::StoreElement(_, _, _) | DgdOp::StoreEntry(_, _, _) => {}
            DgdOp::RegisterAgentActivity(_, header) => {
                // RegisterAgentActivity is the exception where we need to make
                // sure that we have integrated the previous RegisterAgentActivity DgdOp
                // from the same chain.
                ...
                if let Some(prev_header_hash) = prev_header_hash.cloned() {
                    ...
                    {
                        Some(prev_header) => {
                            let op_hash = DgdOpHash::with_data_sync(
            ...
            DgdOp::RegisterUpdatedContent(_, entry_update, _)
            | DgdOp::RegisterUpdatedElement(_, entry_update, _) => {
            ...
            DgdOp::RegisterDeletedBy(_, element_delete)
            | DgdOp::RegisterDeletedEntryHeader(_, element_delete) => {
            ...
            DgdOp::RegisterAddLink(_, create_link) => {
            ...
            DgdOp::RegisterRemoveLink(_, delete_link) => {
...
/// Store a DgdOp's data in an element buf
pub fn integrate_single_data<P: PrefixType>(
    ...
    {
        match op {
            DgdOp::StoreElement(signature, header, maybe_entry) => {
                put_data(signature, header, maybe_entry.map(|e| *e), element_store)?;
            }
            DgdOp::StoreEntry(signature, new_entry_header, entry) => {
                put_data(
                    signature,
            ...
            DgdOp::RegisterUpdatedContent(signature, entry_update, _)
            | DgdOp::RegisterUpdatedElement(signature, entry_update, _) => {
...
/// The outcome of integrating a single DgdOp: either it was, or it wasn't
enum Outcome {
    Integrated(IntegratedDgdOpsValue),
    Deferred(DgdOp),
}

pub struct IntegrateDgdOpsWorkspace {
    /// integration queue
    ...
    /// Ops to disintegrate
    pub to_disintegrate_pending: Vec<DgdOpLight>,
    /// READ ONLY
...
impl Workspace for IntegrateDgdOpsWorkspace {
    fn flush_to_txn_ref(&mut self, writer: &mut Writer) -> WorkspaceResult<()> {
        ...
        // flush integrated
        self.integrated_dgd_ops.flush_to_txn_ref(writer)?;
...
impl IntegrateDgdOpsWorkspace {
    ...
    fn integrate(&mut self, hash: DgdOpHash, v: IntegratedDgdOpsValue) -> DgdOpConvertResult<()> {
        ...
        self.integrated_dgd_ops.put(hash, v)?;
        Ok(())
    }

    pub fn op_exists(&self, hash: &DgdOpHash) -> DatabaseResult<bool> {
        Ok(self.integrated_dgd_ops.contains(&hash)? || self.integration_limbo.contains(&hash)?)
    }
...
pub fn dump_state(env: EnvironmentRead) -> WorkspaceResult<IntegrationStateDump> {
    let workspace = IncomingDgdOpsWorkspace::new(env.clone())?;
    let (validation_limbo, integration_limbo, integrated) = fresh_reader!(env, |r| {
        ...
        let i = workspace.integrated_dgd_ops.iter(&r)?.count()?;

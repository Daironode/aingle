...
impl Outcome {
    ...
    pub fn awaiting<E, I: Into<AnySgdHash> + Clone>(h: &I) -> OutcomeOrError<Self, E> {
        OutcomeOrError::Outcome(Outcome::AwaitingDeps(vec![h.clone().into()]))
    }
    /// Exit early with an awaiting outcome
    pub fn exit_with_awaiting<T, I: Into<AnySgdHash>, It: IntoIterator<Item = I>>(
        h: It,

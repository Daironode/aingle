...
use aingle_state::prelude::fresh_reader_test;
...
fn show_limbo(txn: &Transaction) -> Vec<SgdOpLight> {
    txn.prepare("SELECT blob FROM SgdOp WHERE when_integrated IS NULL")
        .unwrap()
        .query_and_then([], |row| from_blob(row.get("blob")?))
        .unwrap()
        .collect::<StateQueryResult<Vec<SgdOpLight>>>()
        .unwrap()
}

fn num_valid(txn: &Transaction) -> usize {
    txn
    .query_row("SELECT COUNT(hash) FROM SgdOP WHERE when_integrated IS NOT NULL AND validation_status = :status", 
            named_params!{
...
async fn run_test(
    ...
    aingle_state::prelude::dump_tmp(&alice_env);
    ...
    let (invalid_header_hash, invalid_entry_hash) =
        commit_invalid(&bob_cell_id, &handle, saf_file).await;
    let invalid_entry_hash: AnySgdHash = invalid_entry_hash.into();
    ...
    let invalid_link_hash: HeaderHash =
        call_zome_directly(&bob_cell_id, &handle, saf_file, invocation)
            .await
            .decode()
            .unwrap();
    ...
    .unwrap();
    call_zome_directly(&bob_cell_id, &handle, saf_file, invocation).await;
...
async fn run_test_entry_def_id(
    ...
    let (invalid_header_hash, invalid_entry_hash) =
        commit_invalid_post(&bob_cell_id, &handle, saf_file).await;
    let invalid_entry_hash: AnySgdHash = invalid_entry_hash.into();
...
async fn commit_invalid(
    ...
    let call_data = HostFnCaller::create(bob_cell_id, handle, saf_file).await;
    // 4
    ...
    triggers.publish_sgd_ops.trigger();
...
async fn commit_invalid_post(
    ...
    let call_data = HostFnCaller::create_for_zome(bob_cell_id, handle, saf_file, 2).await;
    // 9
    let invalid_header_hash = call_data

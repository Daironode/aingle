...
/// Op data that will be ordered by [SgdOpOrder]
#[derive(Debug, Clone)]
pub struct OrderedOp<V> {
    pub order: SgdOpOrder,
    pub hash: SgdOpHash,
    pub op: SgdOp,
    pub value: V,
}
...
impl From<&SgdOp> for SgdOpOrder {
    fn from(op: &SgdOp) -> Self {
        use SgdOpOrder::*;
        match op {
            SgdOp::StoreElement(_, h, _) => StoreElement(h.timestamp()),
            SgdOp::StoreEntry(_, h, _) => StoreEntry(*h.timestamp()),
            SgdOp::RegisterAgentActivity(_, h) => RegisterAgentActivity(h.timestamp()),
            SgdOp::RegisterUpdatedContent(_, h, _) => RegisterUpdatedContent(h.timestamp),
            SgdOp::RegisterUpdatedElement(_, h, _) => RegisterUpdatedElement(h.timestamp),
            SgdOp::RegisterDeletedBy(_, h) => RegisterDeletedBy(h.timestamp),
            SgdOp::RegisterDeletedEntryHeader(_, h) => RegisterDeletedEntryHeader(h.timestamp),
            SgdOp::RegisterAddLink(_, h) => RegisterAddLink(h.timestamp),
            SgdOp::RegisterRemoveLink(_, h) => RegisterRemoveLink(h.timestamp),
        }
    }
}

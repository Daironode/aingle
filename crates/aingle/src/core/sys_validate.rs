...
use super::workflow::incoming_sgd_ops_workflow::incoming_sgd_ops_workflow;
...
pub(super) use error::*;
pub use ai_hash::*;
pub use aingle_state::source_chain::SourceChainError;
pub use aingle_state::source_chain::SourceChainResult;
pub use aingle_types::Timestamp;
pub use aingle_zome_types::HeaderHashed;
...
/// for this header.
/// If not Saf then cannot be root of chain
...
/// Check that Saf headers are only added to empty source chains
pub async fn check_valid_if_saf(
    header: &Header,
...
pub async fn check_app_entry_type(
    ...
    let saf_file = conductor_api.get_this_saf().await.map_err(Box::new)?;
    ...
    let entry_def = get_entry_def(entry_type.id(), zome, saf_file.saf(), conductor_api).await?;
...
/// retrieve it and send it as a RegisterAddLink SgdOp
/// to our incoming_sgd_ops_workflow.
///
    ...
    F: FnOnce(&Element) -> SysValidationResult<()>,
{
    ...
    f(source.as_ref())?;
    if let (Some(incoming_sgd_ops_sender), Source::Network(element)) =
        (incoming_sgd_ops_sender, source)
    {
...
/// If we are not holding this header then
/// retrieve it and send it as a RegisterAgentActivity SgdOp
/// to our incoming_sgd_ops_workflow.
///
/// Apply a checks callback to the Element.
///
...
/// If we are not holding this header then
/// retrieve it and send it as a StoreEntry SgdOp
/// to our incoming_sgd_ops_workflow.
///
/// Apply a checks callback to the Element.
///
    ...
    F: FnOnce(&Element) -> SysValidationResult<()>,
{
    ...
    if let (Some(incoming_sgd_ops_sender), Source::Network(element)) =
        (incoming_sgd_ops_sender, source)
    {
        incoming_sgd_ops_sender.send_store_entry(element).await?;
    }
    Ok(())
}
...
/// and send it as a StoreEntry SgdOp
/// to our incoming_sgd_ops_workflow.
///
...
/// If we are not holding this header then
/// retrieve it and send it as a StoreElement SgdOp
/// to our incoming_sgd_ops_workflow.
///
/// Apply a checks callback to the Element.
///
    ...
    F: FnOnce(&Element) -> SysValidationResult<()>,
{
    ...
    if let (Some(incoming_sgd_ops_sender), Source::Network(element)) =
        (incoming_sgd_ops_sender, source)
    {
        incoming_sgd_ops_sender.send_store_element(element).await?;
    }
    Ok(())
}

/// Allows you to send an op to the
/// incoming_sgd_ops_workflow if you
/// found it on the network and were supposed
/// to be holding it.
#[derive(derive_more::Constructor)]
pub struct IncomingSgdOpSender {
    env: EnvWrite,
    sys_validation_trigger: TriggerSender,
}

impl IncomingSgdOpSender {
    /// Sends the op to the incoming workflow
    async fn send_op(
        self,
        element: Element,
        make_op: fn(Element) -> Option<(SgdOpHash, SgdOp)>,
    ) -> SysValidationResult<()> {
        if let Some(op) = make_op(element) {
            let ops = vec![op];
            incoming_sgd_ops_workflow(&self.env, self.sys_validation_trigger, ops, None, false)
                .await
                .map_err(Box::new)?;
        }
        Ok(())
...
async fn check_and_hold<I: Into<AnySgdHash> + Clone>(
    hash: &I,
    workspace: &mut SysValidationWorkspace,
    network: AIngleP2pCell,
) -> SysValidationResult<Source> {
    let hash: AnySgdHash = hash.clone().into();
...
/// Make a StoreElement SgdOp from an Element.
...
/// to return an error.
fn make_store_element(element: Element) -> Option<(SgdOpHash, SgdOp)> {
    // Extract the data
    ...
    let op = SgdOp::StoreElement(signature, header, maybe_entry_box);
    let hash = SgdOpHash::with_data_sync(&op);
    Some((hash, op))
}

/// Make a StoreEntry SgdOp from an Element.
...
/// is only faster then waiting for them through gossip we don't care enough
/// to return an error.
fn make_store_entry(element: Element) -> Option<(SgdOpHash, SgdOp)> {
    // Extract the data
    let (shh, element_entry) = element.into_inner();
    let (header, signature) = shh.into_header_and_signature();
    ...
    // Create the hash and op
    let op = SgdOp::StoreEntry(signature, header, entry_box);
    let hash = SgdOpHash::with_data_sync(&op);
    Some((hash, op))
}

/// Make a RegisterAddLink SgdOp from an Element.
/// Note that this can fail if the header is the wrong type
///
/// Because adding ops to incoming limbo while we are checking them
/// is only faster then waiting for them through gossip we don't care enough
/// to return an error.
fn make_register_add_link(element: Element) -> Option<(SgdOpHash, SgdOp)> {
    // Extract the data
    ...
    // Create the hash and op
    let op = SgdOp::RegisterAddLink(signature, header);
    let hash = SgdOpHash::with_data_sync(&op);
    Some((hash, op))
}

/// Make a RegisterAgentActivity SgdOp from an Element.
/// Note that this can fail if the header is the wrong type
///
/// Because adding ops to incoming limbo while we are checking them
/// is only faster then waiting for them through gossip we don't care enough
/// to return an error.
fn make_register_agent_activity(element: Element) -> Option<(SgdOpHash, SgdOp)> {
    // Extract the data
    let (shh, _) = element.into_inner();
    let (header, signature) = shh.into_header_and_signature();
    ...
    // Create the hash and op
    let op = SgdOp::RegisterAgentActivity(signature, header);
    let hash = SgdOpHash::with_data_sync(&op);
    Some((hash, op))
}

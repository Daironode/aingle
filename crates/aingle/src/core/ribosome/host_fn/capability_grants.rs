...
pub mod wasm_test {
    ...
    // TODO: [ B-03669 ] can move this to an integration test (may need to switch to using a RealSafStore)
    #[tokio::test(flavor = "multi_thread")]
    async fn ribosome_authorized_call() -> anyhow::Result<()> {
        observability::test_run().ok();
        let (saf_file, _) = SweetSafFile::unique_from_test_wasms(vec![TestWasm::Capability])
            .await
            .unwrap();
        ...
        saf_store
            .expect_get()
            .return_const(Some(saf_file.clone().into()));
        saf_store.expect_add_saf().return_const(());
        saf_store.expect_add_safs::<Vec<_>>().return_const(());
        saf_store.expect_add_entry_defs::<Vec<_>>().return_const(());

        let mut conductor =
            SweetConductor::from_builder(ConductorBuilder::with_mock_saf_store(saf_store)).await;

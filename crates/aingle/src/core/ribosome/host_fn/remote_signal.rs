...
use aingle_zome_types::signal::RemoteSignal;
...
mod tests {
    ...
    async fn remote_signal_test() -> anyhow::Result<()> {
        ...
        let (saf_file, _) = SweetSafFile::unique_from_inline_zome(
            "zome1",
        ...
        let apps = conductors
            .setup_app_for_zipped_agents("app", &agents, &[saf_file.clone().into()])
            .await
            .unwrap();

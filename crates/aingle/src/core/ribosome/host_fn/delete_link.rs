...
pub fn delete_link<'a>(
    ...
    let maybe_add_link: Option<SignedHeaderHashed> = tokio_helper::block_forever_on(async move {
        ...
        CascadeResult::Ok(
            Cascade::from_workspace_network(workspace, network)
                .sgd_get(address.into(), GetOptions::content())
                .await?
    ...
    let base_address = match maybe_add_link {
        ...
        // that isn't also discoverable in either the cache or SGD, but it _is_ possible so we have
...
pub mod slow_tests {
    ...
    use aingle_zome_types::link::Links;

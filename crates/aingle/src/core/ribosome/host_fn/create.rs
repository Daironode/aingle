...
pub mod wasm_test {
    ...
    async fn ribosome_create_entry_test<'a>() {
        ...
        let bytes: Vec<u8> = match round.and_then(|el| el.into()) {
            Some(aingle_zome_types::entry::Entry::App(entry_bytes)) => {
    ...
    async fn test_serialize_bytes_hash() {
        ...
        // Now I can convert to AnySgdHash
        let any_hash: AnySgdHash = entry_hash.clone().into();
        assert_eq!(
            "uhCEkPjYXxw4ztKx3wBsxzm-q3Rfoy1bXWbIQohifqC3_HNle3-SO",
            &entry_hash.to_string()
        );
        ...
        let any_hash: AnySgdHash = sb.try_into().unwrap();
        assert_eq!(
            "uhCEkPjYXxw4ztKx3wBsxzm-q3Rfoy1bXWbIQohifqC3_HNle3-SO",
            &any_hash.to_string()
        );
        ...
        let any_hash: AnySgdHash = entry_hash.clone().try_into().unwrap();
        assert_eq!(
            "uhCEkPjYXxw4ztKx3wBsxzm-q3Rfoy1bXWbIQohifqC3_HNle3-SO",
            &any_hash.to_string()
        );

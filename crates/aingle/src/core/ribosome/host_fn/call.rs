...
pub mod wasm_test {
    ...
    async fn call_the_same_cell() {
        ...
        let has_hash: bool = fresh_reader_test(alice_call_data.env.clone(), |txn| {
            txn.query_row(
                "SELECT EXISTS(SELECT 1 FROM SgdOp WHERE header_hash = :hash AND is_authored = 1)",
                named_params! {
                    ":hash": header_hash
                },
                |row| row.get(0),
    ...
    async fn bridge_call() {
        ...
        // Install a different saf for bob
        let zomes = vec![TestWasm::WhoAmI];
        let bob_cell_id = install_new_app("bobs_saf", zomes, &handle).await;
    ...
    async fn install_new_app(
        ...
        install_app("bob_app", cell_data, vec![saf_file], handle.clone()).await;
        bob_cell_id
    }
}

...
pub mod wasm_test {
    ...
    async fn hash_entry_test() {
        ...
        assert_eq!(*output.hash_type(), ai_hash::hash_type::Entry);
    }
    ...
    async fn ribosome_hash_path_pwd_test() {
        ...
        let expected_path = adk::hash_path::path::Path::from("foo.bar");

        let expected_hash = aingle_types::entry::EntryHashed::from_content_sync(

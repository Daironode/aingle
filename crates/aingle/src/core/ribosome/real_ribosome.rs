...
impl RealRibosome {
    ...
    pub fn module(&self, zome_name: &ZomeName) -> RibosomeResult<Module> {
        ...
        Ok(aingle_wasmer_host::instantiate::module(
    ...
    pub fn instance(&self, call_context: CallContext) -> RibosomeResult<Instance> {
        ...
        Ok(aingle_wasmer_host::instantiate::instantiate(
            self.wasm_cache_key(&zome_name)?,
            &wasm,
            &imports,
            std::env::var_os(WASM_CACHE_PATH_ENV),
    ...
    fn imports(&self, call_context: CallContext) -> ImportObject {
        ...
        macro_rules! invoke_host_function {
            ( $host_function:ident ) => {{
                ...
                move |ctx: &mut Ctx, guest_allocation_ptr: GuestPtr| -> Result<Len, WasmError> {
                    let result = match $crate::aingle_wasmer_host::guest::from_guest_ptr(
                        ctx,
                    ...
                    $crate::aingle_wasmer_host::import::set_context_data(ctx, result)
                }
            }};
        }
        ...
        // standard memory handling used by the aingle_wasmer guest and host macros
        ns.insert(
            "__import_data",
            func!(aingle_wasmer_host::import::__import_data),
        );
        ...
        match zome.zome_def() {
            ZomeDef::Wasm(_) => {
                ...
                if module.info().exports.contains_key(to_call.as_ref()) {
                    ...
                    let result: Result<ExternIO, WasmError> = aingle_wasmer_host::guest::call(
                        &mut instance,
                        to_call.as_ref(),

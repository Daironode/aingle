...
but we have created conventions and macros that makes working with aingle as
...
## How aingle uses wasm

The aingle binary acts as both a wasm host and a websockets server that
...
websockets and the aingle host can call `extern` functions itself to
provide "hook" style extension points for developers.

For example, when a aingle "SAF" is first installed it will reference one or
several wasm files. If any of these wasm files defines a function called `init`
then aingle will call each of the `init` functions one after the other in the
order they are referenced in the SAF config files.

Defining an extern function for aingle in Rust is straightforward:
...
- and `RemotePointer` arg/return values from the `aingle-wasmer-guest` crate.
...
examples that demonstrates working with aingle-friendly wasm _without_ a ADK.

### Example: Minimal wasm that will run

All the core externs required for aingle to interact with this wasm are
defined by the `aingle_externs!()` macro.

Every callback is optional so defining the core externs is enough to compile
the wasm and have aingle install and run it.

```rust
aingle_wasmer_guest::aingle_externs!();
```
...
- the `aingle_externs!()` macro as above
...
`aingle_middleware_bytes` crate. Anything that can be serialized can be
...
All the callback input and output values are defined in `aingle_zome_types`.

```rust
use aingle_wasmer_guest::*;
...
The aingle host would return a messagepack-serialized representation of this
...
`SerializedBytes` from the `aingle_middleware_bytes` crate. This means
...
In this case the custom `HelloWorld` struct will be ignored by the aingle
...
Entries must be validated by both the aingle host and the guest wasm before
they are finalized on the local source chain or broadcast to the SGD network.
...
interact with aingle and the outside world, from `init` through to an extern
...
![aingle wasm flow](https://thedavidmeister.keybase.pub/aingle/docs/adk/images/wasm-flow.jpg)

So here is the wasm code for all that.

It consists of a few components, some new and some already demonstrated above:

- `aingle_externs!()` to enable the aingle host to run the wasm
...
pub extern "C" fn save_image(remote_ptr: GuestPtr) -> GuestPtr {
 ...
 let _: CreateOutput = host_call(
  // note that all host functions from aingle start with prefix `__`
  __create,
  CreateInput::new(
   Entry::App(
    // this serializes the image into an Entry enum that the aingle host
...
pub extern "C" fn validate_entry(remote_ptr: GuestPtr) -> GuestPtr {
 ...
 ret!(ExternOutput::new(
  ...
  match Entry::try_from(input.into_inner()) {
   ...
   // entry we are asked to validate incoming from the sgd
...
- It opens the potential for malicious actors on the SGD to force your wasm to
...
If you read the `aingle_middleware_bytes` documentation linked above, or have
...
To fill the `Result`/`?` gap we have two macros in `aingle_wasmer_guest`.
...
pub extern "C" fn commit_message(remote_ptr: GuestPtr) -> GuestPtr {
 ...
 // handles memory and serialization logic for the aingle host
...
At this point you should start to have a clear understanding of what the ADK is
doing under the hood and to decide for yourself whether you really need or want
the sugar that it provides.

The ADK macros simply expand to this extern boilerplate, saving you from typing
...
Think of the ADK as a tool and safety net but also don't feel you can't peek

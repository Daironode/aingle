...
pub fn create_x25519_keypair() -> ExternResult<X25519PubKey> {
    ADK.with(|h| h.borrow().create_x25519_keypair(()))
}
...
///  - Consider using capability tokens and/or dedicated SGD networks to control access.
...
///  - Consider that encrypted data cannot be validated effectively by the public SGD.
///

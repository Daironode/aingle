...
use aingle_zome_types::link::LinkTag;
use std::str::FromStr;
...
/// is hashed to create something that can be linked and discovered by all SGD participants.
...
/// At this point it is possible to follow SGD links from the root back up the path.
/// i.e. the ahead-of-time predictability of the hashes of a given path allows us to travel "up"
/// the tree and the linking functionality of the aingle SGD allows us to travel "down" the tree
/// after at least one SGD participant has followed the path "up".
#[derive(
...
/// layer so there is a risk that there are hash collisions with other data on the SGD network if
...
impl Path {
    ...
    pub fn hash(&self) -> ExternResult<ai_hash::EntryHash> {
    ...
    pub fn children(&self) -> ExternResult<aingle_zome_types::link::Links> {
        Self::ensure(&self)?;
        let links = get_links(
            self.hash()?,
            Some(aingle_zome_types::link::LinkTag::new(NAME)),
        )?;
        // Only need one of each hash to build the tree.
        let mut unwrapped: Vec<aingle_zome_types::link::Link> = links.into_inner();
        unwrapped.sort_unstable_by(|a, b| a.tag.cmp(&b.tag));
        unwrapped.dedup_by(|a, b| a.tag.eq(&b.tag));
        Ok(aingle_zome_types::link::Links::from(unwrapped))
    }

    pub fn children_details(&self) -> ExternResult<aingle_zome_types::link::LinkDetails> {
        Self::ensure(&self)?;
        get_link_details(
            self.hash()?,
            Some(aingle_zome_types::link::LinkTag::new(NAME)),
...
fn hash_path_linktag() {
    assert_eq!("adk.path".as_bytes(), NAME);

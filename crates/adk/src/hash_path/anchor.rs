...
///
/// The string "adkanchor".
pub const ROOT: &str = "adkanchor";
...
pub fn anchor(anchor_type: String, anchor_text: String) -> ExternResult<ai_hash::EntryHash> {
...
/// Old version of aingle that anchors was designed for had two part link tags but now link
...
fn hash_path_anchor_path() {
    for (atype, text, path_string) in vec![
        ("foo", None, "adkanchor.foo"),
        ("foo", Some("bar".to_string()), "adkanchor.foo.bar"),
    ] {
        assert_eq!(

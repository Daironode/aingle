...
/// The entry value only needs to exist on the SGD for the link to validate, it doesn't need to be
...
pub fn delete_link(add_link_header: HeaderHash) -> ExternResult<HeaderHash> {
    ADK.with(|h| h.borrow().delete_link(add_link_header))
}
...
pub fn get_links(base: EntryHash, link_tag: Option<LinkTag>) -> ExternResult<Links> {
    ADK.with(|h| h.borrow().get_links(GetLinksInput::new(base, link_tag)))
}

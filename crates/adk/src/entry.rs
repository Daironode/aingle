...
pub fn create(entry_with_def_id: EntryWithDefId) -> ExternResult<HeaderHash> {
    ADK.with(|h| h.borrow().create(entry_with_def_id))
}
...
pub fn update(hash: HeaderHash, entry_with_def_id: EntryWithDefId) -> ExternResult<HeaderHash> {
    ADK.with(|h| h.borrow().update(UpdateInput::new(hash, entry_with_def_id)))
}
...
pub fn delete(hash: HeaderHash) -> ExternResult<HeaderHash> {
    ADK.with(|h| h.borrow().delete(hash))
}
...
/// The default impls from the `#[adk_entry( .. )]` and [ `entry_def!` ] macros include this.
///
...
///
/// Hashes are typed in aingle, e.g. [ `HeaderHash` ] and [ `EntryHash` ] are different and yield different
/// bytes for a given value. This ensures correctness and allows type based dispatch in various
/// areas of the codebase.
///
/// Usually you want to hash a value that you want to reference on the SGD with [ `get` ] etc. because
...
/// also be used with [ `get` ] etc. to retreive a _specific_ element from the SGD rather than the
/// oldest live element.
...
///
/// Anything that is annotated with #[adk_entry( .. )] or entry_def!( .. ) implements this so is
/// compatible automatically.
///
/// [ `hash_entry` ] is "dumb" in that it doesn't check that the entry is defined, committed, on the SGD or
/// any other validation, it simply generates the hash for the serialized representation of
/// something in the same way that the SGD would.
///
...
where
    ...
    WasmError: From<E>,
{
    ADK.with(|h| h.borrow().hash_entry(Entry::try_from(input)?))
}
    ...
    AnySgdHash: From<H>,
{
    ADK.with(|h| {
        h.borrow()
            .get(GetInput::new(AnySgdHash::from(hash), options))
    })
}
...
///       e.g. the SAF itself, links, migrations, etc.
...
pub fn get_details<H: Into<AnySgdHash>>(
    hash: H,
    options: GetOptions,
) -> ExternResult<Option<Details>> {
    ADK.with(|h| h.borrow().get_details(GetInput::new(hash.into(), options)))
}
...
///
/// ```ignore
/// #[adk_entry(id = "foo", visibility = "private", required_validations = 6, )]
/// pub struct Foo;
/// ```
///
/// ```ignore
...
/// The [ `EntryDefIndex` ] is used to reference the entry type in headers on the SGD and as the index of the type exported to tooling.
...
/// It is a macro so that we can call a function with a known name `crate::entry_defs` from the ADK before the function is defined.
///

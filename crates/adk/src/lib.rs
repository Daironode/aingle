...
//!
//! The ADK exists to make working with WASM in aingle much easier.
//!
//! Hopefully you don't notice the WASMness at all and it just feels like Rust 🦀
//!
//! Note: From the perspective of happ development in WASM the 'guest' is the WASM and the 'host' is the running aingle conductor.
//! The host is _not_ the 'host operating system' in this context.
//!
//! ### ADK has layers 🧅
//!
//! ADK is designed in layers so that there is some kind of 80/20 rule.
//! The code is not strictly organised this way but you'll get a feel for it as you write your own happs.
//!
//! Roughly speaking, 80% of your apps can be production ready using just 20% of the ADK features and code.
//! These are the 'high level' functions such as [ `crate::entry::create_entry` ] and macros like [ `#[adk_extern]` ].
//! Every aingle function is available with a typed and documented wrapper and there is a set of macros for exposing functions and defining entries.
//!
...
//!
//! If you need to go deeper still, the next layer is the `aingle_wasmer_guest`, `aingle_zome_types` and `aingle_serialization` crates.
...
//!
//! ### ADK should be pinned 📌
//!
//! The basic functionality of the ADK is to communicate with the aingle conductor using a specific typed interface.
//!
...
//! ADK references these crates with `=x.y.z` syntax in Cargo.toml to be explicit about this.
//!
//! ADK itself has a slower release cycle than the aingle conductor by design to make it easier to pin and track changes.
//!
//! You should pin your dependency on ADK using the `=x.y.z` syntax too!
//!
//! You do _not_ need to pin _all_ your Rust dependencies, just those that take part in defining the host/guest interface.
//!
//! ### ADK has many simple example zomes 🍭
//!
//! The ADK is used in all the wasms used to test aingle itself.
...
//!
//! At the time of writing there were about 40 example/test wasms that can be browsed [on github](https://github.com/AIngleLab/aingle/tree/develop/crates/test_utils/wasm/wasm_workspace).
//!
//! Each example wasm is a minimal demonstration of specific ADK functionality, such as generating random data, creating entries or defining validation callbacks.
...
//! ### ADK code structure 🧱
//!
//! ADK implements several key features:
//!
//! - Base ADKT trait for standardisation, mocking, unit testing support: adk module
//! - Capabilities and function level access control: capability module
//! - Application data and entry definitions for the source chain and SGD: entry module and `entry_defs` callback
//! - Referencing/linking entries on the SGD together into a graph structure: link module
...
//! - Exposing functions to external processes and callbacks to the host: `#[adk_extern]` and `map_extern!` macros
...
//!
//! The latter are all defined by the aingle conductor and implemented by ADK for you, but the former need to all be defined by your application.
//! Any wasm that does _not_ use the ADK will need to define placeholders for and the interface to the host functions.
//!
//! All host functions can be called directly as:
//!
//! ```ignore
//! use crate::prelude::*;
//! let _output: ADK.with(|h| h.borrow().host_fn(input));
//! ```
//!
//! And every host function defined by aingle has a convenience wrapper in ADK that does the type juggling for you.
//!
//! To extend a Rust function so that it can be called by the host, add the `#[adk_extern]` attribute.
//!
...
//!
//! Some externs function as callbacks the host will call at key points in aingle internal system workflows.
...
//!
//! AIngle will merge multiple callback results for the same callback in a context sensitive manner. For example, the host will consider initialization failed if _any_ init callback fails.
//!
//! The callbacks are:
//!
...
//!   - The `#[adk_entry]` attribute simplifies generating entry definitions for a struct or enum.
//!   - The `entry_def_index!` macro converts a def id like "post" to an `EntryDefIndex` by calling this callback _inside the guest_.
//!   - All zomes in a SAF define all their entries at the same time for the host
//!   - All entry defs are combined into a single ordered list per zone and exposed to tooling such as SAF generation
//!   - Entry defs are referenced by `u8` numerical position externally and in SGD headers and by id/name e.g. "post" in sparse callbacks
//! - `fn init(_: ()) -> ExternResult<InitCallbackResult>`:
//!   - Allows the guest to pass/fail/retry initialization with `InitCallbackResult`
//!   - All zomes in a SAF init at the same time
//!   - Any failure fails initialization for the SAF, any retry (missing dependencies) causes the SAF to retry
//!   - Failure overrides retry
//! - `fn migrate_agent_{{ open|close }} -> ExternResult<MigrateAgentCallbackResult>`:
//!   - Allows the guest to pass/fail a migration attempt to/from another SAF
...
//!   - All zomes in a SAF migrate at the same time
...
//!   - Only the originating zome is called
//!
//! ### ADK is atomic on the source chain ⚛
//!
...
//!
//! There are no such guarantees for other side effects. Notably we cannot control anything over the network or outside the aingle database.
//!
...
//!
//! ### ADK is integrated with rust tracing for better debugging 🐛
//!
//! Every extern defined with the `#[adk_extern]` attribute registers a [tracing subscriber](https://crates.io/crates/tracing-subscriber) that works in WASM.
//!
...
//!
//! WASM tracing can be filtered at runtime using the `WASM_LOG` environment variable that works exactly as `RUST_LOG` does for the aingle conductor and other Rust binaries.
//!
...
//!
//! ### ADK requires explicit error handling between the guest and host ⚠
//!
...
///
/// Every exposed function in aingle uses capability grants/claims to secure access.
///
...
/// - AIngle compares capability secrets using constant time equality checks to mitigate timing attacks
...
/// Don't store them to the SGD in plaintext, or commit them to github repositories, etc!
///
...
///
/// Most aingle applications will define their own app entry types.
///
...
///
/// CRUD in aingle is represented as a graph/tree of Elements referencing each other (via Header hashes) representing new states of a shared identity.
...
/// This is a key difference between aingle and blockchains.
/// Where blockchains define a consensus algorithm that brings all participants as close as possible to a single value while aingle lets each participant discover their own truth.
///
...
///
/// In aingle terms, blockchain consensus is walking all the known 'updates' (blocks) that pass validation then walking/reducing down them to disover the 'chain with the most work' or similar.
/// For example, to implement a blockchain in aingle, attach a proof of work to each update and then follow the updates with the most work to the end.
///
...
/// Distributed Hash Tables (SGDs) are fundamentally all key/value stores (content addressable).
///
...
/// - AIngle neighbourhoods (who needs to hold the data) center around the content address so the poor nodes closest to "chat-messages" will be forced to hold _all_ messages (SGD hotspots)
///
...
/// If the tree cannot embed meaningful granularity, for example maybe the only option is to build a tree based on the binary representation of the hash of the content, then we solve SGD hotspots but our applications will have no way to narrow down polling, other than to brute force the tree.
///
...
/// - We avoid SGD hotspots because each branch of the tree has its own hash and set of links, therefore a different neighbourhood of agents
///
...
/// Maps a Rust function to an extern that WASM can expose to the AIngle host.
///
/// Annotate any compatible function with `#[adk_extern]` to expose it to AIngle as a WASM extern.
/// The [ `map_extern!` ] macro is used internally by the `#[adk_extern]` attribute.
///
...
///
/// This module only defines macros so check the ADK crate root to see more documentation.
///
/// A _new_ extern function is created with the same name as the function with the `#[adk_extern]` attribute.
...
/// - Setting the ADK wasm tracing subscriber as the global default
/// - Deserializing the input from the host
/// - Calling the function annotated with `#[adk_extern]`
...
/// In theory the keypairs could also be translated to move between the two algorithms but aingle doesn't offer a way to do this (yet?).
...
///
/// Interacting with a source chain is very different to the SGD.
///
...
/// - Source chains contain both private (local only) and public (broadcast to SGD) elements
///
...
/// Request contextual information from the aingle host.
///
/// The aingle host has additional runtime context that the wasm may find useful and cannot produce for itself including:
///
/// - The calling agent
/// - The current app (bundle of SAFs)
/// - The current SAF
/// - The current Zome
/// - The function call itself
pub mod info;

/// Links in aingle are analogous to a join table in a traditional SQL schema.
///
...
///
/// Links are retrived from the SGD by performing [ `link::get_links` ] or [ `link::get_link_details` ] against the _base_ of a link.
///
...
/// Methods for interacting with peers in the same SGD network.
///
/// Data on the SGD generally propagates at the speed of gossip and must be explicitly polled and retrieved.
///
...
/// Signals simply forward information about the introduction of new data on the SGD so that agents can push updates to each other rather than relying purely on polling.
///
/// @todo introduce a pubsub mechanism
pub mod p2p;

/// Integrates ADK with the Rust tracing crate.
///
/// The functions and structs in this module do _not_ need to be used directly.
/// The `#[adk_extern]` attribute on functions exposed externally all set the `WasmSubscriber` as the global default.
///
...
/// When the `mock` feature is set then this will default to an ADK that always errors, else a wasm host is assumed to exist.

...
/// host environment offers and the crypto implementation within aingle.
///
...
pub fn random_bytes(number_of_bytes: u32) -> ExternResult<Bytes> {
    ADK.with(|h| h.borrow().random_bytes(number_of_bytes))
}
...
impl_try_from_random!(
    CapSecret,
    aingle_zome_types::capability::CAP_SECRET_BYTES
);
...
impl_try_from_random!(
    SecretBoxKeyRef,
    aingle_zome_types::x_salsa20_poly1305::key_ref::KEY_REF_BYTES
);

...
{
    ADK.with(|h| h.borrow().sign(Sign::new(key.into(), data)?))
}
...
where
    K: Into<AgentPubKey>,
{
    ADK.with(|h| h.borrow().sign(Sign::new_raw(key.into(), data)))
}
...
where
    D: serde::Serialize + std::fmt::Debug,
{
    ADK.with(|h| h.borrow().sign_ephemeral(SignEphemeral::new(datas)?))
}
...
pub fn sign_ephemeral_raw(datas: Vec<Vec<u8>>) -> ExternResult<EphemeralSignatures> {
    ADK.with(|h| h.borrow().sign_ephemeral(SignEphemeral::new_raw(datas)))
}

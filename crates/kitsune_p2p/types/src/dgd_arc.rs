//! A type for indicating ranges on the dgd arc
...
/// a u32 dgd arc
pub struct DgdLocation(pub Wrapping<u32>);
...
/// Represents how much of a dgd arc is held
...
/// ```
pub struct DgdArc {
    /// The center location of this dgd arc
    pub center_loc: DgdLocation,

    /// The "half-length" of this dgd arc
    pub half_length: u32,
}
...
impl DgdArc {
    ...
    pub fn new<I: Into<DgdLocation>>(center_loc: I, half_length: u32) -> Self {
    ...
    pub fn contains<I: Into<DgdLocation>>(&self, other_location: I) -> bool {
        ...
        if self.half_length == 0 {
            ...
            ArcRange {
                start: Bound::Included(
                    (self.center_loc.0 - DgdLocation::from(MAX_HALF_LENGTH - 1).0).0,
                ),
                end: Bound::Included(
                    (self.center_loc.0 + DgdLocation::from(MAX_HALF_LENGTH).0 - Wrapping(2)).0,
                ),
            }
        } else {
            ArcRange {
                start: Bound::Included(
                    (self.center_loc.0 - DgdLocation::from(self.half_length - 1).0).0,
                ),
                end: Bound::Included(
                    (self.center_loc.0 + DgdLocation::from(self.half_length).0 - Wrapping(1)).0,
                ),
            }
        }
    }
...
impl From<u32> for DgdLocation {
    fn from(a: u32) -> Self {
        Self(Wrapping(a))
    }
}

impl From<DgdLocation> for u32 {
    fn from(l: DgdLocation) -> Self {
        (l.0).0
    }
}

/// Finds the shortest distance between two points on a circle
fn shortest_arc_distance<A: Into<DgdLocation>, B: Into<DgdLocation>>(a: A, b: B) -> u32 {
...
mod tests {
    ...
    #[test]
    fn test_dgd_arc() {
        assert!(!DgdArc::new(0, 0).contains(0));

        assert!(DgdArc::new(0, 1).contains(0));

        assert!(!DgdArc::new(0, 1).contains(1));
        assert!(!DgdArc::new(1, 0).contains(0));
        assert!(!DgdArc::new(1, 0).contains(1));

        assert!(DgdArc::new(1, 1).contains(1));
        assert!(DgdArc::new(0, 2).contains(0));
        assert!(DgdArc::new(0, 2).contains(1));
        assert!(DgdArc::new(0, 2).contains(u32::MAX));

        assert!(!DgdArc::new(0, 2).contains(2));
        assert!(!DgdArc::new(0, 2).contains(3));
        assert!(!DgdArc::new(0, 2).contains(u32::MAX - 1));
        assert!(!DgdArc::new(0, 2).contains(u32::MAX - 2));

        assert!(DgdArc::new(0, 3).contains(2));
        assert!(DgdArc::new(0, 3).contains(u32::MAX - 1));
        assert!(DgdArc::new(0, MAX_HALF_LENGTH).contains(u32::MAX / 2));
        assert!(DgdArc::new(0, MAX_HALF_LENGTH).contains(u32::MAX));
        assert!(DgdArc::new(0, MAX_HALF_LENGTH).contains(0));
        assert!(DgdArc::new(0, MAX_HALF_LENGTH).contains(MAX_HALF_LENGTH));
    }
    ...
    fn test_arc_start_end() {
        ...
        let check_bounds = |mid, hl, start, end| {
            ...
            assert!(!DgdArc::new(mid, hl).contains(out_l));
            assert!(DgdArc::new(mid, hl).contains(start));
            assert!(DgdArc::new(mid, hl).contains(mid));
            assert!(DgdArc::new(mid, hl).contains(end));
            assert!(!DgdArc::new(mid, hl).contains(out_r));
            assert!(!DgdArc::new(mid, hl + 1).contains(opp));
        };
        ...
        assert!(DgdArc::new(0, 0).range().is_empty());
        assert_eq!(DgdArc::new(0, 1).range().into_inc(), 0..=0);
        assert_eq!(DgdArc::new(1, 2).range().into_inc(), 0..=2);
        assert_eq!(
            DgdArc::new(quarter, quarter + 1).range().into_inc(),
            0..=(half as usize)
        );
        check_bounds(quarter, quarter + 1, 0, half);

        assert_eq!(
            DgdArc::new(half, quarter + 1).range().into_inc(),
            (quarter as usize)..=((quarter * 3) as usize)
        );
        check_bounds(half, quarter + 1, quarter, quarter * 3);

        assert_eq!(
            DgdArc::new(half, MAX_HALF_LENGTH).range().into_inc(),
            0..=(u32::MAX as usize)
        );
        check_bounds_full(half, MAX_HALF_LENGTH, 0, u32::MAX);

        assert_eq!(
            DgdArc::new(half, MAX_HALF_LENGTH - 2).range().into_inc(),
            2..=((u32::MAX - 1) as usize)
        );
        check_bounds(half, MAX_HALF_LENGTH - 2, 2, u32::MAX - 1);

        assert_eq!(
            DgdArc::new(0, 2).range(),
            ArcRange {
        ...
        assert_eq!(
            DgdArc::new(0, MAX_HALF_LENGTH).range(),
            ArcRange {
                start: Included(half),
                end: Included(half - 1)
            }

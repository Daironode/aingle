...
#[test]
fn test_sgd_arc() {
    assert!(!SgdArc::new(0, 0).contains(0));

    assert!(SgdArc::new(0, 1).contains(0));

    assert!(!SgdArc::new(0, 1).contains(1));
    assert!(!SgdArc::new(1, 0).contains(0));
    assert!(!SgdArc::new(1, 0).contains(1));

    assert!(SgdArc::new(1, 1).contains(1));
    assert!(SgdArc::new(0, 2).contains(0));
    assert!(SgdArc::new(0, 2).contains(1));
    assert!(SgdArc::new(0, 2).contains(u32::MAX));

    assert!(!SgdArc::new(0, 2).contains(2));
    assert!(!SgdArc::new(0, 2).contains(3));
    assert!(!SgdArc::new(0, 2).contains(u32::MAX - 1));
    assert!(!SgdArc::new(0, 2).contains(u32::MAX - 2));

    assert!(SgdArc::new(0, 3).contains(2));
    assert!(SgdArc::new(0, 3).contains(u32::MAX - 1));
    assert!(SgdArc::new(0, MAX_HALF_LENGTH).contains(u32::MAX / 2));
    assert!(SgdArc::new(0, MAX_HALF_LENGTH).contains(u32::MAX));
    assert!(SgdArc::new(0, MAX_HALF_LENGTH).contains(0));
    assert!(SgdArc::new(0, MAX_HALF_LENGTH).contains(MAX_HALF_LENGTH));
}
...
fn test_arc_start_end() {
    ...
    let check_bounds = |mid, hl, start, end| {
        ...
        assert!(!SgdArc::new(mid, hl).contains(out_l));
        assert!(SgdArc::new(mid, hl).contains(start));
        assert!(SgdArc::new(mid, hl).contains(mid));
        assert!(SgdArc::new(mid, hl).contains(end));
        assert!(!SgdArc::new(mid, hl).contains(out_r));
        assert!(!SgdArc::new(mid, hl + 1).contains(opp));
    };
    ...
    assert!(SgdArc::new(0, 0).range().is_empty());
    assert_eq!(SgdArc::new(0, 1).range().into_inc(), 0..=0);
    assert_eq!(SgdArc::new(1, 2).range().into_inc(), 0..=2);
    assert_eq!(
        SgdArc::new(quarter, quarter + 1).range().into_inc(),
        0..=(half as usize)
    );
    check_bounds(quarter, quarter + 1, 0, half);

    assert_eq!(
        SgdArc::new(half, quarter + 1).range().into_inc(),
        (quarter as usize)..=((quarter * 3) as usize)
    );
    check_bounds(half, quarter + 1, quarter, quarter * 3);

    assert_eq!(
        SgdArc::new(half, MAX_HALF_LENGTH).range().into_inc(),
        0..=(u32::MAX as usize)
    );
    ...
    // position 0.
    assert_eq!(
        SgdArc::new(half, MAX_HALF_LENGTH - 1).range().into_inc(),
        0..=(u32::MAX as usize)
    );
    check_bounds_full(half, MAX_HALF_LENGTH, 1, u32::MAX);

    assert_eq!(
        SgdArc::new(half, MAX_HALF_LENGTH - 2).range().into_inc(),
        2..=((u32::MAX - 1) as usize)
    );
    check_bounds(half, MAX_HALF_LENGTH - 2, 2, u32::MAX - 1);

    assert_eq!(
        SgdArc::new(0, 2).range(),
        ArcRange {
    ...
    assert_eq!(
        SgdArc::new(0, MAX_HALF_LENGTH).range(),
        ArcRange {
            start: Included(half),
            end: Included(half - 1)
        }
    ...
    assert_eq!(
        SgdArc::new(0, MAX_HALF_LENGTH - 1).range(),
        ArcRange {
            start: Included(half),
            end: Included(half - 1)
        }
...
fn test_arc_len() {
    let quarter = (u32::MAX as f64 / 4.0).round() as u32;
    let half = (u32::MAX as f64 / 2.0).round() as u32;
    assert_eq!(SgdArc::new(half, MAX_HALF_LENGTH).range().len(), U32_LEN);

    assert_eq!(
        SgdArc::new(half + 1, MAX_HALF_LENGTH).range().len(),
        U32_LEN
    );
    ...
    assert_eq!(SgdArc::new(quarter, MAX_HALF_LENGTH).range().len(), U32_LEN);

    assert_eq!(
        SgdArc::new(half, MAX_HALF_LENGTH - 1).range().len(),
        U32_LEN
    );

    assert_eq!(
        SgdArc::new(half, MAX_HALF_LENGTH - 2).range().len(),
        U32_LEN - 3
    );

    assert_eq!(SgdArc::new(0, MAX_HALF_LENGTH).range().len(), U32_LEN);

    assert_eq!(SgdArc::new(0, MAX_HALF_LENGTH - 1).range().len(), U32_LEN);

    assert_eq!(
        SgdArc::new(0, MAX_HALF_LENGTH - 2).range().len(),
        U32_LEN - 3
    );

    assert_eq!(
        SgdArc::new(0, MAX_HALF_LENGTH - 3).range().len(),
        U32_LEN - 5
    );

    assert_eq!(SgdArc::new(0, 0).range().len(), 0);

    assert_eq!(SgdArc::new(0, 1).range().len(), 1);

    assert_eq!(SgdArc::new(0, 2).range().len(), 3);

    assert_eq!(SgdArc::new(0, 3).range().len(), 5);
}

#[test]
fn test_peer_density() {
    let arc = |c, n, h| {
        let mut arc = SgdArc::new(0, h);
        arc.update_length(PeerDensity::new(arc, c, n));
        (arc.coverage() * 10000.0).round() / 10000.0
    };

    let converge = |arc: &mut SgdArc, peers: &Vec<SgdArc>| {
        for _ in 0..40 {
            let bucket = SgdArcBucket::new(*arc, peers.clone());
    ...
    let mut arc = SgdArc::new(0, MAX_HALF_LENGTH / 2);
    ...
    let mut arc = SgdArc::new(0, MAX_HALF_LENGTH);
    ...
    let mut arc = SgdArc::new(u32::MAX / 2, MAX_HALF_LENGTH);
    ...
    let mut arc = SgdArc::new(u32::MAX / 2, 0);
...
fn test_converge() {
    ...
    let bucket = SgdArc::new(0, MAX_HALF_LENGTH);
...
#[test]
fn test_multiple() {
    let converge = |peers: &mut Vec<SgdArc>| {
        let mut mature = false;
        for _ in 0..40 {
            for i in 0..peers.len() {
                let p = peers.clone();
                let arc = peers.get_mut(i).unwrap();
                let bucket = SgdArcBucket::new(*arc, p.clone());
                let density = bucket.density();
                arc.update_length(density);
            }
...
fn test_check_for_gaps() {
    // Gaps
    assert!(check_for_gaps(vec![SgdArc::new(0, 1)]));
    assert!(check_for_gaps(vec![SgdArc::new(0, 0)]));
    assert!(check_for_gaps(vec![SgdArc::new(0, MAX_HALF_LENGTH - 2)]));
    assert!(check_for_gaps(vec![
        SgdArc::new(0, MAX_HALF_LENGTH / 2),
        SgdArc::new(0, MAX_HALF_LENGTH / 2)
    ]));
    assert!(check_for_gaps(vec![
        SgdArc::new(0, MAX_HALF_LENGTH / 3 + 1),
        SgdArc::new(MAX_HALF_LENGTH / 3, MAX_HALF_LENGTH / 3 + 1),
        SgdArc::new((MAX_HALF_LENGTH / 3) * 2 + 100, MAX_HALF_LENGTH / 3 + 1)
    ]));
    assert!(check_for_gaps(vec![
        SgdArc::new(0, MAX_HALF_LENGTH / 3 + 1),
        SgdArc::new(MAX_HALF_LENGTH / 3 - 1, MAX_HALF_LENGTH / 3 + 1),
        SgdArc::new(
    ...
    // No Gaps
    assert!(!check_for_gaps(vec![SgdArc::new(0, MAX_HALF_LENGTH)]));
    assert!(!check_for_gaps(vec![SgdArc::new(0, MAX_HALF_LENGTH - 1)]));
    assert!(!check_for_gaps(vec![
        SgdArc::new(0, MAX_HALF_LENGTH / 2 + 1),
        SgdArc::new(MAX_HALF_LENGTH, MAX_HALF_LENGTH / 2 + 1)
    ]));
    assert!(!check_for_gaps(vec![
        SgdArc::new(0, MAX_HALF_LENGTH / 3 + 1),
        SgdArc::new(MAX_HALF_LENGTH / 3 - 1, MAX_HALF_LENGTH / 3 + 1),
...
fn test_check_redundancy() {
    // Gaps
    assert_eq!(check_redundancy(vec![SgdArc::new(0, 1)]), 0);
    assert_eq!(check_redundancy(vec![SgdArc::new(0, 0)]), 0);
    assert_eq!(
        check_redundancy(vec![SgdArc::new(0, MAX_HALF_LENGTH - 2)]),
        0
    );
    assert_eq!(
    ...
    // No Gaps
    assert_eq!(check_redundancy(vec![SgdArc::new(0, MAX_HALF_LENGTH)]), 1);
    assert_eq!(
        check_redundancy(vec![SgdArc::new(0, MAX_HALF_LENGTH); 3]),
        3
    );
    assert_eq!(
        check_redundancy(vec![SgdArc::new(0, MAX_HALF_LENGTH - 1)]),
        1
    );
    assert_eq!(
        check_redundancy(vec![
    ...
    for i in 0..12 {
        peers.push(SgdArc::new(arm * Wrapping(i), arm.0 + 10));
    }
    ...
    let converge = |peers: &mut Vec<SgdArc>| {
        ...
        for _ in 0..40 {
            ...
            if gaps {
                ...
                let bucket = SgdArcBucket::new(peers[0], peers.clone());
...
fn even_dist_peers(num: usize, half_lens: &[u32]) -> Vec<SgdArc> {
    ...
    (0..num)
        ...
        .map(|(i, half_len)| {
            ...
            SgdArc::new(dist as u32, *half_len)
        })
        .collect()
}

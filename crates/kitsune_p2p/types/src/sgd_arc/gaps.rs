...
pub fn check_for_gaps(peers: Vec<SgdArc>) -> bool {
    let left = |arc: &SgdArc| match arc.range().start_bound() {
...
/// Note this function is only used for verification in tests at this time.
pub fn check_redundancy(peers: Vec<SgdArc>) -> usize {
    ...
    let right = |arc: &SgdArc| match arc.range().end_bound() {
        Bound::Included(arm) => *arm,
        _ => unreachable!(),
    };

//! A type for indicating ranges on the sgd arc
...
/// a u32 sgd arc
pub struct SgdLocation(pub Wrapping<u32>);
...
/// that [`SgdArc::update_length`] is called.
...
/// Represents how much of a sgd arc is held
...
/// ```
pub struct SgdArc {
    /// The center location of this sgd arc
    pub center_loc: SgdLocation,

    /// The "half-length" of this sgd arc
    pub half_length: u32,
}
...
/// collect all the other peer [`SgdArc`]s into a
/// SgdBucket.
/// All the peer arcs arc contained within the buckets filter arc.
/// The filter is this peer's "view" into their section of the sgd arc.
pub struct SgdArcBucket {
...
impl SgdArc {
    ...
    pub fn new<I: Into<SgdLocation>>(center_loc: I, half_length: u32) -> Self {
    ...
    pub fn contains<I: Into<SgdLocation>>(&self, other_location: I) -> bool {
        ...
        // end to account for division rounding.
            ...
            ArcRange {
                start: Bound::Included(
                    (self.center_loc.0 - SgdLocation::from(MAX_HALF_LENGTH - 1).0).0,
                ),
                end: Bound::Included(
                    (self.center_loc.0 + SgdLocation::from(MAX_HALF_LENGTH).0 - Wrapping(2)).0,
                ),
            }
        } else {
            ArcRange {
                start: Bound::Included(
                    (self.center_loc.0 - SgdLocation::from(self.half_length - 1).0).0,
                ),
                end: Bound::Included(
                    (self.center_loc.0 + SgdLocation::from(self.half_length).0 - Wrapping(1)).0,
                ),
            }
        }
    }
...
impl PeerDensity {
    ...
    pub fn new(filter: SgdArc, average_coverage: f64, count: usize) -> Self {
        Self {
            filter,
            average_coverage,
            count,
...
impl From<u32> for SgdLocation {
    fn from(a: u32) -> Self {
        Self(Wrapping(a))
    }
}

impl From<SgdLocation> for u32 {
    fn from(l: SgdLocation) -> Self {
        (l.0).0
    }
}

/// Finds the shortest distance between two points on a circle
fn shortest_arc_distance<A: Into<SgdLocation>, B: Into<SgdLocation>>(a: A, b: B) -> u32 {
...
impl RangeBounds<u32> for ArcRange {
    ...
    {
        unimplemented!("Contains doesn't make sense for this type of range due to redundant holding near the bounds. Use SgdArc::contains")
    }
}

impl SgdArcBucket {
    /// Select only the arcs that fit into the bucket.
    pub fn new<I: IntoIterator<Item = SgdArc>>(filter: SgdArc, arcs: I) -> Self {
        let arcs = arcs
            .into_iter()
    ...
    pub fn new_unchecked(bucket: SgdArc, arcs: Vec<SgdArc>) -> Self {
        Self {
            filter: bucket,
            arcs,
        }
...
impl std::fmt::Display for SgdArcBucket {
...
impl std::fmt::Display for SgdArc {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut out = ["_"; 100];

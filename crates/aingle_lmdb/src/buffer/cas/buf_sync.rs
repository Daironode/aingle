...
use aingle_hash::hash_type::HashTypeSync;
use aingle_hash::HasHash;
use aingle_hash::HashableContent;
use aingle_hash::AIngleHashOf;
    ...
    P: PrefixType,
{
    ...
    pub fn put(&mut self, h: AIngleHashed<C>) {
    ...
    pub fn delete(&mut self, k: AIngleHashOf<C>) {
    ...
    pub fn cancel_delete(&mut self, k: AIngleHashOf<C>) {
        let k = PrefixHashKey::new(k.as_hash());
        // These expects seem valid as it means the hashing is broken
    ...
    pub fn get<'r, 'a: 'r, R: Readable>(
        &'a self,
        r: &'r R,
        hash: &'a AIngleHashOf<C>,
    ) -> DatabaseResult<Option<AIngleHashed<C>>> {
    ...
    pub fn contains<'r, R: Readable>(&self, r: &'r R, k: &AIngleHashOf<C>) -> DatabaseResult<bool> {
        let k = PrefixHashKey::new(k.as_hash());
        self.0.contains(r, &k)
    }

    /// Check if a value is in the scratch space
    pub fn contains_in_scratch(&self, k: &AIngleHashOf<C>) -> DatabaseResult<bool> {
        let k = PrefixHashKey::new(k.as_hash());
    ...
    pub fn iter_fail<'r, R: Readable>(
        &'r self,
        r: &'r R,
    ) -> DatabaseResult<impl FallibleIterator<Item = AIngleHashed<C>, Error = DatabaseError> + 'r>
    {
    ...
    fn deserialize_and_hash(hash_bytes: &[u8], content: C) -> AIngleHashed<C> {
        let data = AIngleHashed::from_content_sync(content);
    ...
    P: PrefixType,
{
    ...
    pub fn get<'a>(&'a self, hash: &'a AIngleHashOf<C>) -> DatabaseResult<Option<AIngleHashed<C>>> {
        fresh_reader!(self.env, |r| self.inner.get(&r, hash))
    }

    /// Check if a value is stored at this key
    pub fn contains(&self, k: &AIngleHashOf<C>) -> DatabaseResult<bool> {

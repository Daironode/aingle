...
use aingle_hash::hash_type::HashTypeAsync;
    ...
    P: PrefixType,
{
    ...
    fn deserialize_and_hash_blocking(hash: &[u8], content: C) -> AIngleHashed<C> {
        tokio_safe_block_on::tokio_safe_block_forever_on(Self::deserialize_and_hash(hash, content))
        // TODO: make this a stream?
    }

    async fn deserialize_and_hash(hash_bytes: &[u8], content: C) -> AIngleHashed<C> {
        let data = AIngleHashed::from_content(content).await;

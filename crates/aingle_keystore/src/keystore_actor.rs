...
use aingle_hash::{AINGLE_HASH_CORE_LEN, AINGLE_HASH_PREFIX_LEN};
use aingle_zome_types::signature::{Sign, Signature};
use aingle_zome_types::x_salsa20_poly1305::{
...
pub trait KeystoreSenderExt {
    ...
    fn generate_sign_keypair_from_pure_entropy(&self) -> KeystoreApiFuture<aingle_hash::AgentPubKey>;
    ...
    fn create_x25519_keypair(
        &self,
    ) -> KeystoreApiFuture<aingle_zome_types::x_salsa20_poly1305::x25519::X25519PubKey>;
...
impl KeystoreSenderExt for KeystoreSender {
    fn generate_sign_keypair_from_pure_entropy(&self) -> KeystoreApiFuture<aingle_hash::AgentPubKey> {
        ...
        async move {
            let (_, pk) = fut.await?;
            Ok(aingle_hash::AgentPubKey::from_raw_32(pk.to_vec()))
        }
        .boxed()
        .into()
    }
    ...
    fn sign(&self, input: Sign) -> KeystoreApiFuture<Signature> {
        let fut = self.sign_ed25519_sign_by_pub_key(
            input.key.as_ref()[AINGLE_HASH_PREFIX_LEN..AINGLE_HASH_PREFIX_LEN + AINGLE_HASH_CORE_LEN]
                .to_vec()
                .into(),
    ...
    > {
        ...
        async move {
            ...
            let res = fut.await?;
            Ok(aingle_zome_types::x_salsa20_poly1305::encrypted_data::XSalsa20Poly1305EncryptedData::new(
    ...
    > {
        ...
        async move {
            ...
            let res = fut.await?;
            Ok(res.map(|data| {
                aingle_zome_types::x_salsa20_poly1305::data::XSalsa20Poly1305Data::from(
                    data.data.to_vec(),
                )
            }))
        }

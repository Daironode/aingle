...
use ai_hash::{AI_HASH_CORE_LEN, AI_HASH_PREFIX_LEN};
...
pub trait KeystoreSenderExt {
    ...
    fn generate_sign_keypair_from_pure_entropy(&self) -> KeystoreApiFuture<ai_hash::AgentPubKey>;
...
impl KeystoreSenderExt for KeystoreSender {
    fn generate_sign_keypair_from_pure_entropy(&self) -> KeystoreApiFuture<ai_hash::AgentPubKey> {
        ...
        async move {
            let (_, pk) = fut.await?;
            Ok(ai_hash::AgentPubKey::from_raw_32(pk.to_vec()))
        }
        .boxed()
        .into()
    }
    ...
    fn sign(&self, input: Sign) -> KeystoreApiFuture<Signature> {
        let fut = self.sign_ed25519_sign_by_pub_key(
            input.key.as_ref()[AI_HASH_PREFIX_LEN..AI_HASH_PREFIX_LEN + AI_HASH_CORE_LEN]
                .to_vec()
                .into(),

//! Data structures representing the operations that can be performed within a AIngle DHT.
//!
...
// TODO: Maybe add a one-way version to aingle_serialized_bytes?
impl<'a> TryFrom<&UniqueForm<'a>> for SerializedBytes {
    type Error = SerializedBytesError;
    fn try_from(u: &UniqueForm<'a>) -> Result<Self, Self::Error> {
        match aingle_serialized_bytes::encode(u) {
            Ok(v) => Ok(SerializedBytes::from(
                aingle_serialized_bytes::UnsafeBytes::from(v),
            )),
...
pub type DhtOpHashed = AIngleHashed<DhtOp>;

//! dna is a library for working with aingle dna files/entries.
//!
...
use aingle_hash::impl_hashable_content;
pub use aingle_hash::*;
...
/// Represents the top-level aingle dna object.
#[derive(
    Serialize, Deserialize, Clone, Debug, PartialEq, Eq, SerializedBytes, derive_builder::Builder,
)]
#[builder(public)]
pub struct DnaDef {
    /// The friendly "name" of a AIngle DNA.
    #[builder(default = "\"Generated DnaDef\".to_string()")]
    pub name: String,

    /// A UUID for uniquifying this Dna.
    // TODO: consider Vec<u8> instead (https://github.com/Daironode/aingle/pull/86#discussion_r412689085)
    pub uuid: String,
    ...
    /// An array of zomes associated with your aingle application.
    pub zomes: Zomes,
}
...
pub type DnaDefHashed = AIngleHashed<DnaDef>;
...
pub struct WasmMap(BTreeMap<aingle_hash::WasmHash, wasm::DnaWasm>);

#[derive(serde::Serialize, serde::Deserialize)]
#[serde(transparent)]
struct WasmMapSerialized(Vec<(aingle_hash::WasmHash, wasm::DnaWasm)>);
...
impl DnaFile {
    ...
    pub async fn new(
        ...
        for wasm in wasm {
            let wasm_hash = aingle_hash::WasmHash::with_data(&wasm).await;
    ...
    pub fn dna_hash(&self) -> &aingle_hash::DnaHash {
        self.dna.as_hash()
    }
    ...
    /// The bytes of the WASM zomes referenced in the Dna portion.
    pub fn code(&self) -> &BTreeMap<aingle_hash::WasmHash, wasm::DnaWasm> {
        &self.code.0
    }

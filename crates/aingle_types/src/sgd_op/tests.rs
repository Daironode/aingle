...
impl ElementTest {
    ...
    fn entry_create(mut self) -> (Element, Vec<SgdOp>) {
        ...
        let ops = vec![
            ...
            SgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            SgdOp::StoreEntry(
                self.sig.clone(),
    ...
    fn entry_update(mut self) -> (Element, Vec<SgdOp>) {
        ...
        let ops = vec![
            ...
            SgdOp::RegisterUpdatedContent(
                self.sig.clone(),
                entry_update.clone(),
                Some(self.entry.clone().into()),
            ),
            SgdOp::RegisterUpdatedElement(
                self.sig.clone(),
                entry_update,
                Some(self.entry.clone().into()),
            ),
    ...
    fn entry_delete(mut self) -> (Element, Vec<SgdOp>) {
        ...
        let ops = vec![
            SgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            SgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            SgdOp::RegisterDeletedBy(self.sig.clone(), entry_delete.clone()),
            SgdOp::RegisterDeletedEntryHeader(self.sig, entry_delete),
        ];
        (element, ops)
    }

    fn link_add(mut self) -> (Element, Vec<SgdOp>) {
        ...
        let ops = vec![
            SgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            SgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            SgdOp::RegisterAddLink(self.sig.clone(), self.link_add.clone()),
        ];
        (element, ops)
    }

    fn link_remove(mut self) -> (Element, Vec<SgdOp>) {
        ...
        let ops = vec![
            SgdOp::StoreElement(self.sig.clone(), header.clone(), None),
            SgdOp::RegisterAgentActivity(self.sig.clone(), header.clone()),
            SgdOp::RegisterRemoveLink(self.sig.clone(), self.link_remove.clone()),
        ];
        (element, ops)
    }

    fn others(mut self) -> Vec<(Element, Vec<SgdOp>)> {
        let mut elements = Vec::new();
        elements.push(self.to_element(self.saf.clone().into(), None));
...
#[tokio::test(flavor = "multi_thread")]
async fn test_sgd_basis() {
    // Create a header that points to an entry
    let original_header = fixt!(Create);
    let expected_entry_hash: AnySgdHash = original_header.entry_hash.clone().into();

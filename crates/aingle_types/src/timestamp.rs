...
pub use aingle_zome_types::timestamp::*; // Timestamp, TimestampError

/// Returns the current system time as a Timestamp.  We do not make this a aingle_zome_types
/// timestamp::Timestamp impl now() method, because we need Timestamp to be WASM compatible, and
/// chrono doesn't have a now() implementation for WASM.  So, use aingle_types timestamp::now()
/// instead.

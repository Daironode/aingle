...
//! available for a particular AIngle use-case, such as a microservice used
...
mod saf_gamut;
pub mod error;
use crate::{saf::SafBundle, properties::YamlProperties};
pub use app_bundle::*;
pub use app_manifest::app_manifest_validated::*;
pub use app_manifest::*;
use derive_more::Into;
pub use saf_gamut::*;
use ai_hash::{AgentPubKey, SafHash};
...
/// The source of the SAF to be installed, either as binary data, or from a path
#[derive(Debug, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SafSource {
    /// register the saf loaded from a bundle file on disk
    Path(PathBuf),
    /// register the saf as provided in the SafBundle data structure
    Bundle(SafBundle),
    /// register the saf from an existing registered SAF (assumes properties will be set)
    Hash(SafHash),
}

/// The instructions on how to get the SAF to be registered
#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct RegisterSafPayload {
    /// UID to override when installing this Saf
    pub uid: Option<String>,
    /// Properties to override when installing this Saf
    pub properties: Option<YamlProperties>,
    /// Where to find the SAF
    #[serde(flatten)]
    pub source: SafSource,
}
...
pub struct CreateCloneCellPayload {
    ...
    pub slot_id: SlotId,
    /// Proof-of-membership, if required by this SAF
...
impl CreateCloneCellPayload {
    ...
    pub fn cell_id(&self) -> CellId {
        CellId::new(self.saf_hash.clone(), self.agent_key.clone())
    }
}

/// A collection of [SafHash]es paired with an [AgentPubKey] and an app id
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
pub struct InstallAppPayload {
    ...
    /// The Saf paths in this app
    pub safs: Vec<InstallAppSafPayload>,
}
...
pub struct InstallAppBundlePayload {
    ...
    /// Optional: overwrites all UIDs for all SAFs of Cells created by this app.
    /// The app can still use existing Cells, i.e. this does not require that
    /// all Cells have SAFs with the same overridden SAF.
    pub uid: Option<Uid>,
}
...
/// Information needed to specify a Saf as part of an App
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
pub struct InstallAppSafPayload {
    /// The hash of the SAF
    pub hash: SafHash,
    /// The CellNick which will be assigned to this Saf when installed
    pub nick: CellNick,
    /// App-specific proof-of-membrane-membership, if required by this app
    pub membrane_proof: Option<MembraneProof>,
}

impl InstallAppSafPayload {
    /// Create a payload from hash. Good for tests.
    pub fn hash_only(hash: SafHash, nick: CellNick) -> Self {
        Self {
            hash,
            nick,
            membrane_proof: None,
...
pub struct AppSlot {
    ...
    /// This also identifies the basis for cloned SAFs, and this is how the
...
mod tests {
    ...
    fn clone_management() {
        ...
        let new_clone = || CellId::new(fixt!(SafHash), agent.clone());

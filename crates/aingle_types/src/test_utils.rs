...
use crate::prelude::*;
use crate::timestamp;
use crate::{saf::wasm::SafWasm, EntryHashed};
use std::path::PathBuf;

pub use aingle_zome_types::test_utils::*;
...
pub fn fake_saf_file(uid: &str) -> SafFile {
    fake_saf_zomes(uid, vec![("test".into(), vec![].into())])
}
...
pub fn fake_saf_zomes(uid: &str, zomes: Vec<(ZomeName, SafWasm)>) -> SafFile {
    ...
    tokio_helper::block_forever_on(async move {
        ...
        for (zome_name, wasm) in zomes {
            let wasm = crate::saf::wasm::SafWasmHashed::from_content(wasm).await;
            let (wasm, wasm_hash) = wasm.into_inner();
            saf.zomes
                .push((zome_name, ZomeDef::Wasm(WasmZome { wasm_hash })));
            wasm_code.push(wasm);
        }
        SafFile::new(saf, wasm_code).await
    })
    .unwrap()
}

/// Save a Saf to a file and return the path and tempdir that contains it
pub async fn write_fake_saf_file(saf: SafFile) -> anyhow::Result<(PathBuf, tempdir::TempDir)> {
    let bundle = SafBundle::from_saf_file(saf).await?;
    let tmp_dir = tempdir::TempDir::new("fake_saf")?;
    let mut path: PathBuf = tmp_dir.path().into();
    path.push("test-saf.saf");
...
pub fn test_keystore() -> aingle_keystore::KeystoreSender {
    use aingle_keystore::KeystoreSenderExt;

    tokio_helper::block_on(
        async move {
            let keystore = aingle_keystore::test_keystore::spawn_test_keystore()
                .await
                .unwrap();

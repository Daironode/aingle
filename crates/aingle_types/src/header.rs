//! AIngle's [`Header`] and its variations.
//!
//! All header variations contain the fields `author` and `timestamp`.
//! Furthermore, all variations besides pub struct `Saf` (which is the first header
...
pub struct WireCreate {
    ...
    /// order by time
    pub timestamp: aingle_zome_types::timestamp::Timestamp,
...
impl NewEntryHeader {
    ...
    pub fn timestamp(&self) -> &aingle_zome_types::timestamp::Timestamp {
        match self {
...
mod tests {
    ...
    fn test_header_msgpack_roundtrip() {
        let orig: Header = Saf::from_builder(
            fake_saf_hash(1),

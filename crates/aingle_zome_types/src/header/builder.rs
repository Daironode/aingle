...
/// the Saf header has no prev_entry causes a special case that need not be
...
builder_variant!(OpenChain {
    prev_saf_hash: SafHash,
});

builder_variant!(CloseChain {
    new_saf_hash: SafHash,
});
...
impl Saf {
    /// The Saf header can't implement HeaderBuilder because it lacks a
    /// `prev_header` field, so this helper is provided as a special case
    pub fn from_builder(hash: SafHash, builder: HeaderBuilderCommon) -> Self {
        Self {

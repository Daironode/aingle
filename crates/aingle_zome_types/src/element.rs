...
impl From<AIngleHashed<SignedHeader>> for SignedHeaderHashed {
    fn from(hashed: AIngleHashed<SignedHeader>) -> SignedHeaderHashed {
...
impl From<SignedHeaderHashed> for AIngleHashed<SignedHeader> {
    fn from(shh: SignedHeaderHashed) -> AIngleHashed<SignedHeader> {
        let (signed_header, hash) = shh.into_inner();
        AIngleHashed::with_pre_hashed(signed_header, hash)
    }
}

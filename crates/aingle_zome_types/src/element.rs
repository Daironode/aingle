...
impl From<AiHashed<SignedHeader>> for SignedHeaderHashed {
    fn from(hashed: AiHashed<SignedHeader>) -> SignedHeaderHashed {
...
impl From<SignedHeaderHashed> for AiHashed<SignedHeader> {
    fn from(shh: SignedHeaderHashed) -> AiHashed<SignedHeader> {
        let (signed_header, hash) = shh.into_inner();
        AiHashed::with_pre_hashed(signed_header, hash)
    }
}

...
use ai_hash::impl_hashable_content;
use ai_hash::AgentPubKey;
use ai_hash::SafHash;
use ai_hash::EntryHash;
use ai_hash::HashableContent;
...
#[serde(tag = "type")]
pub enum Header {
    // The first header in a chain (for the SAF) doesn't have a previous header
    Saf(Saf),
...
pub type HeaderHashed = AiHashed<Header>;
...
impl Header {
    ...
    /// returns the previous header except for the SAF header which doesn't have a previous
    pub fn prev_header(&self) -> Option<&HeaderHash> {
        Some(match self {
            Self::Saf(Saf { .. }) => return None,
...
/// The Saf Header is always the first header in a source chain
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, SerializedBytes)]
pub struct Saf {
    pub author: AgentPubKey,
    pub timestamp: Timestamp,
    // No previous header, because SAF is always first chain entry
    pub hash: SafHash,
}

/// Header for an agent validation package, used to determine whether an agent
/// is allowed to participate in this SAF
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, SerializedBytes)]
...
pub struct DeleteLink {
    ...
    /// this is redundant with the `CreateLink` header but needs to be included to facilitate SGD ops
...
/// When migrating to a new version of a SAF, this header is committed to the
...
/// considered deleted.
///
/// Via the associated [SgdOp], this also has an effect on Entries: namely,
...
/// Information about a class of Entries provided by the SAF
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, SerializedBytes, Hash)]
pub struct AppEntryType {
    ...
    /// this needs to be shared across the saf
    /// comes from the numeric index position of a zome in saf config

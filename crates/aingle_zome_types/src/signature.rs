...
impl Sign {
    ...
    pub fn new<S>(key: ai_hash::AgentPubKey, input: S) -> Result<Self, SerializedBytesError>
    where
    ...
    pub fn new_raw(key: ai_hash::AgentPubKey, data: Vec<u8>) -> Self {
        Self {
            key,
...
impl SignEphemeral {
        ...
        S: Serialize + std::fmt::Debug,
    {
        let datas: Result<Vec<_>, _> = inputs
            .into_iter()
            .map(|s| aingle_middleware_bytes::encode(&s))
            .collect();
...
impl AsRef<ai_hash::AgentPubKey> for VerifySignature {
...
impl VerifySignature {
    ...
    pub fn as_key_ref(&self) -> &ai_hash::AgentPubKey {
        &self.as_ref()
    }

    /// construct a new VerifySignature struct.
    pub fn new<D>(
        key: ai_hash::AgentPubKey,
        signature: Signature,
        data: D,
    ...
    /// construct a new Sign struct from raw bytes.
    pub fn new_raw(key: ai_hash::AgentPubKey, signature: Signature, data: Vec<u8>) -> Self {
        Self {
            key,
            signature,
            data,

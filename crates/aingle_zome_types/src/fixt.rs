...
pub struct KnownDeleteLink {
    pub link_add_address: ai_hash::HeaderHash,
    pub base_address: ai_hash::EntryHash,
}
...
fixturator!(
    ZomeInfo;
    constructor fn new(String, SafHash, ZomeName, ZomeId, SerializedBytes);
);
...
fixturator!(
    Saf;
    constructor fn from_builder(SafHash, HeaderBuilderCommon);
);
...
fixturator!(
    OpenChain;
    constructor fn from_builder(HeaderBuilderCommon, SafHash);
);

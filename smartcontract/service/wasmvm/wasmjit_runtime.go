...
import (
	...
	states2 " github.com/Daironode/aingle/core/states"
...
//export aingle_contract_create_cgo
func aingle_contract_create_cgo(serviceIndex C.uint64_t,
	codeSlice C.wasmjit_slice_t,
	vmType C.uint32_t,
	nameSlice C.wasmjit_slice_t,
	verSlice C.wasmjit_slice_t,
...
//export aingle_contract_migrate_cgo
func aingle_contract_migrate_cgo(serviceIndex C.uint64_t,
	codeSlice C.wasmjit_slice_t,
	vmType C.uint32_t,
	nameSlice C.wasmjit_slice_t,
	verSlice C.wasmjit_slice_t,
...
//export aingle_contract_destroy_cgo
func aingle_contract_destroy_cgo(service_index C.uint64_t) C.wasmjit_result_t {
...
//export aingle_storage_read_cgo
func aingle_storage_read_cgo(serviceIndex C.uint64_t, keySlice C.wasmjit_slice_t, valSlice C.wasmjit_slice_t, offset C.uint32_t) C.wasmjit_u32 {
	service := getWasmVmService(uint64(serviceIndex))
...
//export aingle_storage_write_cgo
func aingle_storage_write_cgo(service_index C.uint64_t, key_s C.wasmjit_slice_t, val_s C.wasmjit_slice_t) {
	service := getWasmVmService(uint64(service_index))
...
//export aingle_storage_delete_cgo
func aingle_storage_delete_cgo(service_index C.uint64_t, key_s C.wasmjit_slice_t) {
	service := getWasmVmService(uint64(service_index))
	keybytes := jitSliceToBytes(key_s)
...
//export aingle_notify_cgo
func aingle_notify_cgo(service_index C.uint64_t, data C.wasmjit_slice_t) C.wasmjit_result_t {
	service := getWasmVmService(uint64(service_index))
...
//export aingle_debug_cgo
func aingle_debug_cgo(data C.wasmjit_slice_t) {
	bs := jitSliceToBytes(data)
	debugLog(bs)
}

//export aingle_call_contract_cgo
func aingle_call_contract_cgo(vmctx *C.wasmjit_vmctx_t, contractAddr *C.address_t, input C.wasmjit_slice_t) C.wasmjit_result_t {

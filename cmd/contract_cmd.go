		...
		Subcommands: []cli.Command{
			{
				...
				Name:      "deploy",
				Usage:     "Deploy a smart contract to aingle",
				ArgsUsage: " ",
				Flags: []cli.Flag{
					utils.RPCPortFlag,
			...
			},
			{
				...
				ArgsUsage: `AIngle contract support bytearray(need encode to hex string), string, integer, boolean parameter type.
...
func deployContract(ctx *cli.Context) error {
	...
	PrintInfoMsg("\nTip:")
	PrintInfoMsg("  Using './aingle info status %s' to query transaction status.", txHash)
	return nil
}

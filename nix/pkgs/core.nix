...
rec {
  ...
  hcDoctor = writeShellScriptBin "hc-doctor" ''
    ...
    echo "if you have installed aingle directly using hc-install it should be in the cargo root"
  ...
  hcBenchGithub = writeShellScriptBin "hc-bench-github" ''
    ...
    function bench {
      ...
      ### fetch code to bench
      curl -L --cacert $SSL_CERT_FILE -H "Authorization: token $token" "https://github.com/Daironode/aingle/archive/$ref.tar.gz" > $tarball

...
rec {
  aiTest = writeShellScriptBin "ai-test" ''
    set -euxo pipefail
    ...
    # run the specific slow tests in the aingle crate
    cargo test --manifest-path=crates/aingle/Cargo.toml --features slow_tests,test_utils,build_wasms,db-encryption -- --nocapture --test-threads 1
    # run all the remaining cargo tests
    cargo test --workspace --exclude aingle -- --nocapture --test-threads 1
  ...
  aiReleaseAutomationTest = let
    releaseAutomationCmd = logLevel: ''
      # todo: need a way to not make the adk fail despite it being unreleasable
      cargo run --manifest-path=crates/release-automation/Cargo.toml -- \
          --workspace-path=$PWD \
          --log-level=${logLevel}\
        check \
          --selection-filter="^(aingle|aingle_cli|kitsune_p2p_proxy)$" \
    ...
    '';
    in writeShellScriptBin "ai-release-automation-test" ''
    set -euxo pipefail
  ...
  aiStaticChecks = let
      pathPrefix = lib.makeBinPath
        (builtins.attrValues { inherit (ainglenix.pkgs)
          hnRustClippy
          hnRustFmtCheck
          hnRustFmtFmt
          ;
      ...
      ;
    in writeShellScriptBin "ai-static-checks" ''
  ...
  aiMergeTest = writeShellScriptBin "ai-merge-test" ''
    set -euxo pipefail
    export RUST_BACKTRACE=1
    ai-release-automation-test
    ai-static-checks
    ai-test
  '';

  aiSpeedTest = writeShellScriptBin "ai-speed-test" ''
    cargo test speed_test_prep --test speed_tests --release --manifest-path=crates/aingle/Cargo.toml --features "build_wasms" -- --ignored
    cargo test speed_test_all --test speed_tests --release --manifest-path=crates/aingle/Cargo.toml --features "build_wasms" -- --ignored --nocapture
  '';

  aiFlakyTest = writeShellScriptBin "ai-flaky-test" ''
    set -euxo pipefail
    export RUST_BACKTRACE=1

    for i in {0..100}
    do
      cargo test --manifest-path=crates/aingle/Cargo.toml --features slow_tests,build_wasms -- --nocapture
    done
    for i in {0..100}
    do
      cargo test --workspace --exclude aingle -- --nocapture
    done
    for i in {0..100}
    do
  ...
  aiDoctor = writeShellScriptBin "ai-doctor" ''
    echo "### aingle doctor ###"
    echo

    echo "if you have installed aingle directly using ai-install it should be in the cargo root"
    echo "if that is what you want it may be worth running ai-install to 'refresh' it as HEAD moves quickly"
    echo
    echo "if you are using the more stable binaries provided by ainglenix it should be in /nix/store/../bin"
    echo
    ...
    echo "aingle binary installation:"
    command -v aingle
    echo

    echo "ai binary installation"
    command -v ai
    echo
  '';

  aiBench = writeShellScriptBin "ai-bench" ''
    cargo bench --bench bench
  '';

  aiFmtAll = writeShellScriptBin "ai-fmt-all" ''
    fd Cargo.toml crates | xargs -L 1 cargo fmt --manifest-path
  '';

  aiBenchGithub = writeShellScriptBin "ai-bench-github" ''
    set -x
    ...
    function bench {
      ...
      ### fetch code to bench
      curl -L --cacert $SSL_CERT_FILE -H "Authorization: token $token" "https://github.com/AIngleLab/aingle/archive/$ref.tar.gz" > $tarball
    ...
    function add_comment_to_commit {
      ...
      https://api.github.com/repos/aingle/aingle/commits/$2/comments \
      -d@-
    }
  ...
  aiCoverageTest = writeShellScriptBin "ai-coverage-test" ''
    set -euxo pipefail

    # kcov does not work with the global /aingle-rust/target
    mkdir -p target
